 ...
 *
 * $Id: wlc_amsdu.c 778733 2019-09-09 16:39:23Z $
 */
...
enum {
	...
	IOV_RX_AMSDU_IN_AMPDU_OVR, /**<Overrides rx_amsdu_in_ampdu setting */
...
#endif /* WLCNT */
#endif /* BCMDBG */
	{"rx_amsdu_in_ampdu_ovr", IOV_RX_AMSDU_IN_AMPDU_OVR,
...
struct amsdu_info {
	...
	amsdu_dbg_t *amdbg;
#endif // endif
	bool rx_amsdu_in_ampdu_ovr;  /**< when set, override rx amsdu in ampdu setting */
};
...
typedef struct {
	...
	uint amsdu_aggsf;	/* Maximum allowed subframes per A-MSDU */
...
static uint32 wlc_amsdu_scb_max_agg_len_upd(amsdu_info_t *ami, struct scb *scb);
static void wlc_amsdu_scb_aggsf_upd(amsdu_info_t *ami, struct scb *scb);
...
static void wlc_amsdu_scb_state_upd(void *ctx, scb_state_upd_data_t *notif_data);
#if defined(BCMDBG) || defined(BCMDBG_AMSDU)
...
wlc_amsdu_set_scb_default_txpolicy_all(amsdu_info_t *ami)
{
	...
	FOREACHSCB(ami->wlc->scbstate, &scbiter, scb) {
		...
		wlc_amsdu_scb_aggsf_upd(ami, scb);
	}
	...
	if (AMSDU_TX_ENAB(wlc->pub)) {
		...
		if (!err && bsscfg) {
			...
			if (scb && SCB_AMSDU(scb)) {
				...
				wlc_amsdu_dotxstatus(scb_ami, scb, pkt);
			} else {
...
BCMATTACHFN(wlc_amsdu_attach)(wlc_info_t *wlc)
{
	...
	if (D11REV_IS(wlc->pub->corerev, 128)) {
		ami->rx_amsdu_in_ampdu_ovr = TRUE;
	}
...
BCMATTACHFN(wlc_amsdu_tx_attach)(amsdu_info_t *ami, wlc_info_t *wlc)
{
	...
	if ((err = wlc_scb_state_upd_register(wlc, wlc_amsdu_scb_state_upd, ami)) != BCME_OK) {
		WL_ERROR(("wl%d: %s: unable to register callback %p\n",
		          wlc->pub->unit, __FUNCTION__, wlc_amsdu_scb_state_upd));
...
wlc_amsdu_scb_init(void *context, struct scb *scb)
{
	...
	for (i = 0; i < NUMPRIO; i++) {
		...
		scb_txpolicy->amsdu_aggsf = ami->txpolicy.amsdu_max_sframes;
	}
 ...
 */
static void
wlc_amsdu_scb_state_upd(void *ctx, scb_state_upd_data_t *notif_data)
{
	...
	void *p, uint plen, void *a, uint alen, uint val_size, struct wlc_if *wlcif)
{
	...
	case IOV_SVAL(IOV_RX_AMSDU_IN_AMPDU):
		...
		break;
	case IOV_GVAL(IOV_RX_AMSDU_IN_AMPDU_OVR):
		int_val = (int8)(ami->rx_amsdu_in_ampdu_ovr);
		bcopy(&int_val, a, val_size);
		break;

	case IOV_SVAL(IOV_RX_AMSDU_IN_AMPDU_OVR):
		if (D11REV_IS (wlc->pub->corerev, 128)) {
			ami->rx_amsdu_in_ampdu_ovr = bool_val;
			wlc_bmac_swcap_set(wlc->hw, MSWCAP_DEAGG_OFF_NBIT, bool_val);
		} else {
			WL_AMSDU(("wl%d: unsupported corerev (%d)\n", wlc->pub->unit,
				wlc->pub->corerev));
			err = BCME_UNSUPPORTED;
		}
 ...
 * driver need to update amsdu_vht_agg_bytes_max value.
 */
void BCMFASTPATH
...
wlc_amsdu_scb_vht_agglimit_upd(amsdu_info_t *ami, struct scb *scb)
{
	...
	max_agg_len = wlc_amsdu_scb_max_agg_len_upd(ami, scb);
	vht_pref = MIN(vht_pref, max_agg_len);
#ifdef BCMDBG
...
#define _RSPEC2RATE(r)  wf_rspec_to_rate(r) /* Make a static table later */
	uint byte_limit;
	BCM_REFERENCE(scb);
	BCM_REFERENCE(p);
...
uint32 BCMFASTPATH
wlc_amsdu_scb_max_sframes(amsdu_info_t *ami, struct scb *scb, uint8 tid)
{
	scb_amsdu_t *scb_ami;
	scb_ami = SCB_AMSDU_CUBBY(ami, scb);

	return scb_ami->scb_txpolicy[tid].pub.amsdu_max_sframes;
}
...
wlc_amsdu_scb_max_agg_len(amsdu_info_t *ami, struct scb *scb, uint8 tid)
{
	...
	return scb_ami->scb_txpolicy[tid].amsdu_vht_agg_bytes_max;
}
...
wlc_amsdu_scb_max_agg_len_upd(amsdu_info_t *ami, struct scb *scb)
{
	wlc_info_t *wlc;
	uint8 nss, mcs, i;
	ratespec_t rspec;
...
wlc_amsdu_scb_aggsf_upd(amsdu_info_t *ami, struct scb *scb)
{
	...
	/* XXX: Increase amsdu_aggsf value only if bandwidth is 160Mhz for now.
	 * Could apply the change to other bandwidth in the future. (ex. 80Mhz)
	 */
	if (bw == BW_160MHZ) {
		 ...
		 */
		amsdu_aggsf = MIN(ami->txpolicy.amsdu_max_sframes, 2);
		for (i = 0; i < NUMPRIO; i++) {
			scb_ami->scb_txpolicy[i].pub.amsdu_max_sframes = amsdu_aggsf;
		}
	}

	for (i = 0; i < NUMPRIO; i++) {
		scb_ami->scb_txpolicy[i].amsdu_aggsf = amsdu_aggsf;
	}
}
...
wlc_amsdu_scb_aggsf(amsdu_info_t *ami, struct scb *scb, uint8 tid)
{
	...
	return scb_ami->scb_txpolicy[tid].amsdu_aggsf;
}
#endif /* WLAMSDU_TX */
	...
	uint8 tid, uint32 lifetime)
{
	...
	if (scb_ami->scb_txpolicy[tid].pub.amsdu_max_sframes <= 1) {
	...
	uint8 tid, uint32 lifetime)
{
	...
	while (1) {
		...
		if (i >= scb_ami->scb_txpolicy[tid].pub.amsdu_max_sframes) {
			WLCNTINCR(ami->cnt->agg_stop_sf);
			break;
...
wlc_amsdu_agg(void *ctx, struct scb *scb, void *p, uint prec)
{
	...
	if ((totlen > maxagglen - ETHER_HDR_LEN) ||
		...
		scb_ami->scb_txpolicy[tid].pub.amsdu_max_sframes)) {
...
wlc_amsdu_agg_close(amsdu_info_t *ami, struct scb *scb, uint tid)
{
	...
	if (aggstate->amsdu_agg_padlast) {
		...
		BCM_REFERENCE(osh);
		/* pad data will be the garbage at the end of host data */
		PKTSETFRAGLEN(osh, aggstate->amsdu_agg_ptail, 1,
			PKTFRAGLEN(osh, aggstate->amsdu_agg_ptail, 1) -
...
wlc_amsdu_agg_open(amsdu_info_t *ami, wlc_bsscfg_t *bsscfg, struct ether_addr *ea, void *p)
{
	...
	ASSERT(headroom <= (uint)PKTHEADROOM(wlc->osh, p));
	...
	ratespec_t rspec)
{
	...
	if (aggstate->amsdu_agg_p == NULL) {
		 ...
		 * Now, this msdu has all DHDHDR 22B in host.
		 */
		PKTSETFRAGDATA_LO(osh, p, 1,
			PKTFRAGDATA_LO(osh, p, 1) - ETHER_HDR_LEN);
		PKTSETFRAGLEN(osh, p, 1,
			PKTFRAGLEN(osh, p, 1) + ETHER_HDR_LEN);
	...
	if (pad != 0) {
		...
		if (totlen + pad < max_agg_bytes) {
			...
			/* pad data will be the garbage at the end of host data */
			PKTSETFRAGLEN(osh, p, 1, PKTFRAGLEN(osh, p, 1) + pad);
...
#if defined(WL_DATAPATH_LOG_DUMP) && defined(WLAMSDU_TX) && \
...
#endif /* WL_DATAPATH_LOG_DUMP && WLAMSDU_TX && !WLAMSDU_TX_DISABLED */
...
/* Get rx amsdu in ampdu override setting */
bool
wlc_rx_amsdu_in_ampdu_override(amsdu_info_t *ami)
{
	ASSERT(ami);

	return ami->rx_amsdu_in_ampdu_ovr;
}
#ifdef WLCFP
/**
	...
	struct scb *scb)
{
	...
	aggtype = (D11RXHDR_GE128_ACCESS_VAL(rxh, mrxs) & RXSS_AGGTYPE_MASK) >>
		RXSS_AGGTYPE_SHIFT;
#else /* ! DONGLEBUILD */
	aggtype = RXHDR_GET_AGG_TYPE(rxh, wlc);
#endif /* ! DONGLEBUILD */
	...
	fifo = (D11RXHDR_GE128_ACCESS_VAL(rxh, fifo));
	ASSERT((fifo < RX_FIFO_NUMBER));
	deagg = &ami->amsdu_deagg[fifo];
 ...
 */
void
wlc_cfp_amsdu_tx_counter_upd(wlc_info_t *wlc, uint32 nmsdu, uint32 totlen)
{
	amsdu_info_t *ami = wlc->ami;
 ...
 * If cant fit in, should make sure its enqueud back to prec queues.
 */
uint8 BCMFASTPATH
...
wlc_amsdu_subframe_insert(wlc_info_t* wlc, void* head)
{
	...
	while (pkt) {
		...
		PKTSETFRAGDATA_LO(wlc->osh, pkt, 1,
			PKTFRAGDATA_LO(wlc->osh, pkt, 1) - ETHER_HDR_LEN);
		PKTSETFRAGLEN(wlc->osh, pkt, 1,
			PKTFRAGLEN(wlc->osh, pkt, 1) + ETHER_HDR_LEN);
