 ...
 *
 * $Id: wlc_fbt.c 778270 2019-08-28 12:59:49Z $
 */
...
#ifdef WLRSDB
#define FBT_COPY_SIZE	(sizeof(wlc_fbt_copy_t))
typedef struct {
	bss_fbt_priv_t *fbt_bss_priv;	/* fbt private info */
} wlc_fbt_copy_t;

static int
wlc_fbt_bss_get(void *ctx, wlc_bsscfg_t *cfg, uint8 *data, int *len)
{
	wlc_fbt_copy_t *cp = (wlc_fbt_copy_t *)data;
	...
	if (!data || *len < sizeof(*cp)) {
		WL_ERROR(("wl%d:%d %s() buffer passed to copy data is short\n",
				cfg->wlc->pub->unit, cfg->_idx, __FUNCTION__));
		*len = sizeof(*cp);
		return BCME_BUFTOOSHORT;
	}

	if (!FBT_BSSCFG_CUBBY(cfg->wlc->fbt, cfg) ||
			!(fbt_bss_priv = FBT_BSSCFG_CUBBY_PRIV(cfg->wlc->fbt, cfg))) {
		WL_INFORM(("wl%d:%d %s() FBT BSSCFG private cubby not found\n",
				cfg->wlc->pub->unit, cfg->_idx, __FUNCTION__));
		*len = 0;
		return BCME_OK;
	}
	...
	*len = sizeof(*cp);
	cp->fbt_bss_priv = fbt_bss_priv;
	return BCME_OK;
}

static int
wlc_fbt_bss_set(void *ctx, wlc_bsscfg_t *cfg, const uint8 *data, int len)
{
	const wlc_fbt_copy_t *cp = (const wlc_fbt_copy_t *)data;
	bss_fbt_priv_t *fbt_bss_priv;

	if (!data || (len && (len < sizeof(*cp)))) {
		WL_ERROR(("%s: data(%p) len(%d/%lu)\n",
			__FUNCTION__, OSL_OBFUSCATE_BUF(data), len, (unsigned long)sizeof(*cp)));
		return BCME_ERROR;
	}

	fbt_bss_priv = wlc_fbt_get_bsscfg(cfg->wlc->fbt, cfg);
	ASSERT(fbt_bss_priv);
	fbt_bss_priv->use_sup_wpa = len ? cp->fbt_bss_priv->use_sup_wpa : FALSE;
	wlc_fbt_bsscfg_init(cfg->wlc->fbt, cfg);

	if (!len) {
		return BCME_OK;
	}

	if (cp->fbt_bss_priv->wpa && fbt_bss_priv->wpa) {
		...
		memcpy(wpa, cp->fbt_bss_priv->wpa, sizeof(*cp->fbt_bss_priv->wpa));
		cp->fbt_bss_priv->wpa->auth_wpaie_len = 0;
		cp->fbt_bss_priv->wpa->sup_wpaie_len = 0;
		cp->fbt_bss_priv->wpa->auth_wpaie = NULL;
		cp->fbt_bss_priv->wpa->sup_wpaie = NULL;
	}

	if (cp->fbt_bss_priv->wpa_info && fbt_bss_priv->wpa_info) {
		memcpy(SUP_CUBBY_CONFIG_DATA(fbt_bss_priv->wpa_info),
				SUP_CUBBY_CONFIG_DATA(cp->fbt_bss_priv->wpa_info),
				SUP_CUBBY_CONFIG_SIZE);
		fbt_bss_priv->wpa_info->passhash_states =
			cp->fbt_bss_priv->wpa_info->passhash_states;
	}

	memcpy(&fbt_bss_priv->peer_ea, &cp->fbt_bss_priv->peer_ea,
			sizeof(*cp->fbt_bss_priv) - OFFSETOF(bss_fbt_priv_t, peer_ea));
	memset(&cp->fbt_bss_priv->mdie_len, 0,
			OFFSETOF(bss_fbt_priv_t, pmkr0name) - OFFSETOF(bss_fbt_priv_t, mdie_len));
	cp->fbt_bss_priv->fbt_bss_overrides = NULL;

	WL_INFORM(("wl%d:%d %s() ftie_len: %d mdie_len: %d akm_type: %d mdid: %d \n",
			cfg->wlc->pub->unit, cfg->_idx, __func__, fbt_bss_priv->ftie_len,
			fbt_bss_priv->mdie_len, fbt_bss_priv->current_akm_type,
			fbt_bss_priv->mdid));
	return BCME_OK;
}
#endif /* WLRSDB */
...
#endif // endif
#ifdef WLRSDB
	cubby_params.fn_get = wlc_fbt_bss_get;
	cubby_params.fn_set = wlc_fbt_bss_set;
	cubby_params.config_size = FBT_COPY_SIZE;
#endif /* WLRSDB */
...
wlc_fbt_calc_md_ie_len(void *ctx, wlc_iem_calc_data_t *data)
{
	...
	if ((cfg->WPA_auth & WPA2_AUTH_FT) != 0) {
		switch (data->ft) {
		case FC_AUTH:
		case FC_ACTION:
			if (cfg->auth_atmptd == DOT11_FAST_BSS) {
...
wlc_fbt_overds_attempt(wlc_bsscfg_t *cfg)
{
	...
	if (WLFBT_ENAB(wlc->pub) && cfg->BSS && (bi->bss_type == DOT11_BSSTYPE_INFRASTRUCTURE) &&
			(cfg->auth == DOT11_OPEN_SYSTEM) && (cfg->assoc->type == AS_ROAM) &&
		...
		if ((bi->flags2 & WLC_BSS_OVERDS_FBT) && cfg->associated &&
...
wlc_fbt_enabled(wlc_fbt_info_t *fbt_info, wlc_bsscfg_t *cfg)
{
	...
	/* TODO: need per BSS enable for STA */
	...
	struct scb *scb, wlc_fbt_auth_resp_t *fbt_auth_resp)
{
	...
	wlc_ap_sendauth(wlc->ap, bsscfg, scb, DOT11_FAST_BSS, 2,
		fbt_auth_resp->status, NULL, DOT11_CAP_SHORT);
	return BCME_OK;
deauth:
	wlc_scbfree(wlc, scb);
	return BCME_ERROR;
