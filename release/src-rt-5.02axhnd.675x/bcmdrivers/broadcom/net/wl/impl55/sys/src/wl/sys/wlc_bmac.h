 ...
 *
 * $Id: wlc_bmac.h 778660 2019-09-06 12:21:21Z $
 */
...
/**
 * Iterator for all bands: FOREACH_WLC_HW_BAND(wlc_hw_info_t *wlc_hw, uint bandunit). Skips over
 * unused entries in the wlc->bandstate[] array (in case of a single band device). Avoids code
 * duplication.
 */
#define FOREACH_WLC_HW_BAND(wlc_hw, bandunit) \
	for ((bandunit) = 0; (bandunit) < MAXBANDS; (bandunit)++) \
		if (wlc_get_band((wlc_hw)->wlc, (bandunit)) != NULL)
