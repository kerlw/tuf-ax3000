 ...
 *
 * $Id: wlc_vasip.c 778417 2019-08-30 09:07:11Z $
 */
...
wlc_dump_vasip_counters(wlc_info_t *wlc, struct bcmstrbuf *b)
{
	...
	bcm_bprintf(b,
			...
			VASIP_SHARED_OFFSET(wlc->hw, grp_sel_buf),
	...
	bcm_bprintf(b,  "\n      precoder M2V/V2M address:    0x%x / 0x%x\n",
			VASIP_SHARED_OFFSET(wlc->hw, precoder_buf),
	...
	if (IS_VASIP_OFDMA_SCH(wlc->hw->corerev)) {
		bcm_bprintf(b,
			...
			VASIP_SHARED_OFFSET(wlc->hw, cqi_rpt_buf),
			VASIP_SHARED_OFFSET(wlc->hw, v2m_buf_cqi),
			VASIP_SHARED_OFFSET(wlc->hw, ru_alloc_buf),
			VASIP_SHARED_OFFSET(wlc->hw, v2m_ru_alloc_buf));
	}
	bcm_bprintf(b,
