 ...
 *
 * $Id: wlc_rate_sel.c 778242 2019-08-28 04:28:20Z $
 */
...
wlc_ratesel_init(ratesel_info_t *rsi, ratesel_init_t *init)
{
	...
	state->aid = state->scb->aid & DOT11_AID_MASK;
	...
	state->ulrt.txrspec[0] = DUMMY_UINT16;
...
#ifdef WLC_DTPC
	if (DTPC_ENAB(state->rsi->pub) && DTPC_SUPPORTED(state->rsi->wlc->dtpc)) {
	...
	bool is_ampdu, uint8 ampdu_txretry)
{
	...
	RL_SP0(("%s: frameid 0x%x ampdu_txretry %d mcs_sp_flgs 0x%x extsp_flgs 0x%x\n",
		__FUNCTION__, frameid, ampdu_txretry, state->mcs_sp_flgs, state->extsp_flgs));
	...
	uint fbr, uint fbr_succ, bool tx_error, uint8 tx_mcs, bool sgi, uint8 antselid)
{
	...
	WL_DPRINT((state->rsi->wlc->macdbg, state->scb,
		"fid 0x%04x gotpkts %d [mrt succ fbr succ nf e] = [%d %d %d %d %d %d] nupd %d\n",
		frameid, state->gotpkts, mrt, mrt_succ, fbr, fbr_succ,
		state->ncfails, state->epoch, state->nupd));
	...
	ratesel_txs_t *rs_txs, tx_status_t *txs, uint16 flags)
{
	...
	if (!txs_mu) {
		...
		if (txs_mutype == TX_STATUS_MUTP_VHTMU) {
			...
			rcb_itxs->txepoch = (uint16) RCB_TXEPOCH_INVALID;
			RL_INFO(("%s: frameid %x mrt/succ/fbr/succ %d %d %d %d\n",
				__FUNCTION__, frameid, mrt, mrt_succ, fbr, fbr_succ));
		} else {
			return;
		}
#endif /* defined(WL_MU_TX) */
...
#ifdef WLC_DTPC
	if (DTPC_ENAB(state->rsi->pub) && DTPC_SUPPORTED(wlc->dtpc)) {
	...
	uint16 *flags)
{
	...
	*flags = 0;
	...
	if (SP_MODE(state) == SP_EXT)
		RL_MORE(("%s: e %d rspec_pri/fbr 0x%08x 0x%08x rate_pri/fbr %u %u ant_pri/fbr %d %d"
			" sgi %d fid 0x%04x\n", __FUNCTION__, state->epoch,
		...
		RL_MORE(("%s: e %d rspec_pri/fbr 0x%08x 0x%08x rate_pri/fbr %u %u "
			"sgi %d fid 0x%04x\n",
			__FUNCTION__, state->epoch,
			ratesel_rates->rspec[0], ratesel_rates->rspec[1],
			RSPEC2RATE500K(ratesel_rates->rspec[0]),
	...
	if (RATELINKMEM_ENAB(rsi->pub)) {
		...
		RL_MORE(("%s scb %p epoch %d upd_pend %d\n",
			__FUNCTION__, state->scb, state->epoch, state->upd_pend));
	}
