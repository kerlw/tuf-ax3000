 ...
 *
 * $Id: wlc_btcx.c 778625 2019-09-06 01:56:11Z $
 */
...
const bcm_iovar_t btc_iovars[] = {
	{"btc_mode", IOV_BTC_MODE, (IOVF_RSDB_SET), 0, IOVT_UINT32, 0},
	{"btc_stuck_war", IOV_BTC_STUCK_WAR, 0, 0, IOVT_BOOL, 0 },
	{"btc_flags", IOV_BTC_FLAGS,
	(IOVF_SET_UP | IOVF_GET_UP | IOVF_RSDB_SET), 0, IOVT_BUFFER, 0
	},
	{"btc_params", IOV_BTC_PARAMS, (IOVF_RSDB_SET), 0, IOVT_BUFFER, 0 },
	...
	{"btc_profile_active", IOV_BTC_PROFILE_ACTIVE, (IOVF_RSDB_SET), 0, IOVT_INT32, 0
	},
	{"btc_nss", IOV_BTC_NSS, 0, 0, IOVT_UINT8, 0},
#endif /* WL_UCM */
	{"btc_dyagg", IOV_BTC_DYAGG, (IOVF_RSDB_SET), 0, IOVT_INT8, 0
	},
	{"btc_btrssi_avg", IOV_BTC_BTRSSI_AVG, (IOVF_RSDB_SET), 0, IOVT_INT8, 0
	},
	{"btc_ble_grants", IOV_BTC_BLE_GRANTS, (IOVF_RSDB_SET), 0, IOVT_BOOL, 0
	},
...
struct wlc_btc_info {
	...
	wlc_btc_stats_t *stats;		/* btcx antenna stats for ecounter feature */
...
static int btcx_ecounters_get_stats(uint16 type, void *context,
...
static void wlc_btcx_update_stats(wlc_info_t *wlc);
static int wlc_clr_btcxstats(wlc_info_t *wlc);
#endif /* ECOUNTERS */
 ...
 */
	...
	/* ECOUNTERS only for AUX core */
	if (ECOUNTERS_ENAB() &&
		((RSDB_ENAB(wlc->pub) && wlc->pub->unit == MAC_CORE_UNIT_1) ||
		!RSDB_ENAB(wlc->pub))) {
		if ((btc->stats = (wlc_btc_stats_t *)
			MALLOCZ(wlc->osh, sizeof(*(btc->stats)))) == NULL) {
			WL_ERROR(("wl%d: %s: out of mem, malloced %d bytes\n",
			wlc->pub->unit, __FUNCTION__, MALLOCED(wlc->osh)));
			goto fail;
		}
		bzero(btc->stats, sizeof(wlc_btc_stats_t));

		wl_ecounters_register_source(
			WL_IFSTATS_XTLV_WL_SLICE_BTCOEX, btcx_ecounters_get_stats, (void*)wlc);
	} /* ECOUNTERS_ENAB */
...
#if defined(ECOUNTERS) && defined(EVENT_LOG_COMPILE)
	if (ECOUNTERS_ENAB() && wlc->pub->unit == MAC_CORE_UNIT_1 &&
		btc->stats) {
		MFREE(wlc->osh, btc->stats, sizeof(*(btc->stats)));
	}
#endif /* ECOUNTERS && EVENT_LOG_COMPILE */
...
wlc_btc_wlc_up(void *ctx)
{
	...
	if ((btc->btc_bt_gpio && btc->btc_stat_gpio && btc->btc_wlan_gpio)) {
		uint32 mask = 0;
		uint32 bitcount = 0;
		...
		if (btc->coex_mode == COEX_MODE_GCI) {
			...
			mask =	(0x02 << ((btc->btc_wlan_gpio - 1) * 8)) |
				(0x01 << ((btc->btc_bt_gpio - 1) * 8)) |
				(0x01 << ((btc->btc_stat_gpio - 1) * 8));
			si_gci_indirect(sih, 0,
			 ...
			 * GlobalCoexInterfaceBitDefinition#From_BT
			 */
			mask = ((btc->btc_stat_gpio - 1) << 16) | 0x4;
			si_gci_indirect(sih, mask,
...
wlc_btc_apply_profile(struct wlc_info *wlc, wlc_bsscfg_t *bsscfg)
{
	...
	new_band = CHSPEC_IS2G(chanspec) ? WLC_BAND_2G : WLC_BAND_5G;
	...
	if (wlc->clk) {
		wlc_btc_set_ps_protection(wlc, wlc->cfg);
	}
...
wlc_btcx_update_coex_iomask(wlc_info_t *wlc)
{
	...
	if (!RSDB_ENAB(wlc->pub) ||
		...
		if (si_coreunit(wlc_hw->sih)) {
			...
			OR_REG(wlc_hw->osh, D11_MACCONTROL(wlc_hw), MCTL_IHR_EN);
		}
		/* read present core iomask */
		coex_io_mask = R_REG(wlc_hw->osh, D11_COEX_IO_MASK(wlc_hw));
		...
		if (coex_io_mask_ch != coex_io_mask) {
			W_REG(wlc_hw->osh, D11_COEX_IO_MASK(wlc_hw), coex_io_mask_ch);
		}
	}
#endif /* BCMECICOEX */
}
...
wlc_btcx_upd_gci_chanspec_indications(wlc_info_t *wlc)
{
	...
	FOREACH_WLC(wlc->cmn, idx, wlc_iter) {
		if (wlc_iter->pub->up) {
			if (CHSPEC_IS2G(wlc_iter->hw->chanspec)) {
				gci_val |=  GCI_WL_2G_UP_INFO;
			} else if (CHSPEC_IS5G(wlc_iter->hw->chanspec)) {
				gci_val |=  GCI_WL_5G_UP_INFO;
			}
		}
		/* Intentionally left out a break logic as MAX_WLCs are 2
		 * and a break handling would require bulky redundant logic
		 */
	}
...
wlc_btcx_write_core_specific_mask(wlc_info_t *wlc)
{
	...
	} else if (BCM4347_CHIP(wlc->pub->sih->chip) ||	BCM4369_CHIP(wlc->pub->sih->chip)) {
		if (wlc->hw->macunit == 0) {
			/* Mask core 1 */
...
#ifdef WL_UCM
	if (UCM_ENAB(wlc->pub)) {
		wlc_btc_get_matched_chanspec_bsscfg(wlc, WLC_BAND_PI_RADIO_CHANSPEC, &ucm_bsscfg);
...
#endif /* WL_STF_ARBITRATOR */
	{
		if (BAND_2G(wlc->band->bandtype) && btc->chain_2g_shared) {
			...
			if (wlc->stf->txchain_pending == 0) {
				...
				wlc_bss_mac_event(wlc, wlc->cfg, WLC_E_DMA_TXFLUSH_COMPLETE,
					NULL, 0, 0, 0, NULL, 0);
			} else {
...
void wlc_btc_rxchain_update(wlc_info_t *wlc)
{
	...
	wlc_bss_mac_event(wlc, wlc->cfg, WLC_E_DMA_TXFLUSH_COMPLETE, NULL, 0, 0,
		0, NULL, 0);
}
static int wlc_btc_2g_chain_set(wlc_info_t *wlc, bool bool_val)
{
...
#ifdef ECOUNTERS
static int wlc_btcx_get_stats(wlc_info_t *wlc)
{
	uint8 idx, offset;
	uint16 hi, lo;
	uint32 buff[C_BTCX_DBGBLK_SZ/2];
	uint16 buff2[C_BTCX_DBGBLK2_SZ];
	uint16 base = D11REV_LT(wlc->pub->corerev, 40) ? M_BTCXDBG_BLK(wlc):
		((wlc_bmac_read_shm(wlc->hw, M_PSM2HOST_EXT_PTR(wlc)) + 0xd) << 1);
	uint16 bt_req_type_lo, bt_req_type_hi, btcx_blk_ptr =  wlc->hw->btc->bt_shm_addr;
	int err = BCME_OK;
	wlc_btc_info_t *btc = wlc->btch;
	wlc_hw_info_t *wlc_hw = wlc->hw;

	ASSERT(btc->stats != NULL);
	if (!wlc->clk) {
		err = BCME_NOCLK;
		goto fail;
	}
	for (idx = 0; idx < C_BTCX_DBGBLK_SZ; idx += 2) {
		offset = idx*2;
		lo = wlc_bmac_read_shm(wlc_hw, base+offset);
		hi = wlc_bmac_read_shm(wlc_hw, base+offset+2);
		buff[idx>>1] = (hi<<16) | lo;
	}
	btc->stats->valid = 1;
	btc->stats->stats_update_timestamp = OSL_SYSUPTIME();
	btc->stats->btc_status =  wlc_btcx_get_btc_status(wlc);
	btc->stats->bt_req_cnt = buff[0];
	btc->stats->bt_gnt_cnt = buff[1];
	btc->stats->bt_gnt_dur = buff[2];

	if (btcx_blk_ptr) {
		bt_req_type_lo = wlc_bmac_read_shm(wlc->hw, M_BTCX_BT_TASKS_BM_LOW(wlc));
		bt_req_type_hi = wlc_bmac_read_shm(wlc->hw, M_BTCX_BT_TASKS_BM_HI(wlc));
		btc->stats->bt_req_type_map = (bt_req_type_hi << 16) | bt_req_type_lo;
		btc->stats->bt_abort_cnt = wlc_bmac_read_shm(wlc->hw,
			M_BTCX_ABORT_CNT(wlc));
		btc->stats->bt_latency_cnt = wlc_bmac_read_shm(wlc->hw,
			M_BTCX_LATENCY_CNT(wlc));
		btc->stats->bt_rxf1ovfl_cnt = wlc_bmac_read_shm(wlc->hw, M_PSM2HOST_STATS(wlc) +
			MCSTOFF_RXF0OVFL);
		for (idx = 0; idx < C_BTCX_DBGBLK2_SZ; idx++) {
			buff2[idx] = wlc_bmac_read_shm(wlc_hw, M_BTCX_SUCC_PM_PROTECT_CNT(wlc) +
				(idx << 1));
		}
		memcpy(&(btc->stats->bt_succ_pm_protect_cnt), &buff2, sizeof(buff2));
		btc->stats->slice_index = RSDB_ENAB(wlc->pub) ? ((uint8)wlc->pub->unit) : 0;
	}
	return BCME_OK;
fail:
	return err;
...
static int wlc_clr_btcxstats(wlc_info_t *wlc)
{
	...
	if (btcx_blk_ptr) {
		wlc_bmac_write_shm(wlc->hw, M_BTCX_BT_TASKS_BM_LOW(wlc), 0x0);
		wlc_bmac_write_shm(wlc->hw, M_BTCX_BT_TASKS_BM_HI(wlc), 0x0);
		wlc_bmac_set_shm(wlc->hw, M_BTCX_SUCC_PM_PROTECT_CNT(wlc), 0,
			C_BTCX_DBGBLK2_SZ * 2);
	}
	return BCME_OK;
fail:
...
/*
 * Read BTCX statistics from shmem, and then write 0's for these delta stats.
 */
static void wlc_btcx_update_stats(wlc_info_t *wlc)
{
	int err = 0;
	...
	err = wlc_btcx_get_stats(wlc);
	if (err) goto fail;
#if defined(BCMDBG)
	err = wlc_clr_btcxdump(wlc);    /* Clear statistics after read */
	if (err) goto fail;
#endif // endif
	err = wlc_clr_btcxstats(wlc);   /* Clear statistics after read */
	if (err) goto fail;

	return;
fail:
	WL_ERROR(("Core: %d, wlc_btcx_update_fail\n", wlc->pub->unit));
	return;
}

#if defined(EVENT_LOG_COMPILE)
/*
 * This routine will set a flag so that wlc_watchdog() when radio should be awake.
 * wlc_watchdog() will then update btcx stats.
 */
static int
btcx_ecounters_get_stats(uint16 type, void *context,
	const ecounters_stats_types_report_req_t * req, struct bcm_xtlvbuf *xtlvbuf,
	uint32 *cookie, const bcm_xtlv_t* tlv, uint16 *attempted_write_len)
{
	...
	if (type != WL_IFSTATS_XTLV_WL_SLICE_BTCOEX) {
		return BCME_ERROR;
	}

	/* Get BT Coex ecounters if core is Up */
	if (!wlc->pub->up || !wlc->clk) {
		return BCME_NOTUP;
	}

	/* Update version field so it will be correct even if stats can't be
	 * collected if d11core is asleep.
	 */
	wlc->btch->stats->version = BTCX_STATS_VER;
	wlc_btcx_update_stats(wlc);

	ret = bcm_xtlv_put_data(xtlvbuf, type, (uint8 *)wlc->btch->stats,
		sizeof(*(wlc->btch->stats)));

	/* If there is enough size. */
	if (ret == BCME_NOMEM) {
		*attempted_write_len = BCM_XTLV_HDR_SIZE + sizeof(*(wlc->btch->stats));

		return BCME_BUFTOOSHORT;
	}

	/* Zero structure so that stale data isn't printed twice if mac is asleep
	 * during the next statistics retrieval.
	 */
	bzero(wlc->btch->stats, sizeof(wlc_btc_stats_t));
