 ...
 *
 * $Id: wlc_vht.c 778648 2019-09-06 09:44:56Z $
 */
...
static const bcm_iovar_t vht_iovars[] = {
	{"vhtmode", IOV_VHTMODE,
	(IOVF_SET_DOWN|IOVF_OPEN_ALLOW|IOVF_RSDB_DEFAULT), 0, IOVT_UINT32, 0
	},
...
wlc_write_vht_cap_ie(wlc_bsscfg_t *cfg, vht_cap_ie_t *cap_ie)
{
	...
	uint32 cap;
	const uint8 bw_cap = (wlc->bandstate[BAND_5G_INDEX]->bw_cap);
...
wlc_vht_upd_rate_mcsmap(wlc_vht_info_t *vhti, struct scb *scb)
{
	...
	if (BSSCFG_AP(cfg)) {
		rxmcsmap = wlc_rateset_filter_mcsmap(rxmcsmap,
...
wlc_vht_up(void *ctx)
{
	...
	if (D11REV_GE(wlc->pub->corerev, 128)) {
		...
		wlc_bmac_write_shm(wlc->hw, M_MAXRXMPDU_LEN(wlc), max_mpdu);
	}
	return BCME_OK;
}
