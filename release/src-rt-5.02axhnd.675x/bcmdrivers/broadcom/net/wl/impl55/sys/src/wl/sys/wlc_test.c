 ...
 *
 * $Id: wlc_test.c 778719 2019-09-09 11:09:33Z $
 */
...
#define MAX_RU_TYPE			6
#define MAX_HE_LTFSYMBSZ_IDX		4
#define MAX_DATA_SYMBSZ_IDX		4
...
static int wlc_bmac_pkteng_trig_fillinfo(wlc_hw_info_t *wlc_hw,
		wl_pkteng_ru_fill_t *pkteng_ru_fill);
static int wlc_bmac_get_trig_info(wlc_hw_info_t *wlc_hw, wl_trig_frame_info_t *trig_frame_info);
	...
	void *params, uint plen, void *arg, uint alen, uint vsize, struct wlc_if *wlcif)
{
	...
	case IOV_SVAL(IOV_PKTENG): {
		...
		if ((pkteng_flags == WL_PKTENG_PER_TX_START) ||
			...
			(pkteng_flags == WL_PKTENG_PER_RU_TX_START) ||
	...
	case IOV_SVAL(IOV_LOAD_TRIG_INFO): {
		...
		err = wlc_bmac_pkteng_trig_fillinfo(wlc->hw, &pkteng_ru_fill);
		if (err != BCME_OK)
			WL_ERROR(("problem with some parameters while filling trig info fields\n"));
		break;
	}
	case IOV_GVAL(IOV_GET_TRIG_INFO): {
		wl_trig_frame_info_t trig_frame_info;
		err = wlc_bmac_get_trig_info(wlc->hw, &trig_frame_info);
	...
	void *userdata, int userdata_len)
{
	...
	while (len > 0) {
		...
		if (first) {
			buflen = DOT11_MGMT_HDR_LEN + TXOFF;
		}
...
wlc_bmac_pkteng_trig_fillinfo(wlc_hw_info_t *wlc_hw, wl_pkteng_ru_fill_t *pkteng_trig_fill)
{
	ru_type_t ru_type;
	uint8 idx, err = BCME_OK;
	uint8 m_stbc, ldpc_extra_symb, pe_disambg;
	uint8 t_pe, a_calc, pe_time_req, a_init;
	uint16 n_excess, n_sym_calc = 0, a_factor, a_fac_int;
	uint16 lsig_len, l_ldpc, n_cw, n_shrt, n_punc;
	uint16 n_cbps_short, n_dbps_short, he_ltf_symb_sz, symb_sz;
	uint16 tx_time, n_sym_init = 0, pe_disamb_chk;
	uint32 n_dbps, num_bytes, n_pld, n_av_bits, tmp4 = 0, n_cbps_last_init;
	...
	uint8 doppler = 0, he_siga_rsvd = 0, rsvd = 0, target_rssi = 0, strm_offset = 0;
	/* common info structure */
	he_trig_cmninfo_set_t s_temp, *cmn_f;
	/* user info structure */
	he_trig_usrinfo_set_t z_temp, *usr_f;
	/* pointer to parse the required vaules to shms */
	uint16 *smem;
	/* data bits per symbol , row :: RU_index and coloumn :: mcs_val */
	uint16 n_dbps_tabl[MAX_RU_TYPE][MCS_MAX_IDX] = {{12, 24, 36, 48, 72, 96, 108, 120,
		144, 160, 0, 0},
		{24, 48, 72, 96, 144, 192, 216, 240, 288, 320, 0, 0},
		{51, 102, 153, 204, 306, 408, 459, 510, 612, 680, 0, 0},
	...
	uint8 n_sd_short[MAX_RU_TYPE] = {6, 12, 24, 60, 120, 240};
	/* coding rates for different mcs index values */
	/* express in Q8 format ( scale_factor =  2^ 8) */
	uint16 code_rate[MCS_MAX_IDX] = {128, 128, 192, 128, 192, 170, 192,
		213, 192, 213, 192, 213};
	/* bits per sub-carrier per spatial stream */
	uint8 n_bpscs[MCS_MAX_IDX] = {1, 2, 2, 4, 4, 6, 6, 6, 8, 8, 10, 10};
	/* he_ltf symbol sizes */
	/* Q8 format */
	uint16 he_ltf_symbsize[MAX_HE_LTFSYMBSZ_IDX] = {1229, 1843, 2048, 4096};
	/* data symbol sizes */
	/* Q8 format */
	uint16 data_symbsize[MAX_DATA_SYMBSZ_IDX] = {3686, 3482, 3686, 4096};
	/* pe time array */
	...
	uint8 ruidx_alloc_tmp = (pkteng_trig_fill->ru_alloc_val);
	...
	if (ruidx_alloc_tmp > 128) {
		ru160 = 1;
		ruidx_alloc_tmp -= 128;
	}
	if (ruidx_alloc_tmp > HE_MAX_996_TONE_RU_INDX) {
		ru_type = D11AX_RU996_TYPE;
	} else {
		ru_type = wf_he_ruidx_to_ru_type(ruidx_alloc_tmp);
	}

	n_cbps_short = (n_bpscs[mcs_temp] * nss_temp * n_sd_short[ru_type]);
	...
	n_dbps = ((n_dbps_tabl[ru_type][mcs_temp])* nss_temp);
	n_cbps = ROUNDUP((LEFT_SHIFT(n_dbps, SCALE_FACT)/code_rate_R), 1);
	...
	if (coding_tmp == 0) {
		...
		tmp4 = (LEFT_SHIFT(1, SCALE_FACT) - code_rate_R);
		tmp4 = ROUNDUP(RIGHT_SHIFT(tmp4, SCALE_FACT), 1);
		if (n_av_bits > 2592) {
			l_ldpc = 1944;
			n_cw = CEIL(n_pld, ROUNDUP(RIGHT_SHIFT(1944 * code_rate_R, SCALE_FACT), 1));
		} else if (n_av_bits <= 2592 && n_av_bits > 1944) {
			l_ldpc = 1296;
			n_cw = 2;
			if (n_av_bits >= (n_pld + (2916 * tmp4))) {
				l_ldpc = 1944;
			}
			...
			l_ldpc = 1296;
			n_cw = 1;
			if (n_av_bits >= (n_pld + (1464 * tmp4))) {
				l_ldpc = 1944;
			}
		} else {
			l_ldpc = 648;
			n_cw = 1;
			if (n_av_bits >= (n_pld + (912 * tmp4))) {
				l_ldpc = 1296;
			}
		}
		tmp4 = RIGHT_SHIFT(n_cw * l_ldpc * code_rate_R, SCALE_FACT);
		n_shrt = MAX(0, ((int32)(ROUNDUP(tmp4, 1) - n_pld)));
		n_punc = MAX(0, ((int32)((n_cw * l_ldpc) - n_av_bits - n_shrt)));
		tmp4 = ((n_cw * l_ldpc) * (LEFT_SHIFT(1, SCALE_FACT) - code_rate_R));
		tmp1 = ROUNDUP((RIGHT_SHIFT(tmp4, SCALE_FACT)/10), 1);
		tmp2 = ROUNDUP(((12 * n_punc * (code_rate_R/(LEFT_SHIFT(1,
			SCALE_FACT) - code_rate_R)))/10), 1);
		tmp3 = ROUNDUP((RIGHT_SHIFT(3 * tmp4, SCALE_FACT)/10), 1);
		/* calculations for LDPC extra symbol */
		if (((n_punc > tmp1) && (n_shrt < tmp2)) || (n_punc > tmp3)) {
			ldpc_extra_symb = 1;
			if (a_init == 4) {
	...
	// calculate num of final symbols/txtime/L-sig length
	a_factor = (a_calc % 4);
	a_fac_int = a_calc;
	...
	tx_time = HE_T_LEG_STF + HE_T_LEG_LTF + HE_T_LEG_LSIG +
		...
		ROUNDUP(RIGHT_SHIFT((he_ltf_symb_temp * he_ltf_symb_sz) +
		(n_sym_calc * symb_sz), SCALE_FACT), 1) + t_pe;
	lsig_len = (CEIL((tx_time - HE_T_LEG_PREAMBLE),
		HE_T_LEG_SYMB)* HE_N_LEG_SYM) -3 - 2;
	/* disambiguity bit check( adjust the scaling also) */
	pe_disamb_chk = (LEFT_SHIFT(1, SCALE_FACT)*(t_pe +
		(4 * (CEIL((tx_time - HE_T_LEG_PREAMBLE),
		HE_T_LEG_SYMB) - ((tx_time - HE_T_LEG_PREAMBLE)/HE_T_LEG_SYMB)))));
	...
	setbits((uint8 *)usr_f, sizeof(*usr_f), HE_TRIG_USRINFO_RU_ALLOC_INDX,
		HE_TRIG_USRINFO_RU_ALLOC_FSZ, ((ruidx_alloc_tmp << 1) + ru160));
	/* coding type */
	...
	/* write the values to shmem block */
	smem = (uint16 *)cmn_f;
	/* common info */
	for (idx = 0; idx < 4; idx++) {
		wlc_bmac_write_shm(wlc_hw, M_RXTRIG_CMNINFO_LOC(wlc_hw, idx), *smem++);
	}
	smem = (uint16 *)usr_f;
	/* user info */
	for (idx = 0; idx < 3; idx++) {
		wlc_bmac_write_shm(wlc_hw, M_RXTRIG_USRINFO_LOC(wlc_hw, idx), *smem++);
	}
	return err;
}

static int
wlc_bmac_get_trig_info(wlc_hw_info_t *wlc_hw, wl_trig_frame_info_t *trig_frame_info)
{
	uint16 val;
	uint8 vals[8];
	...
	char *outbuf, int outlen)
{
	...
	if (print_shm) {
		if (is_dl || WLC_TEST_PKTENGCMD_IS_DLSCH(info->flags)) {
#if defined(WL_PSMX)
		...
		if (!is_dl || WLC_TEST_PKTENGCMD_IS_ULSCH(info->flags)) {
	...
	if (WLC_TEST_PKTENGCMD_IS_ULSCH(info->flags)) {
		bcm_bprintf(&bstr, "\n  UL info nbyte_pad %d ul_bw %d ap_txpwr %d ul_pe %d "
			"ul_cp_ltftype %d ul_cs_req %d ul_lsig_len 0x%x\n",
			info->nbyte_pad, info->ul_bw, info->ap_txpwr, info->ul_pe,
			info->ul_cp_ltftype, info->ul_cs_req, info->ul_lsig_len);
	}

	if (is_verbose) {
		bcm_bprintf(&bstr, "\tmode 0x%x flag 0x%x numheltf %d siga [%x %x %x] "
			"phyctl [%x %x %x %x %x] rspec %x shm_sts %x\n",
			info->mode, info->flags, info->numheltf,
		...
		bcm_bprintf(&bstr, "\tUL info nbyte_pad %d ul_bw %d ap_txpwr %d ul_pe %d "
			"ul_cp_ltftype %d ul_cs_req %d ul_lsig_len 0x%x\n",
			info->nbyte_pad, info->ul_bw, info->ap_txpwr, info->ul_pe,
			info->ul_cp_ltftype, info->ul_cs_req, info->ul_lsig_len);
	}
...
wlc_test_pkteng_cmd_set(wlc_test_info_t *test, wl_pkteng_cmd_params_t *params)
{
	...
	case PKTENGCMD_OPCODE_PUB_ARGINT_COMMIT:
		...
		if (info->mode == WL_PKTENG_CMD_STRT_DL_CMD) {
			...
			if ((err = wlc_test_pkteng_prepare_for_trigger_frame(test)) != BCME_OK) {
				break;
			}
		}  else if (info->mode == WL_PKTENG_CMD_STRT_ULTX_CMD) {
			/* PlaceHolder for MFGTEST SHMWRITE */
		} else {
			err = BCME_ERROR;
			break;
		}
...
static uint32
wlc_get_valid_lsiglen(uint32 lsiglen_in, uint16 ul_cp_ltftype, uint16 numheltf)
{
...
/* Function to prepare trigger frame and write it to shm */
static int
wlc_test_pkteng_prepare_for_trigger_frame(wlc_test_info_t *test)
{
	wlc_info_t *wlc;
	wlc_hw_info_t *wlc_hw;
	wlc_test_pkteng_info_t *info;
	...
	setbits((uint8 *)cmn_info, sizeof(*cmn_info), HE_TRIG_CMNINFO_CASCADEIND_INDX,
		HE_TRIG_CMNINFO_CASCADEIND_FSZ, !!info->ul_auto_ru);
	/* CS required */
	...
	setbits((uint8 *)cmn_info, sizeof(*cmn_info), HE_TRIG_CMNINFO_MUMIMO_LTF_INDX,
		HE_TRIG_CMNINFO_MUMIMO_LTF_FSZ, 0);
	/* HE-LTF symbols */
	...
	/* Write usr_info into shm M_TXTRIG_CMNINFO */
...
wlc_test_set_scheduler(wlc_test_info_t *test, uint16 sch_type)
{
	...
	if (sch_type & WLC_TEST_SCHTYPE_ULSCH) {
		...
		err = wlc_test_pkteng_prepare_for_trigger_frame(test);
	}
...
wlc_test_update_stalist(wlc_test_info_t *test)
{
	...
	FOREACH_BSS(wlc, idx, bsscfg) {
		FOREACH_BSS_SCB(wlc->scbstate, &scbiter, bsscfg, scb) {
			...
			usr = &info->usr[i];
			usr->aid = (scb->aid & DOT11_AID_MASK);
			usr->scb = scb;
			usr->dl_schpos = i;
			usr->dl_schid = 0;
			usr->ul_schpos = i;
	...
	wl_pkteng_cmd_params_t* params)
{
	...
	!strncmp(keystr, "en", strlen("en"))) {
		...
		/* for ul params */
		if (!strncmp(keystr, "ul_bw", strlen("ul_bw"))) {
			val16 = bcm_strtoul(valstr, NULL, 0);
			switch (val16) {
			case 20:
				info->ul_bw = D11_REV128_BW_20MHZ;
		...
		} else if (!strncmp(keystr, "ul_pe", strlen("ul_pe"))) {
			val16 = bcm_strtoul(valstr, NULL, 0);
			info->ul_pe = val16;
		...
		} else if (!strncmp(keystr, "ul_lsig_len", strlen("ul_lsig_len"))) {
			val16 = bcm_strtoul(valstr, NULL, 0);
