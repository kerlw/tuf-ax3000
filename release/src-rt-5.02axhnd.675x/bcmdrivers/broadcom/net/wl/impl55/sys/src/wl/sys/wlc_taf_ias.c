...
static const uint8 taf_tid_service_order[TAF_NUM_TIDSTATE] = {
	DPI_PRIO_VO, DPI_PRIO_VI, DPI_PRIO_BE4,
	DPI_PRIO_BE3, DPI_PRIO_BE2, DPI_PRIO_BE1,
	DPI_PRIO_BE, DPI_PRIO_BK};
#else /* HOSTDPI */
...
static uint16 taf_mu_mimo_rel_limit[TAF_MAX_MU] = {
	0,
	4000, /* 2 */
	4000, /* 3 */
	4000  /* 4 */
...
taf_ias_est_release_time(taf_scb_cubby_t *scb_taf, taf_ias_sched_data_t* ias_sched, int len)
{
	...
	pktbytes = TAF_PKTBYTES_TO_UNITS(est_pkt_size,
		scb_taf->info.scb_stats.ias.data.pkt_rate[TAF_RSPEC_SU_DL],
		scb_taf->info.scb_stats.ias.data.byte_rate[TAF_RSPEC_SU_DL]);
...
taf_update_item_stat(taf_method_info_t *method, taf_scb_cubby_t *scb_taf, int sched_tid)
{
	...
	if (sched_tid == ALLPRIO) {
		...
		scb_taf->info.traffic.available &= ~(1 << sched_tid);
	...
	do {
		...
		tid = (sched_tid == ALLPRIO) ? taf_tid_service_order[tid_index] : sched_tid;
...
#if TAF_ENABLE_MU_TX
		scb_taf->info.mu_type[tid] = TAF_TECH_INVALID;
#endif // endif
		...
		for (s_idx = TAF_FIRST_REAL_SOURCE; s_idx < TAF_NUM_SCHED_SOURCES; s_idx ++) {
			...
			if ((scb_taf->info.traffic.count[s_idx][tid] =
				...
				scb_taf->info.traffic.available |= (1 << tid);

				est_release = taf_ias_est_release_time(scb_taf,
					ias_sched, scb_taf->info.traffic.count[s_idx][tid]);

				est_release_total += est_release;

				WL_TAFM(method, MACF" tid %u qlen %s: %u, est %u (in flight %u)\n",
					TAF_ETHERC(scb_taf), tid,
					TAF_SOURCE_NAME(s_idx),
					scb_taf->info.traffic.count[s_idx][tid],
					est_release, SCB_PKTS_INFLT_FIFOCNT_VAL(scb_taf->scb, tid));
			}
		}
		if (scb_taf->info.traffic.available & (1 << tid)) {
#if TAF_ENABLE_MU_TX
			...
			for (tech = 0; tech < TAF_NUM_MU_TECH_TYPES; tech++) {
				...
				if (scb_taf->info.mu_tech[tech].enable & (1 << tid)) {
	...
	method->ias->sched_active |= scb_taf->info.traffic.available;
}
...
static INLINE void BCMFASTPATH taf_update_item(taf_method_info_t *method, taf_scb_cubby_t* scb_taf)
{
	...
	scb_taf->info.scb_stats.ias.data.relative_score =
		(scb_taf->score << TAF_COEFF_IAS_MAX_BITS) / method->ias->total_score;
...
static INLINE void BCMFASTPATH taf_ias_pre_update_list(taf_method_info_t *method, uint32 now_time)
{
	...
	for (item = method->list; item; item = item->next) {
		...
		if (SCB_MARKED_DEL(scb_taf->scb) || SCB_DEL_IN_PROGRESS(scb_taf->scb)) {
			scb_taf->force = 0;
			scb_taf->score = TAF_SCORE_MAX; /* max score means least priority for IAS */
	...
	taf_tech_type_t tech_idx)
{
	...
	if (scb_taf->info.mu_type[tid] < TAF_TECH_INVALID) {
		return FALSE;
	}
	if (scb_taf->info.traffic.est_units[tid] == 0) {
		return FALSE;
	}
	if (scb_taf->info.mu_tech[tech_idx].enable & (1 << tid)) {
		return TRUE;
	}
	...
	taf_release_context_t *context)
{
	...
	WL_TAFM(method, "this %s item is "MACF" having %u units tid %u and score %u, "
		...
		scb_taf->info.traffic.est_units[tid], tid, scb_taf->score,
		scb_taf->info.scb_stats.ias.data.relative_score);
	...
	while (list) {
		...
		for (list = list->next; list; list = list->next) {
			...
			if (taf_ias_dl_ofdma_suitable(scb_taf_next, tid, tech_idx)) {
				break;
			}
		...
		if (list && scb_taf_next) {
			...
			WL_TAFM(method, "next %s item is "MACF" having %u units tid %u and score "
				...
				scb_taf_next->info.traffic.est_units[tid], tid, scb_taf_next->score,
				scb_taf_next->info.scb_stats.ias.data.relative_score,
				est_inter_total);
			...
			WL_TAFM(method, "no next %s item found\n", taf_mutech_text[tech_idx]);
			break;
		}

		rel_diff = scb_taf_next->info.scb_stats.ias.data.relative_score -
			scb_taf->info.scb_stats.ias.data.relative_score;
	...
	for (index = 0; index < mu_user_count; index++) {
		...
		scb_taf_next->info.released_units_limit = time_limit_units;

		WL_TAFM(method, "set rel limit %u to "MACF" %s\n",
			scb_taf_next->info.released_units_limit, TAF_ETHERC(scb_taf_next),
			taf_mutech_text[tech_idx]);
	}
...
static INLINE bool BCMFASTPATH taf_ias_dl_mimo_suitable(taf_scb_cubby_t *scb_taf, uint8 bw, int tid)
{
	if (scb_taf->info.ps_mode) {
		return FALSE;
	}
	...
	taf_release_context_t *context)
{
	...
	streams = taf_nss(scb_taf);
	bw = taf_bw(scb_taf);
	items[mu_user_count] = scb_taf;

	mu_user_count++;
	orig_release = scb_taf->info.traffic.est_units[tid];
	max_release = orig_release;
	...
	while (list) {
		for (list = list->next; list; list = list->next) {
			...
			if (!taf_ias_dl_mimo_suitable(scb_taf_next, bw, tid)) {
				continue;
			}
			if (scb_taf_next->info.mu_tech[tech_idx].enable & (1 << tid)) {
				break;
			}
		}
		...
		if (list && scb_taf_next) {
			items[mu_user_count] = scb_taf_next;
			mu_user_count++;
			streams += taf_nss(scb_taf_next);

			max_release = MAX(max_release, scb_taf_next->info.traffic.est_units[tid]);
	...
	if (mu_user_count == 3 && streams > 4) {
		WL_TAFM(method, "3 total %s users (%u streams), but choosing just 2 users\n",
			taf_mutech_text[tech_idx], streams);

		mu_user_count = 2;
		streams -= taf_nss(items[2]);
	}

	user_rel_limit = TAF_MICROSEC_TO_UNITS(method->ias->mu_mimo_rel_limit[mu_user_count - 1]);

	if (user_rel_limit && max_release > user_rel_limit) {
		WL_TAFM(method, "mu-mimo release %u limiting to %uus (%u)\n", mu_user_count,
			TAF_UNITS_TO_MICROSEC(user_rel_limit), user_rel_limit);
	} else {
		user_rel_limit = max_release;
	}

	if (method->ias->release_limit > 0) {
		uint32 global_rel_limit = TAF_MICROSEC_TO_UNITS(method->ias->release_limit);
		user_rel_limit = MIN(user_rel_limit, global_rel_limit);
	}

	WL_TAFM(method, "final %s possible: %u users (%u streams), %u max traffic possible per "
		"user\n", taf_mutech_text[tech_idx], mu_user_count, streams, user_rel_limit);

	orig_release = MIN(orig_release, user_rel_limit);
	...
	for (index = 0; index < mu_user_count; index++) {
		...
		scb_taf_next->info.mu_type[tid] = tech_idx;
		scb_taf_next->info.released_units_limit = user_rel_limit;
		if (index > 0) {
			extend_units += orig_release;
		}

		WL_TAFM(method, "set rel limit %u to "MACF" tid %u %s, extend units %u\n",
			scb_taf_next->info.released_units_limit, TAF_ETHERC(scb_taf_next),
		        tid, taf_mutech_text[tech_idx], extend_units);
	}
	...
	taf_release_context_t *context)
{
	...
	for (tech_idx = 0; scb_taf->info.mu_type[tid] == TAF_TECH_INVALID &&
			tech_idx < TAF_NUM_MU_TECH_TYPES;
			tech_idx++) {

		if (!(method->taf_info->mu & (1 << tech_idx))) {
			continue;
		}
		if (!(scb_taf->info.mu_tech[tech_idx].enable & (1 << tid))) {
			continue;
		...
		WL_TAFM(method, MACF" tid %u consider %s\n", TAF_ETHERC(scb_taf), tid,
			taf_mutech_text[tech_idx]);
		switch (tech_idx) {
		...
		taf_release_context_t *context)
{
	...
	if (actual_release)
	{
		...
		context->public.complete == TAF_REL_COMPLETE_NOTHING &&
	...
	switch (context->public.complete) {
		...
		case TAF_REL_COMPLETE_ERR:
			...
			context->status_release_fail = TRUE;
			break;
	...
	*/
	if (scbtidstats->data.release_ncount) {
		...
		if (coeff < TAF_COEFF_IAS_MAX) {
			...
			calc = ((uint64)scbtidstats->data.total_scaled_ncount * (uint64)coeff);
	...
	*/
	...
	switch (method->type) {
		...
		case TAF_PSEUDO_RR:
			...
			scb_taf->score = MIN(scb_taf->score + new_score, TAF_SCORE_IAS_MAX);
			break;
		...
		case TAF_ATOS2:
			...
			scb_taf->score = MIN(scb_taf->score + new_score, TAF_SCORE_IAS_MAX);
			method->ias->total_score += new_score;
			break;
	...
	taf_schedule_state_t op_state)
{
	...
	switch (scb_taf->info.mu_type[tid]) {
		...
		case TAF_TECH_DL_OFDMA:
			if (taf_info->use_sampled_rate_sel & (1 << scb_taf->info.mu_type[tid])) {
				rindex = TAF_RSPEC_MU_DL_INSTANT;
				break;
			} else {
				/* fall through */
			}
		default:
			if (taf_info->use_sampled_rate_sel & (1 << TAF_TECH_DL_SU)) {
				rindex = TAF_RSPEC_SU_DL_INSTANT;
			} else {
	...
	for (s_idx = start_source; s_idx < end_source; s_idx++) {
		...
		if (op_state == TAF_SCHEDULE_VIRTUAL_PACKETS) {
			...
			if (TAF_SOURCE_IS_VIRTUAL(s_idx)) {
				...
				(pending >= method->ias->pre_rel_limit[s_idx])) {
				/* do release */
			} else if (SCB_PKTS_INFLT_FIFOCNT_VAL(scb_taf->scb, tid) == 0) {
				/* do release */
			} else {
				...
				uint32 est_rel =
					...
					TAF_IAS_TID_STATE(scb_taf, tid), pending);
			...
			if (TAF_SOURCE_IS_REAL(s_idx) && method->ias->pre_rel_limit[s_idx]) {
		...
		emptied = taf_ias_send_source(method, scb_taf, tid, s_idx, tid_state, context);
		...
		if (scb_taf->info.released_units_limit > 0 && TAF_SOURCE_IS_REAL(s_idx)) {
			taf_ias_sched_data_t* ias_sched = TAF_IAS_TID_STATE(scb_taf, tid);
			taf_ias_sched_scbtid_stats_t* scbtidstats = &ias_sched->scbtidstats;

			if (scb_taf->info.released_units_limit >
					scbtidstats->data.release_units) {
	...
	scb_taf->info.traffic.available = 0;
	for (s_idx = TAF_FIRST_REAL_SOURCE; s_idx < TAF_NUM_SCHED_SOURCES; s_idx++) {
		scb_taf->info.traffic.available |= scb_taf->info.traffic.map[s_idx];
	}
	/* return whether SCB, TID was emptied (TRUE) across all sources (or not) */
	return (scb_taf->info.traffic.available & (1 << tid)) == 0;
}
	...
	for (list = method->list; list; list = list->next) {
		...
		if (scb_taf->info.ps_mode) {
			...
			WL_TAFM(method, "skip "MACF" tid %u ps %u psq\n",
				TAF_ETHERC(scb_taf), tid, wlc_apps_psq_len(TAF_WLCM(method),
				scb_taf->scb));
			continue;
#endif /* !TAF_ENABLE_SQS_PULL */
		}
	...
	taf_schedule_state_t op_state)
{
	...
	if (finished && (tid_state->real.released_units > 0)) {
		...
		WL_TAFM(method, "high %uus, low %uus, extend %uus\n",
			TAF_UNITS_TO_MICROSEC(high_units),
			TAF_UNITS_TO_MICROSEC(low_units),
			TAF_UNITS_TO_MICROSEC(tid_state->extend.units));

		tid_state->total_units_in_transit += tid_state->real.released_units;
		WL_TAFT(taf_info, "total %u units in transit\n", tid_state->total_units_in_transit);

		if (tid_state->real.released_units < high_units) {
			if ((low_units > 0) && (tid_state->trigger == TAF_TRIGGER_IMMEDIATE) &&
				(tid_state->real.released_units < low_units) &&
				(tid_state->total_units_in_transit > low_units)) {
		...
		if (tid_state->real.released_units > low_units) {
			tid_state->resched_units[tid_state->cycle_next] =
				tid_state->real.released_units - low_units;
			...
			if (tid_state->cycle_next == 1) {
				WL_TAFT(taf_info, "cycle_next is 1 and low underflow\n");
				tid_state->cycle_next = 0;
				tid_state->resched_units[0] = tid_state->real.released_units;
				context->status_release_fail = TRUE;
			} else {
				tid_state->resched_units[tid_state->cycle_next] =
					tid_state->cycle_now ? 1 : 0;
			}

		}

		tid_state->resched_index[tid_state->cycle_next]  = context->public.ias.index;
		...
		if (tid_state->resched_units[tid_state->cycle_next] > 0) {
			...
			if ((tid_state->resched_index_suppr ==
				...
				(tid_state->cumul_units[tid_state->resched_index[cnext]] >=
			...
			if (suppressed_star_packet) {
				...
				if (taf_info->super && cnext == 0 && tid_state->cycle_now == 0 &&
						est_traffic > high_units &&
						tid_state->real.released_units >= low_units &&
						!context->status_release_fail) {
				...
				if (extra_cycle) {
					tid_state->cycle_next = 1;
				} else if (taf_info->super && tid_state->cycle_next == 1 &&
...
static int taf_ias_watchdog(void* handle)
{
	...
	do {
		...
		if (tid_state->total_units_in_transit > 0) {
			WL_ERROR(("wl%u %s: %u units in transit (expect 0)\n",
...
void* BCMATTACHFN(wlc_taf_ias_method_attach)(wlc_taf_info_t *taf_info, taf_scheduler_kind type)
{
	...
	method->ias->margin = 12;
#endif /* TAF_ENABLE_SQS_PULL */

	method->ias->release_limit = 0;
	method->ias->barrier = 2;
	...
	switch (type) {
...
#if TAF_ENABLE_MU_TX
			method->ias->mu_pair = NULL;
			method->ias->mu_mimo_rel_limit = NULL;
#endif /* TAF_ENABLE_MU_TX */
			break;
		...
		case TAF_ATOS:
			...
			method->ias->score_weights = taf_tid_score_weights;
#if TAF_ENABLE_MU_TX
			method->ias->mu_pair = taf_mu_pair;
			method->ias->mu_mimo_rel_limit = taf_mu_mimo_rel_limit;
#endif /* TAF_ENABLE_MU_TX */
			break;
	...
	switch (state) {
		...
		case TAF_SCBSTATE_MU_UL_OFDMA:
			...
			if (scb_taf) {
				...
				switch (state) {
					case TAF_SCBSTATE_MU_DL_VHMIMO:
						idx = TAF_TECH_DL_VHMUMIMO; break;
					case TAF_SCBSTATE_MU_DL_HEMIMO:
						idx = TAF_TECH_DL_HEMUMIMO; break;
					case TAF_SCBSTATE_MU_DL_OFDMA:
						idx = TAF_TECH_DL_OFDMA; break;
					//case TAF_SCBSTATE_MU_UL_OFDMA:
					//	idx = TAF_TECH_UL_OFDMA; break;
					default: return BCME_ERROR;
				}
	...
	taf_txpkt_state_t status)
{
	...
	switch (status) {
		...
		case TAF_TXPKT_STATUS_PKTFREE_DROP:
			WL_TAFM(method, MACF" tid %d TAF_TXPKT_STATUS_PKTFREE_DROP tag %u/%u:%u\n",
				TAF_ETHERC(scb_taf), tid, TAF_IS_TAGGED(WLPKTTAG(p)),
	...
	if (!ret && TAF_IS_TAGGED(WLPKTTAG(p)) &&
			WLPKTTAG(p)->pktinfo.taf.ias.units == TAF_PKTTAG_PROCESSED) {
		ret = TRUE;
	}
	return ret;
