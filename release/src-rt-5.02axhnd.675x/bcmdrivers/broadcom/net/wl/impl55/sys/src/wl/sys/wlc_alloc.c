 ...
 *
 * $Id: wlc_alloc.c 778274 2019-08-28 14:07:44Z $
 */
...
#if defined(WLRSDB) && defined(DONGLEBUILD) && defined(USE_RSDBAUXCORE_TUNE)
static void wlc_rsdb_tunables_init(wlc_tunables_t *tunables, uint unit);
#define WLC_RSDB_AUX_CORE 1
#endif /* defined(WLRSDB) && defined(DONGLEBUILD) && defined(USE_RSDBAUXCORE_TUNE) */
...
#if defined(WLRSDB) && defined(DONGLEBUILD) && defined(USE_RSDBAUXCORE_TUNE)
void
BCMATTACHFN(wlc_rsdb_tunables_init)(wlc_tunables_t *tunables, uint unit)
{
	if (unit == WLC_RSDB_AUX_CORE) {
		tunables->nrxbufpost = NRXBUFPOST_AUX;
		tunables->rxbnd = WL_RXBND_AUX;
		tunables->ntxd_lfrag = NTXD_LFRAG_AUX;
		tunables->nrxd_large = WL_NRXD_AUX;
	}
}
#endif /* WLRSDB && DONGLEBUILD && USE_RSDBAUXCORE_TUNE */
...
#if defined(WLRSDB) && defined(DONGLEBUILD) && defined(USE_RSDBAUXCORE_TUNE)
	if (RSDB_ENAB(pub)) {
		wlc_rsdb_tunables_init(pub->tunables, unit);
	}
#endif /* WLRSDB && DONGLEBUILD && USE_RSDBAUXCORE_TUNE */
...
#ifdef WL_NO_BSSCFG_SHARE
	obj_registry_disable(wlc->objr, OBJR_BSSCFG_PTR);
	obj_registry_disable(wlc->objr, OBJR_DEFAULT_BSS);
#endif // endif
