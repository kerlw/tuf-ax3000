 ...
 *
 * $Id: wlc_modesw.c 778492 2019-09-03 22:21:48Z $
 */
...
#define MODESW_CUBBY_CONFIG_GET_SIZE 	sizeof(modesw_bsscfg_cubby_t)
#define MODESW_CUBBY_CONFIG_SET_SIZE 	OFFSETOF(modesw_bsscfg_cubby_t, oper_mode_timer)
#ifdef WLRSDB
static int wlc_modesw_bsscfg_cubby_get(void * ctx, wlc_bsscfg_t *cfg,
	uint8 *buf, int *len);
static int wlc_modesw_bsscfg_cubby_set(void * ctx, wlc_bsscfg_t *cfg,
	const uint8 *buf, int len);
#else
#define wlc_modesw_bsscfg_cubby_get	NULL
#define	wlc_modesw_bsscfg_cubby_set	NULL
#endif /* WLRSDB */
...
BCMATTACHFN(wlc_modesw_attach)(wlc_info_t *wlc)
{
...
#ifdef WLRSDB
	cubby_params.fn_get = wlc_modesw_bsscfg_cubby_get;
	cubby_params.fn_set = wlc_modesw_bsscfg_cubby_set;
	cubby_params.config_size = MODESW_CUBBY_CONFIG_GET_SIZE;
#endif /* WLRSDB */
	...
	if ((modesw_info->scbh = wlc_scb_cubby_reserve(wlc, sizeof(modesw_scb_cubby_t *),
		wlc_modesw_scb_init, wlc_modesw_scb_deinit, NULL,
		(void *)modesw_info)) < 0) {
...
wlc_modesw_send_omnm_event(wlc_modesw_info_t *mswi)
{
	...
	wlc_bss_mac_event(wlc, wlc->cfg, WLC_E_OMN_MASTER, NULL, 0,
			0, 0, &ev, ev.len);
 ...
 */
int wlc_modesw_omn_master_set(wlc_modesw_info_t *mswi, wl_omnm_mod_t mod, uint16 flags)
{
	wlc_info_t *wlc;
	int ret = BCME_OK;
...
int wlc_modesw_omn_master_get(wlc_modesw_info_t *mswi, wl_omnm_info_t *omnm_out)
{
	wlc_info_t *wlc;
...
int wlc_modesw_omn_master_clear(wlc_modesw_info_t *mswi, wl_omnm_mod_t mod)
{
	wlc_info_t *wlc = mswi->wlc;
...
static int wlc_modesw_omn_master(wlc_modesw_info_t *mswi, void *in, int in_len,
	void *out, int out_len)
{
	wlc_info_t *wlc = mswi->wlc;
...
INLINE
void wlc_set_modesw_bw_init(wlc_modesw_info_t *modesw, bool init)
{
...
INLINE
bool wlc_get_modesw_bw_init(wlc_modesw_info_t *modesw)
{
...
INLINE
bool wlc_modesw_get_last_load_req_if(wlc_modesw_info_t* modesw)
{
...
INLINE
void wlc_modesw_set_last_load_req_if(wlc_modesw_info_t* modesw, bool setval)
{
	...
	int signal)
{
	...
	/* Sync other interfaces */
#ifdef WLRSDB
	/* For dual mac rsdb case, modeswitch was strted with other wlc */
	/* so open up the original wlc instead of current wlc */
	/* If dual mac rsdb move not enabled, theres no wlc change. */
	if (RSDB_ENAB(wlc->pub) && CTRL_FLAGS_HAS(pmodesw_bsscfg->ctrl_flags,
			MODESW_CTRL_RSDB_MOVE)) {
		other_wlc =  wlc_rsdb_get_other_wlc(wlc);
	}
#endif /* WLRSDB */
	/* Unblock data fifo */
	if (other_wlc->block_datafifo & DATA_BLOCK_TXCHAIN) {
		WLC_BLOCK_DATAFIFO_CLEAR(other_wlc, DATA_BLOCK_TXCHAIN);
	}
...
void wlc_modesw_oper_mode_timer(void *arg)
{
	...
	switch (notif) {
#ifdef WLRSDB
		case MODESW_TIMER_BSSCFG_CLONE:
		{
			int ret = 0;
			WL_MODE_SWITCH(("wl%d: Start BSSCFG Clone\n", WLCWLUNIT(wlc)));
			if (RSDB_ENAB(wlc->pub)) {
				other_wlc =  wlc_rsdb_get_other_wlc(wlc);
				wlc_rsdb_bsscfg_clone(wlc, other_wlc, bsscfg, &ret);
			}
			break;
		}
#endif // endif
	...
	wlc_bsscfg_t *bsscfg, uint8 oper_mode, uint8 enabled, uint32 ctrl_flags)
{
	...
	/* TODO: ADD 4366C0 specific checks here */
	if ((!DYN160_ACTIVE(wlc->pub)) && WLC_PHY_AS_80P80(wlc, bsscfg->current_bss->chanspec) &&
		(new_nss > 2)) {
	...
	if (BSSCFG_AP(bsscfg) || (BSSCFG_IBSS(bsscfg) && !(BSSCFG_NAN_MGMT(bsscfg)))) {
	...
	if (BSSCFG_INFRA_STA(bsscfg) || BSSCFG_NAN_MGMT(bsscfg) || BSSCFG_PSTA(bsscfg)) {
...
wlc_modesw_clear_phy_chanctx(void *ctx, wlc_bsscfg_t *cfg)
{
	...
	int idx;
	wlc_bsscfg_t *bsscfg;
	chanspec_t chanspec_160, chanspec_80, chanspec_40, chanspec_20;
	chanspec_t ctl_ch;
	...
	chanspec_160 = wf_channel2chspec(ctl_ch, WL_CHANSPEC_BW_160);
	...
	*/
	...
	if ((!cspec160) && (chanspec_160))
		phy_chanmgr_destroy_ctx((phy_info_t *) wlc->pi, chanspec_160);
...
wlc_modesw_mutx_state_upd_cb(void *ctx, mutx_state_upd_data_t *notif_data)
{
	...
	FOREACH_UP_AP(wlc, idx, bsscfg)
	{
		...
		if (curr_oper_mode == DOT11_D8_OPER_MODE(0, nss, 0, 1, DOT11_OPER_MODE_80MHZ)) {
...
wlc_modesw_move_cfgs_for_mimo(wlc_modesw_info_t *modesw_info)
{
	...
	if (!wlc0->pub->associated &&
		 ...
		 */
		FOREACH_BSS(wlc1, idx, icfg) {
			/* Avoid moving default inactive primary cfg in core 1 */
			if ((icfg->_idx != RSDB_INACTIVE_CFG_IDX) &&
#ifdef WLP2P
				!(BSS_P2P_DISC_ENAB(wlc1, icfg)) &&
#endif /* WLP2P */
				!icfg->associated &&
#if defined(WLTDLS) && !defined(WLTDLS_DISABLED)
				!BSSCFG_IS_TDLS(icfg) &&
#endif // endif
				(icfg->assoc->state == AS_IDLE)) {
					WL_ERROR(("wl%d: Moving Un-associated CFG %d, as_type=%d\n",
						WLCWLUNIT(wlc1), icfg->_idx, icfg->assoc->type));
					if (RSDB_ENAB(wlc->pub)) {
					icfg = wlc_rsdb_bsscfg_clone(wlc1,
						wlc_rsdb_get_other_wlc(wlc1),
						icfg, &ret);
					}
				}
		}
...
#ifdef WLRSDB
/* Copy the modesw bsscfg cubby info */
static int
wlc_modesw_bsscfg_cubby_get(void * ctx, wlc_bsscfg_t *cfg,
	uint8 *buf, int *len)
{
	modesw_bsscfg_cubby_t *pmodesw_bsscfg;
	wlc_modesw_info_t * modesw_info = (wlc_modesw_info_t *)ctx;
	int config_len = MODESW_CUBBY_CONFIG_GET_SIZE;
	...
	WL_MODE_SWITCH(("GET CUBBY modesw %p cubby %p state %d \n",
		modesw_info, pmodesw_bsscfg, pmodesw_bsscfg->state));

	/* Validate cubby info */
	if (!pmodesw_bsscfg) {
		WL_MODE_SWITCH(("%s : modeswitch cubby not enabled modesw info %p cfg %p \n",
			__FUNCTION__, modesw_info, cfg));
		*len = 0;
		return BCME_OK;
	}
	/* Validate buffer/length */
	if ((buf == NULL) || (*len < config_len)) {
		WL_ERROR(("%s: Insufficient buffer data(%p) len(%d/%d)\n",
			__FUNCTION__, buf, *len, config_len));
		*len = config_len;
		return BCME_BUFTOOSHORT;
	}

	if (buf) {
		WL_MODE_SWITCH(("wl%d.%d get cubby config info \n",
			modesw_info->wlc->pub->unit, WLC_BSSCFG_IDX(cfg)));

		*len = config_len;
		bcopy(pmodesw_bsscfg, buf, config_len);
	}
...
/* Copy the modesw bsscfg cubby info */
static int
wlc_modesw_bsscfg_cubby_set(void * ctx, wlc_bsscfg_t *cfg,
	const uint8 *buf, int len)
{
	modesw_bsscfg_cubby_t *pmodesw_bsscfg;
	wlc_modesw_info_t * modesw_info = (wlc_modesw_info_t *)ctx;
	int config_len = MODESW_CUBBY_CONFIG_SET_SIZE;
	...
	/* Validate buffer/length */
	if ((buf == NULL) || (len < config_len)) {
		WL_ERROR(("%s: Length mismatch in cubby set data(%p) len(%d/%d)\n",
			__FUNCTION__, buf, len, config_len));
		return BCME_ERROR;
	}

	/* Set the info in new bsscfg */
	bcopy(buf, pmodesw_bsscfg, config_len);

	/* Set ctrl flags separately which is not covered in MODESW_CUBBY_CONFIG_SET_SIZE */
	pmodesw_bsscfg->ctrl_flags = ((const modesw_bsscfg_cubby_t *)buf)->ctrl_flags;

	WL_MODE_SWITCH(("wl%d.%d Set cubby config info state : modesw %p  cubby %p state %d\n",
		modesw_info->wlc->pub->unit, WLC_BSSCFG_IDX(cfg),
		modesw_info, pmodesw_bsscfg, pmodesw_bsscfg->state));
