...
static const uint8 WPA_info_element_aibss[] = {
	DOT11_MNG_WPA_ID, 0x16,
	0x00, 0x50, 0xf2, 0x01, 0x01, 0x00,
	0x00, 0x50, 0xf2, 0xff,
	0x01, 0x00, 0x00, 0x50, 0xf2, 0xff,
	0x01, 0x00, 0x00, 0x50, 0xf2, 0xff
...
wlc_wpa_cap(wlc_info_t *wlc, wlc_bsscfg_t *cfg, uint8 *cap, int len)
{
	...
	if (WLC_MFP_ENAB(wlc->pub))
		cap[0] |= wlc_mfp_wsec_to_rsn_caps(wlc->mfp, cfg->wsec);
#endif // endif
...
wlc_write_wpa_ie_safe(wlc_info_t *wlc, wlc_bsscfg_t *cfg, uint8 *buf, int buflen)
{
	...
	if (!cfg->current_bss->infra) {
		...
		WPA_len = WPA_info_element_aibss[1];
		...
		bcopy(WPA_info_element_aibss, buf, TLV_HDR_LEN + WPA_len);

		/* select multicast and unicast cipher */
		/* - use highest encryption status enabled */
		if (WSEC_AES_ENABLED(wsec) && (MCAST_AES(&cfg->current_bss->wpa) ||
			MCAST_AES(&cfg->current_bss->wpa2)))
		    buf[11] = buf[17] = WPA_CIPHER_AES_CCM;
		else if (WSEC_TKIP_ENABLED(wsec))
		    buf[11] = buf[17] = WPA_CIPHER_TKIP;
		else {
			/* - w/ v.22 client, IE must specify WEP key size
			 *   which isn't available from config, so use the key...
			 */
			wsec_key_t *key = WSEC_BSS_DEFAULT_KEY(cfg);

			if (key != NULL) {
				if (key->algo == CRYPTO_ALGO_WEP1) {
					buf[11] = buf[17] = WPA_CIPHER_WEP_40;
				} else if (key->algo == CRYPTO_ALGO_WEP128) {
					buf[11] = buf[17] = WPA_CIPHER_WEP_104;
				} else if (key->algo == CRYPTO_ALGO_AES_CCM) {
					buf[11] = buf[17] = WPA_CIPHER_AES_CCM;
				} else {
					WL_ERROR(("wl%d: %s: key->algo %d"
						  " doesn't match wsec 0x%x\n",
						  wlc->pub->unit, __FUNCTION__, key->algo, wsec));
					buf[11] = buf[17] = WPA_CIPHER_WEP_40;
				}
			} else {
				buf[11] = buf[17] = WPA_CIPHER_WEP_40;
		...
		/* set authenticated key management */
		buf[23] = RSN_AKM_NONE;
...
wlc_write_rsn_ie_safe(wlc_info_t *wlc, wlc_bsscfg_t *cfg, uint8 *buf, int buflen)
{
...
#if defined(WLDPT)
	if (WPA_auth == BRCM_AUTH_DPT) {
		bcopy(BRCM_OUI, mcast->oui, DOT11_OUI_LEN);
		mcast->type = WPA_CIPHER_NONE;
	} else
#endif /* WLDPT */
#if defined(WLTDLS)
...
#ifdef MFP
		if (WLC_MFP_ENAB(wlc->pub) &&
			((wsec & MFP_REQUIRED) || (wsec & MFP_SHA256) ||
	...
	if (WPA_auth & BRCM_AUTH_DPT) {
		...
		auth->list[count++].type = BRCM_AKM_DPT;
		WPA_len += WPA_SUITE_LEN;
		buflen -= WPA_SUITE_LEN;
	}
#endif /* WLDPT */
	...
	/* length check */
	/* if buffer too small, return untouched buffer */
	BUFLEN_CHECK_AND_RETURN(RSN_CAP_LEN, buflen, orig_buf);
	wlc_wpa_cap(wlc, cfg, cap, RSN_CAP_LEN);
	WPA_len += RSN_CAP_LEN;
	buflen -= RSN_CAP_LEN;

#ifdef MFP
	if (WLC_MFP_ENAB(wlc->pub) && (wsec & MFP_CAPABLE)) {
		wpa_suite_t bip;
		if (mfp_get_bip(wlc, cfg, &bip)) {
			/* PMKID: PMKID count = 0 for AP */
			BUFLEN_CHECK_AND_RETURN(WPA2_PMKID_COUNT_LEN, buflen, orig_buf);
			bzero(cap + RSN_CAP_LEN, WPA2_PMKID_COUNT_LEN);
			WPA_len += WPA2_PMKID_COUNT_LEN;
			buflen -= WPA2_PMKID_COUNT_LEN;

			/* BIP */
			BUFLEN_CHECK_AND_RETURN(WPA_SUITE_LEN, buflen, orig_buf);
			bcopy((const uint8 *)&bip, cap + RSN_CAP_LEN + WPA2_PMKID_COUNT_LEN,
					WPA_SUITE_LEN);
			WPA_len += WPA_SUITE_LEN;
			buflen -= WPA_SUITE_LEN;
		}
...
wlc_rsn_build_ie(wlc_info_t *wlc, wlc_bsscfg_t *cfg, struct rsn_parms *rsn, void *pmcast)
{
	...
	/* mixed mode configuration also supported  */
	...
	if (cfg->WPA_auth & WPA_AUTH_UNSPECIFIED)
		...
		if (WLC_MFP_ENAB(wlc->pub) && (cfg->wsec & MFP_SHA256))
			mgmt->list[0].type = RSN_AKM_MFP_1X;
		else if (WLC_MFP_ENAB(wlc->pub) &&
			wlc_rsn_akm_lookup(rsn, RSN_AKM_MFP_1X))
			mgmt->list[0].type = RSN_AKM_MFP_1X;
		else
#endif /* MFP */
		mgmt->list[0].type = RSN_AKM_UNSPECIFIED;
		...
		if (WLC_MFP_ENAB(wlc->pub) && (cfg->wsec & MFP_SHA256))
			...
			wlc_rsn_akm_lookup(rsn, RSN_AKM_MFP_PSK))
			mgmt->list[0].type = RSN_AKM_MFP_PSK;
		else
#endif /* MFP */
		mgmt->list[0].type = RSN_AKM_PSK;
...
#if defined(WLFBT)
	bool fbt =  BSSCFG_IS_FBT(cfg, wlc->pub) &&
 ...
 */
		...
		if (
			...
			!(fbt && (reassoc == 0)) &&
#endif /* WLFBT */
			bcmwpa_is_wpa2_auth(cfg->WPA_auth) && putpmkid) {
	/* use cached PMKIDs */
...
static int
wlc_check_wpaie(wlc_info_t *wlc, wlc_bsscfg_t *bsscfg, uint8 *wpaie, uint16 *auth, uint32 *wsec)
{
...
wlc_check_wpa2ie(wlc_info_t *wlc, wlc_bsscfg_t *bsscfg, bcm_tlv_t *wpa2ie, struct scb *scb)
{
...
#ifdef MFP
	if (WLC_MFP_ENAB(wlc->pub)) {
		if ((mgmt->list[0].type == RSN_AKM_MFP_1X) ||
		    (mgmt->list[0].type == RSN_AKM_MFP_PSK)) {
			if (bsscfg->wsec & (MFP_SHA256 | MFP_REQUIRED))
	...
	if (WLC_MFP_ENAB(wlc->pub)) {
		...
		if (!wlc_mfp_check_rsn_caps(wlc->mfp, bsscfg->wsec, rsn_cap, &scb_mfp)) {
...
#if defined(WLDPT)
	if (!bcmp(mcast->oui, BRCM_OUI, DOT11_OUI_LEN)) {
		if (mcast->type == WPA_CIPHER_NONE)
			bi->wpa2.multicast = mcast->type;
		else
			WL_INFORM(("wl%d: unsupported WPA2 multicast cipher %d\n",
				wlc->pub->unit, mcast->type));
...
#endif /* BCMCCX */
#if defined(WLDPT) || defined(IBSS_PSK)
...
#endif /* defined(WLDPT) || defined(IBSS_PSK) */
			WL_INFORM(("wl%d: unsupported proprietary auth OUI "
				   "%02X:%02X:%02X\n", wlc->pub->unit,
				   mgmt->list[i].oui[0], mgmt->list[i].oui[1],
				   mgmt->list[i].oui[2]));
...
BCMATTACHFN(wlc_akm_attach)(wlc_info_t *wlc)
{
	...
	if ((akm = MALLOC(wlc->osh, sizeof(wlc_akm_info_t))) == NULL) {
		WL_ERROR(("wl%d: %s: out of mem, malloced %d bytes\n",
			wlc->pub->unit, __FUNCTION__, MALLOCED(wlc->osh)));
		goto fail;
	}
