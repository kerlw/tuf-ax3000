 ...
 *
 * $Id: wbd_master_com_hdlr.c 778661 2019-09-06 12:28:01Z $
 */
	...
	wbd_monitor_sta_item_t *monitor_sta)
{
	...
	monitor_sta->bcn_rpt_rssi = wbd_master_conv_rcpi_to_rssi(
		&monitor_sta->rssi_or_rcpi, rmrep_bcn->rcpi);
	WBD_INFO("RCPI recieved: [%d] converted RSSI: [%d] rssi_or_rcpi: %d\n",
		rmrep_bcn->rcpi, monitor_sta->bcn_rpt_rssi, monitor_sta->rssi_or_rcpi);
		...
		if (monitor_sta) {
			...
			if (i5_assoc_sta && assoc_sta) {
				assoc_sta->stats.bcn_rpt_rssi = wbd_master_conv_rcpi_to_rssi(
					&assoc_sta->stats.rssi_or_rcpi, rmrep_bcn->rcpi);
