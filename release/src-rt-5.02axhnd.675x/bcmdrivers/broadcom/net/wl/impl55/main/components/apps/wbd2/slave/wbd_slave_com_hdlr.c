 ...
 *
 * $Id: wbd_slave_com_hdlr.c 778661 2019-09-06 12:28:01Z $
 */
...
typedef struct wbd_slave_map_beacon_report_arg {
	wbd_info_t *info;		/* WBD Info */
	wbd_beacon_reports_t *report;	/* Beacon report for a STA */
...
/* Free taf' list */
static int wbd_slave_free_taf_list(wbd_taf_params_t* taf);
...
wbd_slave_get_taf_defprio(wbd_slave_item_t* slave)
{
	...
	if (!slave->taf_info) {
		wbd_slave_free_taf_list(slave->taf_info);
		goto end;
	}
	...
	*/
	...
	switch (evt_type) {
		...
		case WLC_E_CAC_STATE_CHANGE:
		{
			...
			if (!pdmif) {
				WBD_ERROR("No valid interface exist for event[%d], skip \n",
					evt_type);
				goto end;
			}
...
wbd_slave_get_fbt_config_req_data(wbd_cmd_fbt_config_t *fbt_config_req)
{
	...
	foreach_i5glist_item(iter_ifr, i5_dm_interface_type, self_device->interface_list) {
		...
		foreach_i5glist_item(iter_bss, i5_dm_bss_type, iter_ifr->bss_list) {
			...
			if (wbd_is_fbt_possible(prefix) == WBD_FBT_DEF_FBT_DISABLED) {
				continue;
			}
...
static int
wbd_slave_free_taf_list(wbd_taf_params_t* taf)
{
	int i;
	int ret = WBDE_OK;
	WBD_ENTER();
	...
	unsigned char *tlv_data, unsigned short tlv_data_len)
{
	...
	/* Check if the STA exists in Self Device or not */
	i5_assoc_sta = wbd_ds_find_sta_in_device(sdev, (unsigned char*)&cmd.stamac, &ret);
	WBD_ASSERT_MSG("STA["MACF"]. %s\n", ETHER_TO_MACF(cmd.stamac), wbderrorstr(ret));

	/* Get Parent BSS */
	i5_parent_bss = (i5_dm_bss_type*)WBD_I5LL_PARENT(i5_assoc_sta);
	parent_bssid = (struct ether_addr*)i5_parent_bss->BSSID;

	/* Remove a STA item from a Slave's Assoc STA List */
	ret = blanket_sta_assoc_disassoc(parent_bssid, &cmd.stamac, 0, 0, 1, NULL, 0, 0);
	if (ret == WBDE_OK) {
		/* Update Driver to remove stale client entry, at present reason is fixed i.e.
		 * DOT11RC_DEAUTH_LEAVING, it can be generalized if required.
		 */
