 ...
 *
 * $Id: wbd_master_control.c 778661 2019-09-06 12:28:01Z $
 */
	...
	i5_dm_bss_type *bcn_bss, int adjust)
{
	...
	if (WBD_RSSI_VALID(sta->bcn_rpt_rssi) &&
		BEACON_RSSI_RECENT(sta->bcn_tm, wbd_get_ginfo())) {
		return sta->bcn_rpt_rssi;
	} else if (WBD_RSSI_VALID(sta->rssi)) {
		/* stamon rssi is present consider it */
		est_rssi = sta->rssi;
		bss = curr_bss;
	} else if (BEACON_RSSI_RECENT(bcn_tm, wbd_get_ginfo())) {
	...
	WBD_DEBUG("ESTIMATE: MONITOR_RSSI[%d] RSSI_BCN[%d] BEACON_TM[%lu]\n",
		monitor_rssi, bcn_rssi,	(unsigned long)bcn_tm);
	return wbd_tbss_get_estimated_rssi(est_rssi, est_rssi_bcn, curr_bss, bss, adjust);
}
...
wbd_tbss_algo_nws(wbd_master_info_t *master_info, i5_dm_clients_type *weak_sta, bool is_dual_band)
{
	...
	foreach_i5glist_item(i5_device, i5_dm_device_type, i5_topology->device_list) {
		...
		if (i5_device == parent_device) {
			...
			if (BEACON_RSSI_RECENT(assoc_sta->stats.bcn_tm, wbd_get_ginfo())) {
