 ...
 *
 * $Id: wbd_slave.c 778661 2019-09-06 12:28:01Z $
 */
...
wbd_read_blanket_slave_nvrams(wbd_blanket_slave_t *bkt_slave)
{
	...
	/* Get NVRAM : Avoid Looping between repeaters method (block MAC, block BSS etc...) */
	loop_avoid_method = (uint16)blanket_get_config_val_uint(NULL, WBD_NVRAM_AVOID_LOOP_METHOD,
		WBD_DEF_AVOID_LOOP_BSS_DOWN);
	if (loop_avoid_method == WBD_DEF_AVOID_LOOP_MAC_BLOCK) {
		bkt_slave->flags |= WBD_BKT_SLV_FLAGS_AVOID_LOOP_BLOCK_MAC;
	}

	/* Get NVRAM : Whether to send BTM request for the BTM query from application. By default
	 * firmware sends BTM request upon BTM query from STA
	 */
	if (blanket_get_config_val_int(NULL, NVRAM_MAP_WNM_NO_BTQ_RESP, 0)) {
		bkt_slave->flags |= WBD_BKT_SLV_FLAGS_WNM_NO_BTQ_RESP;
	}

	WBD_INFO("AP Auto Config Search Treshold[%d] LoopAvoidMethod[%d]\n",
		bkt_slave->n_ap_auto_config_search_thld, loop_avoid_method);
}
...
wbd_ieee1905_device_deinit_cb(i5_dm_device_type *i5_device)
{
	...
	wbd_blanket_slave_t *wbd_slave = (wbd_blanket_slave_t*)g_wbdinfo->wbd_slave;
	WBD_ENTER();
	...
	if (!WBD_CLOSING_APP(g_wbdinfo->flags) && !I5_IS_CTRLAGENT(self_device->flags) &&
		...
		if (wbd_slave->flags & WBD_BKT_SLV_FLAGS_AVOID_LOOP_BLOCK_MAC) {
			wbd_slave_block_unblock_backhaul_sta_assoc(0);
		} else {
			wbd_slave_enable_disable_backhaul_bss(0);
		}
	}
...
wbd_ieee1905_ap_auto_config_resp_cb(i5_dm_device_type *i5_device)
{
	...
	if (!I5_IS_CTRLAGENT(self_device->flags) && I5_IS_MULTIAP_CONTROLLER(i5_device->flags)) {
		if (wbd_slave->flags & WBD_BKT_SLV_FLAGS_AVOID_LOOP_BLOCK_MAC) {
			...
			wbd_slave_enable_disable_backhaul_bss(1);
		}
	}
...
wbd_ieee1905_ap_auto_config_search_sent()
{
	...
	if (!I5_IS_CTRLAGENT(self_device->flags) &&
		...
		WBD_DEBUG("Disable Backhaul BSS Current Count[%d] Threshold[%d]\n",
			wbd_slave->n_ap_auto_config_search,
	...
	ieee1905_chscan_req_msg *chscan_req)
{
	...
	/* Just a hack for plugfest. Set this NVRAM to 0. This is because, In process event message,
	 * we are not sending the scan report if the onboot is TRUE. onboot is TRUE even if this
	 * NVRAM is set.
	 */
	blanket_nvram_prefix_set(NULL, NVRAM_MAP_CHSCAN_FORCE_ONBOOT, 0);
...
wbd_ieee1905_get_bh_sta_profile_cb(char *wds_ifname)
{
	WBD_INFO("Get profile for wds_ifname %s\n", wds_ifname);

	return wbd_blanket_get_bh_sta_profile(wds_ifname);
}
...
main(int argc, char *argv[])
{
	...
	ret = ieee1905_module_init(g_wbdinfo, MAP_MODE_FLAG_AGENT, 0, &cbs);
	WBD_ASSERT();
	...
	/* If any of the bSTA is associated, inform IEEE1905 to create VLAN interface */
	if ((i5_ifr = wbd_slave_is_any_bsta_associated())) {
