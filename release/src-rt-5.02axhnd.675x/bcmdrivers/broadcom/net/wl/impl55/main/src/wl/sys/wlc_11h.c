 ...
 *
 * $Id: wlc_11h.c 778492 2019-09-03 22:21:48Z $
 */
...
/* 11h common module info */
typedef struct wlc_11h_shared {
...
BCMATTACHFN(wlc_11h_attach)(wlc_info_t *wlc)
{
	...
	m11h->wlc = wlc;
	m11h->cmn = (wlc_11h_shared_t *) obj_registry_get(wlc->objr, OBJR_11H_CMN_INFO);
	if (m11h->cmn == NULL) {
		if (!(m11h->cmn = (wlc_11h_shared_t *)MALLOCZ(wlc->osh,
			sizeof(wlc_11h_shared_t)))) {
			WL_ERROR(("wl%d: %s: out of memory, malloced %d bytes\n",
				wlc->pub->unit, __FUNCTION__, MALLOCED(wlc->osh)));
			goto fail;
		}
		obj_registry_set(wlc->objr, OBJR_11H_CMN_INFO, m11h->cmn);
	}
	...
	m11h->cmn->_spect_management = SPECT_MNGMT_LOOSE_11H;	/* 802.11h */
...
BCMATTACHFN(wlc_11h_detach)(wlc_11h_info_t *m11h)
{
	...
	if ((m11h->cmn) && (obj_registry_unref(m11h->wlc->objr, OBJR_11H_CMN_INFO) == 0)) {
		obj_registry_set(m11h->wlc->objr, OBJR_11H_CMN_INFO, NULL);
		MFREE(m11h->wlc->osh, m11h->cmn, sizeof(wlc_11h_shared_t));
		m11h->cmn = NULL;
	}
                           ...
                           uint8 *cp, int buflen)
{
	...
	const struct subband_desc subbands_5g[] = {
		{34, 4}, {36, 4},       /* 34-46, 36-48 */
		{52, 4},                /* 52-64 */
		{100, 12}, {149, 4},    /* 100-144, 149-161 */
		{165, 1}, {184, 4},     /* 165, 184-196 */
	};
	...
	for (; subband_idx < end_subband; subband_idx++) {
		...
		if (band == WL_CHANSPEC_BAND_5G && subband_idx == 0)
			subband_idx = 1;
	}
...
wlc_11h_calc_sup_chan_ie_len(void *ctx, wlc_iem_calc_data_t *data)
{
	...
	if (BSS_WL11H_ENAB(wlc, cfg)) {
		uint8* ptr = _wlc_11h_write_sup_chan_ie(m11h, cfg, target->chanspec,
		                                        buf, sizeof(buf));
		return (uint)(ptr - buf);
	}
...
wlc_11h_tdls_calc_sup_chan_ie_len(void *ctx, wlc_iem_calc_data_t *calc)
{
	...
	return (uint)(_wlc_11h_write_sup_chan_ie(m11h, calc->cfg,
...
wlc_11h_set_spect(wlc_11h_info_t *m11h, uint spect)
{
	...
	if (m11h->cmn->_spect_management == spect)
		return BCME_OK;

	m11h->cmn->_spect_management = spect;
...
{
	return m11h->cmn->_spect_management;
}
