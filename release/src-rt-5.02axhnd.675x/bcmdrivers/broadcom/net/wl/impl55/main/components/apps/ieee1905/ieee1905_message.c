...
void i5MessageAPCapabilityReportSend(i5_message_type *pmsg_req)
{
  ...
  uint8 *pbuf_cac_capability_info = NULL;
#if defined(MULTIAPR2)
  ...
  pbuf_cac_capability_info = (uint8*)malloc(I5_PACKET_BUF_LEN_512);
  if (!pbuf_cac_capability_info) {
      i5TraceDirPrint("Malloc failed cac capability buffer\n");
      goto end;
  }
  memset(pbuf_cac_capability_info, 0, I5_PACKET_BUF_LEN_512);

  if (i5_config.cbs.prepare_cac_capabilities) {
    i5Trace("prepare cac capability : \n");
    i5_config.cbs.prepare_cac_capabilities(&pbuf_cac_capability_info, &payload_len);
  }

  i5TlvCacCapabilitiesTypeInsert(pmsg, pbuf_cac_capability_info, payload_len);
#endif /* MULTIAPR2 */
...
#if defined(MULTIAPR2)
    i5TlvChannelScanReportingPolicyTypeInsert(pmsg, neighbor_al_mac_address, &i5_config.policyConfig);
...
#if defined(MULTIAPR2)
  if (steer_req->profile == map_steer_req_profile2) {
...
#if defined(MULTIAPR2)
  rc |= i5TlvAPExtendedMetricsTypeInsert(pmsg, pbss);
#endif /* MULTIAPR2 */
...
void i5MessageChannelScanRequestSend(i5_socket_type *psock,
  unsigned char *neighbor_al, ieee1905_chscan_req_msg *chscan_req)
{
  i5_message_type *pmsg;
  i5_dm_device_type *device = i5DmDeviceFind(neighbor_al);
  ...
  if (!device) {
    i5TraceError("Channel Scan Request not sent; as Neighbor "
      " Device " I5_MAC_DELIM_FMT " is a Multi-AP R1 Agent.\n", I5_MAC_PRM(neighbor_al));
    return;
  }
...
int i5MessageTunneledMessageSend(i5_socket_type *psock, unsigned char *dst_mac,
  ieee1905_tunnel_msg_t *tunnel_msg)
{
  i5_message_type* pmsg = NULL;

  if (!psock || !dst_mac || !tunnel_msg) {
...
void i5MessageClientDisassociationStatsSend(i5_socket_type *psock,  unsigned char *dst_mac,
  unsigned char *bssid, uint16 reason ,i5_dm_clients_type *pdmclient)
{
  i5_message_type *pmsg = NULL;
...
int i5MessageCACRequestSend(i5_socket_type *psock, unsigned char *dst_mac,
  ieee1905_cac_rqst_list_t *cac_rqst)
{
  i5_message_type* pmsg = NULL;

  if (!psock || !dst_mac || !cac_rqst) {
...
void i5MessageFailedConnectionMessageSend(i5_socket_type *psock, unsigned char *dst_mac, unsigned char *mac, uint16 status)
{
  i5_message_type *pmsg = NULL;
...
int i5MessageCACTerminationSend(i5_socket_type *psock, unsigned char *dst_mac,
  ieee1905_cac_termination_list_t *cac_terminate)
{
  i5_message_type* pmsg = NULL;

  if (!psock || !dst_mac || !cac_terminate) {
