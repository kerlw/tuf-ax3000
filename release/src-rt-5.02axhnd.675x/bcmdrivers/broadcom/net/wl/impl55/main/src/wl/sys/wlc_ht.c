 ...
 *
 * $Id: wlc_ht.c 778752 2019-09-10 06:04:58Z $
 */
...
static void
wlc_ht_mimops_action(wlc_ht_info_t *pub, wlc_bsscfg_t *cfg);
#else
...
static const bcm_iovar_t ht_iovars[] = {
	...
	{"nmode", IOV_NMODE,
	(IOVF_SET_DOWN|IOVF_OPEN_ALLOW|IOVF_RSDB_SET), 0, IOVT_UINT32, 0
	},
...
uint8
wlc_ht_get_cfg_mimops_PM(wlc_ht_info_t *hti, wlc_bsscfg_t *cfg)
{
	return BSS_HT_INFO_PRIV(hti, cfg)->mimops_PM;
}
	...
	switch (cmd) {
		...
		case WLC_SET_FAKEFRAG:
		{
			...
			FOREACH_WLC(wlc->cmn, idx, wlc_iter) {
				...
				if (HE_ULMU_ENAB(wlc_iter->pub)) {
					wlc_ulmu_fburst_set(wlc_iter->ulmu, bool_val);
				}
			}
		}
			wlc_ht_frameburst_limit(pub);
...
wlc_ht_mimops_action(wlc_ht_info_t *pub, wlc_bsscfg_t *cfg)
{
	wlc_ht_bss_info_priv_t *cubby = BSS_HT_INFO_PRIV(pub, cfg);

	if (cubby->mimops_ActionRetry & WLC_MIMOPS_RETRY_SEND) {
		wlc_mimops_action_ht_send(pub, cfg, cubby->mimops_PM);
	} else if (cubby->mimops_ActionRetry & WLC_MIMOPS_RETRY_NOACK) {
		wlc_mimops_action_ht_send(pub, cfg, cubby->mimops_ActionPM);
	}
}
#endif /* STA */
...
wlc_send_action_ht_mimops(wlc_ht_info_t *pub, wlc_bsscfg_t *bsscfg, uint8 mimops_mode)
{
	...
	/*
	 * If sending fails, p will be freed, but not
	 * set to NULL. Return NULL to let the calling function know
	 * sending failed.
	 */
	if (wlc_sendmgmt(wlc, p, bsscfg->wlcif->qi, scb) == FALSE)
		return NULL;
	...
	case WL_11N_4x4:
		...
		FOREACH_WLC_BAND(wlc, bandunit) {
			...
			if (RSDB_ENAB(wlc->pub)) {
				if (!wlc_rsdb_band_match(wlc, band)) {
					continue;
				}
			}
#endif // endif
	...
	ratespec_t rspec, struct wl_rxsts *sts)
{
	...
	if ((subtype == FC_SUBTYPE_QOS_DATA) || (subtype == FC_SUBTYPE_QOS_NULL)) {
		 ...
		 */
			...
			if (plcp[0] | plcp[1] | plcp[2]) {
				if (!(RXS_PHYSTS_VALID_REV128(wlc->pub->corerev, &wrxh->rxhdr))) {
					/* First MPDU:
					 * PLCP header is valid, Phy RxStatus is not valid
					 */
					sts->nfrmtype |= WL_RXS_NFRM_AMPDU_SUB;
...
uint8 wlc_ht_get_mimops_ActionPM(wlc_info_t *wlc, wlc_bsscfg_t *cfg)
{
	wlc_ht_bss_info_priv_t *cubby;
	cubby = BSS_HT_INFO_PRIV(wlc->hti, cfg);

	return cubby->mimops_ActionPM;
}

void wlc_ht_set_mimops_ActionPM(wlc_info_t *wlc, wlc_bsscfg_t *cfg, uint8 action_pm)
{
	wlc_ht_bss_info_priv_t *cubby;
	cubby = BSS_HT_INFO_PRIV(wlc->hti, cfg);
...
uint8 wlc_ht_get_mimops_ActionRetry(wlc_info_t *wlc, wlc_bsscfg_t *cfg)
{
	wlc_ht_bss_info_priv_t *cubby;
	cubby = BSS_HT_INFO_PRIV(wlc->hti, cfg);

	return cubby->mimops_ActionRetry;
}

void wlc_ht_set_mimops_ActionRetry(wlc_info_t *wlc, wlc_bsscfg_t *cfg, uint8 action_retry)
{
	wlc_ht_bss_info_priv_t *cubby;
	cubby = BSS_HT_INFO_PRIV(wlc->hti, cfg);

	cubby->mimops_ActionRetry = action_retry;
	return;
}
