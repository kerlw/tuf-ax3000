 ...
 *
 * $Id: wbd_shared.c 778661 2019-09-06 12:28:01Z $
 */
...
static int wbd_load_ts_vlanid(ieee1905_config *config, unsigned short vlanid)
{
	...
	WBD_DEBUG("Load VLAN config for VLAN ID %d\n", vlanid);

	ts_policy = i5DmTSPolicyFindVlanId(&config->ts_policy_list, vlanid);
	/* If traffic separation policy not exists for VLANID create new and add to list */
	if (ts_policy == NULL) {
		WBD_DEBUG("Create New policy for VLAN ID %d and add SSIDs\n", vlanid);
		ts_policy = (ieee1905_ts_policy_t*)wbd_malloc(sizeof(*ts_policy), &ret);
		WBD_ASSERT();

		memset(ts_policy, 0, sizeof(*ts_policy));
		ts_policy->vlan_id = vlanid;
		ieee1905_glist_append(&config->ts_policy_list, (dll_t*)ts_policy);
		ieee1905_glist_init(&ts_policy->ssid_list);
	} else {
		WBD_DEBUG("VLAN ID %d already exists add SSIDs\n", vlanid);
	}
	...
	if (strlen(nvval) <= 0) {
		WBD_WARNING("NVRAM[%s] Not Defined\n", nvname);
		goto end;
	}

	/* For each SSID in the NVRAM list */
	foreach(name, nvval, next) {
		ssid_list = (ieee1905_ssid_list_type*)wbd_malloc(sizeof(*ssid_list), &ret);
		WBD_ASSERT();

		memset(ssid_list, 0, sizeof(*ssid_list));
		ssid_list->ssid.SSID_len = (unsigned char)strlen(name);
		WBDSTRNCPY((char*)ssid_list->ssid.SSID, name, sizeof(ssid_list->ssid.SSID));
		ieee1905_glist_append(&ts_policy->ssid_list, (dll_t *)ssid_list);
		WBD_INFO("VLAN ID[%d] SSID[%s] Len[%d]\n\n", vlanid, ssid_list->ssid.SSID,
			ssid_list->ssid.SSID_len);
	}
...
static int wbd_get_vlan_config(ieee1905_config *config)
{
	char *nvval;
	char name[256], *next = NULL;
	...
	/* Get primary VLAN ID */
	config->prim_vlan_id = blanket_get_config_val_uint(NULL, NVRAM_MAP_TS_PRIM_VLAN_ID, 0);
	...
	if (config->prim_vlan_id > 0) {
		config->flags |= I5_INIT_FLAG_TS_ACTIVE;
	}

	WBD_INFO("Primary VLAN ID[%d] Default PCP[%d] Flags[%x]\n\n",
	...
	i5DmTSPolicyCleanup(&config->ts_policy_list);
	WBD_INFO("Failed to load traffic separation policy\n");
	return -1;
}
...
/* Create Traffic separation policy like profile2 from ifnames for profile1 */
static int wbd_get_vlan_config(ieee1905_config *config)
{
	char *lan1_ifnames, *wbd_ifnames, *next_intf, *nvval;
	char var_intf[IFNAMSIZ] = {0}, prefix[IFNAMSIZ];
	unsigned short vlan_id, sec_vlan_id;
	ieee1905_ts_policy_t *ts_policy = NULL;
	...
	sec_vlan_id = (unsigned short)blanket_get_config_val_uint(NULL, NVRAM_MAP_TS_SEC_VLAN_ID,
		WBD_DEF_SEC_VLAN_ID);
	...
	WBD_INFO("Primary VLAN ID[%d] Secondary VLAN ID[%d] Flags[%x]\n\n",
		config->prim_vlan_id, sec_vlan_id, config->flags);
	...
	foreach(var_intf, wbd_ifnames, next_intf) {
		...
		WBD_DEBUG("VLAN ID[%d] for ifname[%s] prefix[%s]\n", vlan_id, var_intf, prefix);
