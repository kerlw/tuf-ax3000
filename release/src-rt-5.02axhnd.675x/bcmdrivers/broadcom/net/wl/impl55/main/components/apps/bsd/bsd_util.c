...
int bsd_info_init(bsd_info_t *info)
{
	...
	foreach(var_intf, bsd_ifnames_sorted, next_intf) {
		...
		bssinfo->wlif_hdl = wl_wlif_init(NULL, bssinfo->ifnames,
			bsd_ioctl_callback_func, &bssinfo->intf_info->remote);
...
*/
			...
			for (cnt = 0; cnt < count; cnt++) {
				 ...
				 * if the STA is not on any radio (not added by (re)assoc event)
				 * and txrate is big enough (use 10 Mb/s as threshold)
				 */
				if ((bsd_check_sta_assoc_any(info, &(maclist->ea[cnt])) != TRUE) &&
					(sta_info->tx_rate / 1000) >= 10)
					enable = TRUE;
				else
					enable = FALSE;

				sta = bsd_sta_by_addr(info, bssinfo, &(maclist->ea[cnt]), enable);
