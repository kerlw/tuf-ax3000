 ...
 *
 * $Id: wbd_ds.h 778661 2019-09-06 12:28:01Z $
 */
...
#define WBD_BOUNCE_DETECT_CNT_BH	2
...
/* WBD BSS Item Bit flags */
#define WBD_FLAGS_BSS_FBT_ENABLED	0x0001	/* FBT enabled */
#define WBD_FLAGS_BSS_BLOCKED		0x0002	/* BSS is blocked for all the associations */
...
typedef enum {
	...
	WBD_REASON_DFS_AP_MOVE_STUNT_SUCCESS, /* mark dfs ap move stunt success */
	WBD_REASON_CSA_TO_DFS_CHAN_FOR_CAC_ONLY /* perform full time CAC only */
...
typedef struct wbd_ifr_item {
	...
	uint8 cac_rqst_opclass;		/* opclass from controller to intiate cac rqst */
	uint8 cac_rqst_chan;		/* chan from controller to initiate cac rqst */
	uint8 cac_rqst_status;		/* current status of CAC on radio */

	uint8 cac_method_flag;		/* 0x00 - Continous CAC
					 * 0x02 - MIMO dimension reduced
					 */
	uint32 seconds;			/* seconds required to finish CAC */
...
i5_dm_bss_type* wbd_ds_get_i5_bss_match_ifname_in_ifr(i5_dm_interface_type *i5_ifr,
	unsigned char *ifname, int *error);
