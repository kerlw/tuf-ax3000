...
static void i5CreatePrimaryVLAN(char *ifr, unsigned short vlanid, char *bridge, int noTag)
{
  ...
  snprintf(strvlanid, sizeof(vlanid), "%d", vlanid);
  ...
  if (I5_IS_TS_USING_IPTABLE(i5_config.flags)) {
    ...
    if (noTag) {
      ...
      eval("vlanctl", "--if", ifr, "--rx", "--tags", "0", "--filter-ethertype", "0x888e",
        "--set-rxif", vprim, "--rule-append");

      /* Receive EAPOL packets to primary interface with TAG and remove the TAG */
      eval("vlanctl", "--if", ifr, "--rx", "--tags", "1", "--filter-ethertype", "0x888e",
      ...
      eval("vlanctl", "--if", ifr, "--tx", "--tags", "0", "--filter-txif", vprim,
        "--filter-ethertype", "0x888e", "--rule-append");
...
void i5GlueCreateVLAN(char *ifname, int isWireless)
{
  ...
  /* If the backhaul STA joined to the current AP is of profile1(i.e. R1), then do not create
   * VLANs
   */
  if (isWireless && i5GlueIsIfnameWDS(ifname)) {
