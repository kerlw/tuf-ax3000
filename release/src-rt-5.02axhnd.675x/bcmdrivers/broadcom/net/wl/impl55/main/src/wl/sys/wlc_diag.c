 ...
 *
 * $Id: wlc_diag.c 776422 2019-06-27 22:27:11Z $
 */
...
#define WAIT_COUNT_XI	100	/* wait count for tx interrupt */
	...
	if (!PIO_ENAB(wlc->pub)) {
		...
		if (wlc_pio_txdepthget(wlc->hw->pio[TX_DATA_FIFO]) == 0) {
			wlc_pio_txdepthset(wlc->hw->pio[TX_DATA_FIFO],
			                   (wlc_bmac_read_shm(wlc_hw, M_FIFOSIZE0(wlc_hw)) << 8));
		}
	...
	if (!PIO_ENAB(wlc->pub))
		...
		ASSERT(!"PIO not supported");
		/* dma_txpioloopback(osh, &regs->f64regs.dmaregs[TX_DATA_FIFO].xmt); */
	}
...
wlc_diag_queue_tx_packet(wlc_info_t *wlc)
{
	...
	msdulen += payload_len;
	if ((pkt = PKTGET(wlc->osh, TXOFF + msdulen, TRUE)) == NULL) {
		goto exit;
	}
