 ...
 *
 * $Id: wlm.c 777067 2019-07-18 09:28:30Z $
 */
...
#define MALLOC_ARG_MEM(ptr, size) \
	ptr = malloc(size); \
	if (ptr == NULL) { \
		ret = BCME_NOMEM; \
...
int wlmCreateSecondaryInterface(char *ifType, char *ifTypeArgs)
{
	...
	if (ifType == NULL) {
		...
		MALLOC_ARG_MEM(args[i], WLM_MAX_ARG_LEN)
...
int wlmGetCounters(char *stats, int length)
{
	...
	cnt = (wl_cnt_ver_11_t*)malloc(sizeof(wl_cnt_ver_11_t));
	if (cnt == NULL) {
...
wlmCisUpdate(int offset, char *byteStream, int preview)
{
	...
	MALLOC_ARG_MEM(args[i], WLM_MAX_INT2STR_SIZE)
...
wlmOtatestStatus(int cnt)
{
	...
	test_status = (wl_ota_test_status_t *)malloc(sizeof(wl_ota_test_status_t));
	if (test_status == NULL) {
...
wlOtatestRssi()
{
	...
	test_rssi = (wl_ota_test_rssi_t *)malloc(sizeof(wl_ota_test_rssi_t));
	if (test_rssi == NULL) {
		printf("Malloc Failure\n");
		return FALSE;
	}
