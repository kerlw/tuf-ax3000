 ...
 *
 * $Id: wbd_blanket_utility.c 778661 2019-09-06 12:28:01Z $
 */
...
/* Enable or Disable all the backhaul BSS on the self device */
void
wbd_slave_enable_disable_backhaul_bss(int enable)
{
	int ret;
	...
	foreach_i5glist_item(iter_ifr, i5_dm_interface_type, i5_device->interface_list) {
		...
		foreach_i5glist_item(iter_bss, i5_dm_bss_type, iter_ifr->bss_list) {
			...
			if (get_ifname_unit(iter_bss->ifname, NULL, &setbuf.bsscfg_idx) != 0) {
				WBD_ERROR("ifname(%s): unable to parse unit.subunit\n",
					iter_bss->ifname);
				continue;
			}

			if (setbuf.bsscfg_idx == -1)
				setbuf.bsscfg_idx = 0;

			if (enable) {
				/* If enable, check if the BSS is already enabled */
				if (blanket_is_bss_enabled(iter_bss->ifname, setbuf.bsscfg_idx)) {
					WBD_DEBUG("Ifname[%s] BSS["MACDBG"] is already enabled\n",
						iter_bss->ifname, MAC2STRDBG(iter_bss->BSSID));
					continue;
				}
			} else {
				/* If not enable, check if the BSS is already disabled */
				if (!blanket_is_bss_enabled(iter_bss->ifname, setbuf.bsscfg_idx)) {
					WBD_DEBUG("Ifname[%s] BSS["MACDBG"] is already disabled\n",
						iter_bss->ifname, MAC2STRDBG(iter_bss->BSSID));
					continue;
				}
			}

			ret = wl_iovar_set(iter_bss->ifname, "bss", &setbuf, sizeof(setbuf));
			WBD_INFO("Ifname[%s] %s BSS["MACDBG"] ret[%d]\n",
				iter_bss->ifname, enable ? "Enabling" : "Disabling",
				MAC2STRDBG(iter_bss->BSSID), ret);
		}
	}
}
	...
	foreach_i5glist_item(iter_ifr, i5_dm_interface_type, i5_device->interface_list) {
		...
		foreach_i5glist_item(iter_bss, i5_dm_bss_type, iter_ifr->bss_list) {
			...
			if (bss_vndr_data) {
				/* If it is to unblock */
				if (unblock) {
					/* If the BSS is already unblocked, skip */
					if (!(bss_vndr_data->flags & WBD_FLAGS_BSS_BLOCKED)) {
						WBD_DEBUG("Ifname[%s] BSS["MACDBG"] unblock[%d] "
							"Already UnBlocked. So Skip\n",
							iter_bss->ifname,
							MAC2STRDBG(iter_bss->BSSID), unblock);
						continue;
					}
					bss_vndr_data->flags &= ~WBD_FLAGS_BSS_BLOCKED;
				} else {
					/* If the BSS is already blocked, skip */
					if (bss_vndr_data->flags & WBD_FLAGS_BSS_BLOCKED) {
						WBD_DEBUG("Ifname[%s] BSS["MACDBG"] unblock[%d] "
							"Already Blocked. So Skip\n",
							iter_bss->ifname,
							MAC2STRDBG(iter_bss->BSSID), unblock);
						continue;
					}
		...
		switch (cmd) {
			...
			case IEEE1905_BH_STA_ROAM_ENAB_VAP_FOLLOW:
				...
				if (WBD_IS_HAPD_ENABLED(wbd_get_ginfo()->flags)) {
					...
					system(tmp);
					snprintf(tmp, sizeof(tmp), "wpa_cli -p %s/%s_wpa_supplicant"
						" -i %s scan", WBD_HAPD_SUPP_CTRL_DIR,
						i5_iter_ifr->wlParentName, i5_iter_ifr->ifname);
					system(tmp);
				} else {
