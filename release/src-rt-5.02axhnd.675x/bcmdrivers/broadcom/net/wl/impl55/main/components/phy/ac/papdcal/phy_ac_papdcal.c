 ...
 *
 * $Id: phy_ac_papdcal.c 778688 2019-09-07 01:53:29Z $
 */
#include <phy_cfg.h>
#include <typedefs.h>
#include <bcmdefs.h>
...
typedef struct {
	...
	uint8	tia_mode_2g;
	uint8	tia_mode_5g;
	uint8	tia_search_5g[PHY_CORE_MAX];
	uint8	cal_dac_mode;
	uint8	cal_adc_mode;
...
phy_ac_populate_papd_params(phy_ac_papdcal_info_t *papd_info)
{
	...
	if (ACMAJORREV_4(pi->pubpi->phy_rev)) {
		...
		if (PHY_EPAPD(pi)) {
			...
			papd_params->papd_calidx_2g = ((uint16)PHY_GETINTVAR_DEFAULT(pi,
				rstr_nb_papd_calidx, 24)) & PAPD_CAL_IDX_2G;
			papd_params->papd_calidx_5g = (((uint16)PHY_GETINTVAR_DEFAULT(pi,
		...
		FOREACH_CORE(pi, core) {
			/* populate initial settings for 5G TIA gain search */
			papd_params->tia_search_5g[core] = papd_params->tia_mode_5g;
		}
		papd_params->epsilon_offset_2g = ((uint32)PHY_GETINTVAR_DEFAULT(pi,
				rstr_nb_eps_offset, 0)) & PAPD_EPS_OFFSET_2G;
...
/* a preset target value for the EPS curve to have optimum sampling range */
	...
	uint8  bbmult       = 64;
	uint8  bbmult_next  = 64;
	uint8  bbmult_lower = 32;
	uint8  bbmult_upper = 128;
	int32  eps_re, eps_im;
	uint32 eps_complex;
	uint16 numidx;
	uint32 eps_ref = 61;
	...
	if (CHSPEC_IS2G(pi->radio_chanspec)) {
		if (ACMAJORREV_129(pi->pubpi->phy_rev)) {
			/* AM2AM of (4095 * 2 * d) ^ 2 */
			/* Where d = 0.98, 1, 1.02 */
			clipthresholdl = 64419886;
			clipthresholdu = 69785974;
			clipthreshold  = 67174416;
		} else {
			/* AM2AM of (4095 * 1.5 * d) ^ 2 */
			/* Where d = 0.97, 1, 1.03 */
			clipthresholdl = 35500445;
			clipthresholdu = 40028082;
			clipthreshold  = 37730306;
		}
	} else {
		/* AM2AM of (4095 * 1.55 * d) ^ 2 */
		/* Where d = 0.98, 1, 1.02 */
		clipthresholdl = 38692194;
		clipthresholdu = 41915201;
		clipthreshold  = 40287583;
	}
	...
	for (i = 0; i < GAINCTRL_ITER_28NM; i++) {
		...
		if (ACMAJORREV_129(pi->pubpi->phy_rev) && CHSPEC_IS2G(pi->radio_chanspec)) {
...
/* ALPF bypass mode matches the normal reception mode, where ALPF is also bypassed */
	...
	if (RADIOREV(pi->pubpi->radiorev) == 0) {
		...
		/* A1 and above using auxgm with bypass on */
		papd_loopback_mode = AUXGM_PATH;
		papd_bypass_mode   = 1;
	}
...
wlc_phy_txpwr_papd_cal_run_tiny(phy_info_t *pi,	uint8 tx_pre_cal_pwr_ctrl_state)
{
	...
	if (!ACMAJORREV_44(pi->pubpi->phy_rev) && !ACMAJORREV_51(pi->pubpi->phy_rev) &&
			!ACMAJORREV_128(pi->pubpi->phy_rev) &&
	...
	FOREACH_CORE(pi, core) {
		/* clear eps table  */
		uint8 tbl_size = phy_ac_papdcal_eps_table_size(pi, core);
...
wlc_phy_do_papd_cal_acphy(phy_info_t *pi)
{
	...
	if (ACMAJORREV_44(pi->pubpi->phy_rev) ||
		ACMAJORREV_51_129(pi->pubpi->phy_rev) || ACMAJORREV_128(pi->pubpi->phy_rev)) {
...
phy_ac_papd_cal_mode0_1(phy_info_t *pi, acphy_papdCalParams_t *calParams,  uint8 *calmode)
{
	...
	if (TINY_RADIO(pi)) {
		...
		if (ACMAJORREV_1(pi->pubpi->phy_rev)) {
			...
			/* Pulsed PAPD PAoff = 512smps, in BW20 1smp = 25ns */
			MOD_PHYREG(pi, PapdIpaOffCorr, papd_calIpaOffCorr, 0x0ff);
			MOD_PHYREGCEE(pi, PapdCalShifts, core, papdLambda_I, 0xa);
			MOD_PHYREGCEE(pi, PapdCalShifts, core, papdLambda_Q, 0xa);
			MOD_PHYREGCEE(pi, EpsilonTableAdjust, core, epsilonScalar, 0x8);
			phy_ac_papd_phy_core_setup_majorrev51_128_129(pi, core);
...
phy_ac_wbcal_eps_stopidx(phy_info_t *pi, uint8 core)
{
	...
	uint32 eps_table[eps_table_size];
	uint8 idx;
	uint8 epsilon_table_ids[] = { ACPHY_TBL_ID_EPSILON0, ACPHY_TBL_ID_EPSILON1,
		ACPHY_TBL_ID_EPSILON2};
                                     ...
                                     bool tia_2nd)
{
	...
	if (tia_2nd && CHSPEC_IS80(pi->radio_chanspec)) {
		tiagain_final = params->tia_mode_5g + ((Pd2W*8 - adc_pow_est_dB) + 20) / 24;
	} else {
	...
	PHY_PAPD(("wl%d %s: cal_tiagain core %d: %d\n", pi->sh->unit, __FUNCTION__,
		core, tiagain_final));
	params->tia_search_5g[core] = tiagain_final;
...
void
phy_ac_papdcal_multiphase(phy_info_t *pi)
{
	wlc_phy_cals_mac_susp_en_other_cr(pi, TRUE);
