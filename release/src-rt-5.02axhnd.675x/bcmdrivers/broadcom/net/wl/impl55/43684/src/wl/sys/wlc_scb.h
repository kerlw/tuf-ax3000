 ...
 *
 * $Id: wlc_scb.h 778655 2019-09-06 11:27:56Z $
 */
...
#define SCB_PKTS_TOT_INFLT_FIFOCNT_VAL(scb) \
	((scb)->pkts_inflt_fifocnt[0] + \
	 (scb)->pkts_inflt_fifocnt[1] + \
	 (scb)->pkts_inflt_fifocnt[2] + \
	 (scb)->pkts_inflt_fifocnt[3] + \
	 (scb)->pkts_inflt_fifocnt[4] + \
	 (scb)->pkts_inflt_fifocnt[5] + \
	 (scb)->pkts_inflt_fifocnt[6] + \
	 (scb)->pkts_inflt_fifocnt[7])
...
#define WLSCB_HISTO_ADD(map, rspec, count) do { \
	...
	const uint8 lidx = (rType == WL_HISTO_RATE_TYPE_LEGACY) ? (rIdx % WL_NUM_LEGACY_RATES) : \
		((WL_NUM_LEGACY_RATES + WL_NUM_VHT_RATES * bIdx) + rIdx); \
	if ((rspec) && bW && lidx < WL_HISTO_MAP1_ARR_LEN) { \
