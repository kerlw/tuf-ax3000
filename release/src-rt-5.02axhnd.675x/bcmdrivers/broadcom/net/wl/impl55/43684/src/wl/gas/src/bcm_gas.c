...
static int add_entry(bcm_gas_t *gas)
{
	...
	while (*curr != 0) {
		if ((memcmp(&(*curr)->peer, &gas->peer,	sizeof((*curr)->peer)) == 0) &&
			((*curr)->channel == gas->channel) && ((*curr)->drv == gas->drv)) {
#ifdef BCMDBG
...
static bcm_gas_t *find_entry(uint16 channel, struct ether_addr *mac, int isIncoming)
{
	bcm_gas_t **head, **curr;
...
int bcm_gas_is_active(uint16 channel, struct ether_addr *mac, uint8 dialogToken, int isIncoming)
{
	bcm_gas_t *gas;

	gas = find_entry(channel, mac, isIncoming);
	...
	wl_event_msg_t *wlEvent, uint8 *data, uint32 length)
{
	...
	case WLC_E_ACTION_FRAME_RX:
	{
		...
		if (bcm_decode_init(&dec, actFrameLen, actFrame) &&
			...
			gas = find_entry(channel, src, isIncoming);
			if (gas != 0 &&
