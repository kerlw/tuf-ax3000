 ...
 *
 * $Id: wlc_rx.c 778665 2019-09-06 13:07:25Z $
 *
 */
#include <wlc_cfg.h>
#include <typedefs.h>
 ...
 * required.
 *
 * @param p[in]  Received packet, PKTDATA() points at SW+HW rxstatus (wlc_d11rxhdr_t)
 */
void BCMFASTPATH
		...
		if (WAPI_WAI_RESTRICT(wlc, bsscfg) && !SCB_AUTHORIZED(scb)) {
			if (ether_type != ETHER_TYPE_WAI &&
				...
				WL_ERROR(("wl%d: non-WAI frame from unauthorized station %s\n",
					pub->unit, bcm_ether_ntoa(&scb->ea, eabuf)));
				toss_reason = WLC_RX_STS_TOSS_NOT_WAI;
				goto toss;
			}
		}
#endif /* BCMWAPI_WAI */
...
wlc_monitor(wlc_info_t *wlc, wlc_d11rxhdr_t *wrxh, void *p, struct wlc_if *wlcif)
{
	...
	sts.noise = phy_noise_avg((wlc_phy_t *)
		WLC_PI_BANDUNIT(wlc, CHSPEC_BANDUNIT(sts.chanspec)));
