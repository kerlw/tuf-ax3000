 ...
 *
 * $Id: wlc_wlfc.c 778753 2019-09-10 06:20:42Z $
 */
	...
	do {
		...
		if (TSYNC_ACTIVE(wlc->pub) && txs) {
			if (pkt_first) {
				...
				wl_timesync_get_tx_timestamp(wlc->wl, p, &clk_low, &clk_high);
				pkt_first = FALSE;
			} else {
...
#ifdef BCM_DHDHDR
		bzero(PKTFRAGFCTLV(wlc->osh, p), wlhdrlen);
#else
...
wlc_flow_ring_get_scb_handle(wlc_info_t *wlc, struct wlc_if *wlcif, uint8 *da)
{
	...
	if (BSSCFG_SLOTTED_BSS(cfg)) {
		/* By Default NAN flowring has to be in closed state.
		 * These will be opened based on msch slot clbks.
		 * If we just miss the msch sched slot start, then flowring
		 * will remain closed till the next slot start for the very first time
		 * after flowring create.
		 */
		ret |= SCBHANDLE_PS_STATE_MASK;
		/* For P-NDL of NAN, SCBHANDLE_INFORM_PKTPEND_MASK might be needed. */
		return ret;
	}
	...
	uint16 flowid, uint8 tid)
{
	...
	for (tx_q = wlc->tx_queues; tx_q != NULL; tx_q = tx_q->next) {
		while ((pkt = cpktq_deq(&tx_q->cpktq, &prec))) {
		...
		while ((pkt = pktq_deq(&tmp_q, &prec))) {
			cpktq_penq(&tx_q->cpktq, prec, pkt);
		}
	}
}
...
wlc_scb_calc_weight(uint32 pktlen_bytes, uint32 rate)
{
#define SCALE 100000
	uint32 rate_KBps;
	uint32 weight;

	/* the input rate unit is in
	...
	uint8 *da, uint8 fl)
{
	...
	if (scb != NULL && scb->bsscfg != NULL &&
		...
		RAVG_INIT(TXPKTLEN_RAVG(atfd, fl), (ETHER_MAX_DATA/2), RAVG_EXP_PKT);
		pktlen_avg = RAVG_AVG(TXPKTLEN_RAVG(atfd, fl));
		...
		if (rspec > 0) {
			uint32 weight = wlc_scb_calc_weight(pktlen_avg,
				RSPEC2KBPS(rspec));
			RAVG_INIT(WEIGHT_RAVG(atfd, fl), weight, RAVG_EXP_WGT);
			weight_avg = RAVG_AVG(WEIGHT_RAVG(atfd, fl));
		}
	}
	return weight_avg;
}
...
wlc_scb_upd_all_flr_weight(wlc_info_t *wlc, struct scb *scb)
{
	if (BCMPCIEDEV_ENAB()) {
		...
		for (fl = 0; fl < FLOWRING_PER_SCB_MAX; fl++) {
			avg_weight = RAVG_AVG(WEIGHT_RAVG(atfd, fl));
			if (avg_weight > 0)
...
wlc_ravg_add_weight(wlc_atfd_t *atfd, int fl, ratespec_t rspec)
{
	...
	if (BCMPCIEDEV_ENAB()) {
		...
		avg_pktlen = RAVG_AVG(TXPKTLEN_RAVG(atfd, fl));

		/* calculating the weight based on avg packet length and rate spec */
		weight = wlc_scb_calc_weight(avg_pktlen, RSPEC2KBPS(rspec));
#ifdef WLTAF
...
#endif // endif
		/* adding weight into the moving average buffer */
		RAVG_ADD(WEIGHT_RAVG(atfd, fl), weight);
	}
}
...
uint32 wlc_suppress_recent_for_fragmentation(wlc_info_t *wlc, void *sdu, uint nfrags)
{
	...
	while ((p = pktq_deq(&tmp_q, &prec)))
		cpktq_penq_head(&wlc->active_queue->cpktq, prec, p);
	return i;
	...
	uint8** tcb_state, uint16* cfp_flowid)
{
	...
	switch (op) {
		case CFP_FLOWID_LINK :
			/* Link both the flows */
			ret = wlc_scb_cfp_tcb_link(wlc, ringid, tid, scb, tcb_state, cfp_flowid);
			break;
		default:
			return BCME_ERROR;
			break;
	...
	uint8 *da, uint8 tid)
{
	...
	case FLOW_RING_CREATE :
		...
		if (BSSCFG_SLOTTED_BSS(bsscfg)) {
			wlc_slotted_bss_flow_ring_update(wlc->sbi, wlcif, flowid, da, TRUE);
		}
#endif /* WLSLOTTED_BSS */
		break;
...
#ifdef WLSLOTTED_BSS
		if (BSSCFG_SLOTTED_BSS(bsscfg)) {
			wlc_slotted_bss_flow_ring_update(wlc->sbi, wlcif, flowid, da, FALSE);
		}
#endif /* WLSLOTTED_BSS */
	...
	case FLOW_RING_FLUSH :
		...
		if (RSDB_ENAB(wlc->pub) && bsscfg) {
			if ((bsscfg->flags2 & WLC_BSSCFG_FL2_RSDB_LINKED)) {
				wlc_bsscfg_t *ln_cfg;
				/* Flush for linked cfg as well */
				ln_cfg = wlc_rsdb_bsscfg_get_linked_cfg(wlc->rsdbinfo, bsscfg);
				if (ln_cfg) {
					wlc_flush_flowring_pkts(ln_cfg->wlc, ln_cfg->wlcif,
						da, flowid, tid);
				}
			}
		}
	...
	case FLOW_RING_FLUSH_TXFIFO:
		{
			...
			if (RSDB_ENAB(wlc->pub) && bsscfg) {
				if ((bsscfg->flags2 & WLC_BSSCFG_FL2_RSDB_LINKED)) {
					wlc_bsscfg_t *ln_cfg;
					/* Flush for linked cfg as well */
					ln_cfg = wlc_rsdb_bsscfg_get_linked_cfg(wlc->rsdbinfo,
							bsscfg);
					if (ln_cfg) {
						if (!ln_cfg->wlc->pub->up) {
							WL_ERROR(("wl%d: not up, can't flush:%d\n",
								ln_cfg->wlc->pub->unit, flowid));
							break;
						}
						ln_cfg->wlc->fifoflush_id = flowid;
						wlc_sync_txfifo_all(ln_cfg->wlc, ln_cfg->wlcif->qi,
							FLUSHFIFO_FLUSHID);
					}
				}
			}
	...
	case FLOW_RING_UPD_PRIOMAP:
		...
		/* XXX: Hopefully dhd will not be configured to use
		 * XXX: 8 flow-rings for PCIE FD router build.
		 * XXX: Router supports only 4 flow-rings per STA.
		 */
...
wlc_wlfc_scb_psq_chk(wlc_info_t *wlc, struct scb *scb, uint16 fc, struct pktq *pktq)
{
	...
	psq_empty = (psq_len == 0) ? TRUE : FALSE;
	...
	if ((psq_len < wlfc->wlfc_vqdepth) && SCB_PROPTXTSTATUS_TIM(scbi)) {
		...
		if (psq_empty && SCB_PROPTXTSTATUS_TIM(scbi)) {
...
wlc_wlfc_send_if_event(void *ctx)
{
	...
	if (PROP_TXSTATUS_ENAB(wlc->pub) ||
		...
		FALSE) {
			...
			/* WLC_BSSCFG_FL2_RSDB_NOT_ACTIVE is set for the inactive primary cfg and
			 * the check is added here to prevent double events on the primary cfg up.
			 */
			if (!(bsscfg->flags2 & WLC_BSSCFG_FL2_RSDB_NOT_ACTIVE)) {
...
#ifdef WLC_TSYNC
	if (TSYNC_ACTIVE(wlc->pub) && txs) {
		uint32 wlhinfo = WLPKTTAG(pkt)->wl_hdr_information;
		...
		if (WL_TXSTATUS_GET_FLAGS(wlhinfo) & WLFC_PKTFLAG_PKTFROMHOST) {
			...
			wlc_tsync_update_ts(wlc->tsync, pkt, clk_tsf, clk_avb, 1);
		}
	}
#endif // endif
