...
#if defined(BCMDBG) || defined(BCMDBG_DUMP) || defined(DUMP_RATELINKMEM)
	...
	struct bcmstrbuf *b)
{
	...
	bcm_bprintf(b, "\tBssIdx: 0x%x, BssColor: 0x%x, StaID: 0x%x\n", link_entry->BssIdx,
		link_entry->BssColor_valid, ltoh16(link_entry->StaID_IsAP));
	...
	void *rs, uint16 flags)
{
	...
	if (SCB_MARKED_DEL(scb) || SCB_DEL_IN_PROGRESS(scb)) {
		WL_ERROR(("wl%d: %s, %p, %p, STA: "MACF" Skip updating ratemem that"
	...
	bool clr_txbf_stats)
{
	...
	if (SCB_MARKED_DEL(scb) || SCB_DEL_IN_PROGRESS(scb)) {
		WL_ERROR(("wl%d: %s, %p, %p, STA: "MACF" Skip updating linkmem that"
			" is or will be deleted\n", wlc->pub->unit, __FUNCTION__, scb,
			scbh, ETHER_TO_MACF(scb->ea)));
		return BCME_ERROR;
	}
	...
	/* If clr_txbf_stats=1, Ucode handles the state change through macreq
	 * and no need to update the ucode internal linkmem data
	 */
	if (size == WLC_RLM_LINK_ENTRY_SIZE_TXBF && clr_txbf_stats) {
		size = WLC_RLM_LINK_ENTRY_SIZE_SW;
	}
