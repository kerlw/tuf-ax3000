 ...
 *
 * $Id: wlc_sta.c 778492 2019-09-03 22:21:48Z $
 */
...
wlc_sta_return_home_channel(wlc_bsscfg_t *cfg)
{
	...
	if (PROP_TXSTATUS_ENAB(wlc->pub)) {
#if defined(WLMCHAN)
		bool if_state_upd_open = (!mboolisset(cfg->pm->PMblocked,
...
#ifdef WLSCAN_SUMEVT
	if (SCAN_IN_PROGRESS(wlc->scan)&& SCAN_SUMEVT_ENAB(wlc->pub)) {
		wlc_scan_summary_chan_stats_op(wlc->scan, TRUE, SCAN_SUMM_OPS_STATS_UPDATE);
	}
#endif /* WLSCAN_SUMEVT */
...
#ifdef WLSCAN_SUMEVT
	if (SCAN_IN_PROGRESS(wlc->scan)&& SCAN_SUMEVT_ENAB(wlc->pub)) {
		wlc_scan_summary_chan_stats_op(wlc->scan, TRUE, SCAN_SUMM_OPS_SEND);
	}
#endif /* WLSCAN_SUMEVT */
}
	...
	switch (notif_data->type) {
		case CFG_CLONE_END: {
			if (cfg->ibss_up_pending) {
				...
				wlc_join_start_ibss(to_wlc, cfg, cfg->target_bss->bss_type);
				cfg->ibss_up_pending = FALSE;
			} else if ((cfg->assoc->type == AS_ASSOCIATION ||
				cfg->assoc->type == AS_ROAM)) {
				wlc_bss_info_t *bi = wlc_assoc_get_join_target(cfg->wlc, 0);
				WL_INFORM(("wl%d.%d: sta_rsdb_clone_handler INFRASTAJOIN\n",
						WLCWLUNIT(to_wlc), WLC_BSSCFG_IDX(cfg)));
				if (bi) {
					wlc_assoc_set_as_cfg(to_wlc, cfg);
					if (!to_wlc->bandlocked) {
						int other_bandunit =
							(CHSPEC_BANDUNIT(bi->chanspec) ==
					...
					wlc_rsdb_init_max_rateset(to_wlc, bi);
					wlc_rsdb_join_prep_wlc(to_wlc, cfg, cfg->SSID,
						cfg->SSID_len, cfg->scan_params,
						cfg->assoc_params,
						cfg->assoc_params_len);
					...
					wlc_rsdb_init_max_rateset(to_wlc, cfg->current_bss);
