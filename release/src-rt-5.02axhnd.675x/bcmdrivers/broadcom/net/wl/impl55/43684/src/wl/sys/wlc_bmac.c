 ...
 *
 * $Id: wlc_bmac.c 778791 2019-09-10 14:48:03Z $
 */
		...
		if (((wlc_hw->cons_seq[other_idx].sssn == wlc_hw->sssn) ||
			...
			if ((curr == NULL) || (STSBUF_SEQ(curr) != rxh_seq)) {
				...
				if (buf != NULL) {
					...
					WL_ERROR(("%s\n", b.origbuf));
#if defined(WLC_OFFLOADS_RXSTS) && (defined(BCMDBG) || defined(BCMDBG_DUMP))
					if (STS_RX_OFFLOAD_ENAB(wlc->pub)) {
						bcm_binit(&b, buf, DUMP_SIZE);
						wlc_offload_wl_dump(wlc_hw->wlc->offl, &b);
						WL_PRINT(("%s", buf));
					}
#endif /* WLC_OFFLOADS_RXSTS && (BCMDBG || BCMDBG_DUMP) */
...
wlc_bmac_revinfo_get(wlc_hw_info_t *wlc_hw, wlc_bmac_revinfo_t *revinfo)
{
	...
	FOREACH_WLC_HW_BAND(wlc_hw, bandunit) {
		wlc_hwband_t *band;

		band = wlc_hw->bandstate[bandunit];
		revinfo->band[bandunit].bandunit = band->bandunit;
		revinfo->band[bandunit].bandtype = band->bandtype;
		revinfo->band[bandunit].phytype = band->phytype;
		revinfo->band[bandunit].phyrev = band->phyrev;
		revinfo->band[bandunit].phy_minor_rev = band->phy_minor_rev;
		revinfo->band[bandunit].radioid = band->radioid;
		revinfo->band[bandunit].radiorev = band->radiorev;
		revinfo->band[bandunit].anarev = 0;
			...
			if (BCM_DMA_CT_ENAB(wlc)) {
				...
				for (i = TX_FIFO_MU_START; i < WLC_HW_NFIFO_TOTAL(wlc); i++) {
					...
					if (D11REV_IS(wlc_hw->corerev, 65)) {
						vo_hfifo = wlc_fifo_ac_matching(wlc->fifo, i,
								AC_VO);
					}
...
BCMATTACHFN(wlc_bmac_register_iovt_all)(wlc_hw_info_t *hw, wlc_iocv_info_t *ii)
{
	...
	FOREACH_WLC_HW_BAND(hw, bandunit) {
	...
	bool piomode, osl_t *osh, volatile void *regsva, uint bustype, void *btparam, uint macunit)
{
	...
	if ((wlc_hw->deviceid == BCM6362_D11N_ID) ||
	    ...
	    (wlc_hw->deviceid == BCM6710_D11AX_ID) ||
	    (wlc_hw->deviceid == BCM6715_CHIP_ID) ||
	    (wlc_hw->deviceid == BCM6715_D11AX_ID)) {
		/* Dualband boards */
		wlc_hw->_nbands = 2;
	} else
#endif /* DBAND */
	...
	/* initialize software state for each core and band */
	for (j = 0; j < NBANDS_HW(wlc_hw); j++) { /* Can't use FOREACH_WLC_HW_BAND at this stage */
		/*
...
{
	return (D11REV_IS(corerev, 128) || D11REV_IS(corerev, 129) || D11REV_IS(corerev, 132));
}
#endif /* WL_PSMR1 */
...
BCMINITFN(wlc_gpio_init)(wlc_hw_info_t *wlc_hw)
{
	...
	if (WLANTSEL_ENAB(wlc)) {
		if (wlc_hw->antsel_type == ANTSEL_2x3 || wlc_hw->antsel_type == ANTSEL_1x2_CORE1 ||
			...
			* So set them high
			*/
			OR_REG(osh, D11_PSM_GPIOEN(wlc_hw),
			    (uint16)(BOARD_GPIO_12 | BOARD_GPIO_13));
			OR_REG(osh, D11_PSM_GPIOOUT(wlc_hw),
			    (uint16)(BOARD_GPIO_12 | BOARD_GPIO_13));
...
/* the "#else" below is intentional */
	...
	if (D11REV_IS(wlc_hw->corerev, 132)) {
		...
		WL_ERROR(("%s: wl%d: Loading 132 MU ucode\n",
		          __FUNCTION__, wlc_hw->unit));
...
wlc_write_inits(wlc_hw_info_t *wlc_hw, const d11init_t *inits)
{
	...
	for (i = 0; inits[i].addr != 0xffff; i++) {
		...
		if (inits[i].size == 2)
			W_REG(osh, (uint16*)(uintptr)(base+inits[i].addr), (uint16)(inits[i].value +
			offset_val));
...
#if defined(BCMHWA) && defined(HWA_TXSTAT_BUILD)
	if (hwa_dev) {
		hwa_txstat_reclaim(hwa_dev, 0, FALSE);
	}
#else
	bool fatal = FALSE;
...
#if defined(BCMHWA) && defined(HWA_TXSTAT_BUILD)
		if (hwa_dev) {
			hwa_txstat_reclaim(hwa_dev, 0, TRUE);
		}
...
#endif /* BCMDBG || BCMDBG_DUMP || WLTEST || BCMDBG_PHYDUMP */
...
wlc_bmac_dump_phy(wlc_hw_info_t *wlc_hw, const char *name, struct bcmstrbuf *b)
{
...
#endif // endif
	{
		FOREACH_WLC_HW_BAND(wlc_hw, bandunit) {
			wlc_hwband_t *band;
			band = wlc_hw->bandstate[bandunit];
			ret = phy_dbg_dump((phy_info_t *)band->pi, name, b);
		}
	}
...
#if !defined(DONGLEBUILD) && (defined(BCMDBG) || defined(BCMDBG_DUMP) || \
	defined(WLTEST))
...
#if defined(BCMHWA) && (defined(BCMDBG) || defined(BCMDBG_DUMP))
	if (hwa_dev) {
		wlc_bmac_add_dump_fn(wlc_hw, "hwa", (bmac_dump_fn_t)hwa_wl_dump, (void *)hwa_dev);
	}
#endif /* BCMHWA && (BCMDBG || BCMDBG_DUMP) */
...
#endif /* BCMDBG || BCMDBG_DUMP || defined(BCMDBG_PHYDUMP) || WLTEST */
	...
	bool is_sync, bool is_sync_unblk, void *p[], int npkts)
{
	...
	if ((cmd == WL_PKTENG_PER_TX_START) || (cmd == WL_MUPKTENG_PER_TX_START) ||
		(cmd == WL_PKTENG_PER_RU_TX_START) || (cmd == WL_PKTENG_PER_TX_HETB_START)) {
...
wlc_bmac_bmc_dyn_reinit(wlc_hw_info_t *wlc_hw, uint8 bufsize_in_256_blocks)
{
	...
	uint32 fifo_sz, txfifo_sz, bufsize_tx, bufsize_pertxtid, rxfifo_sz;
	uint16 bufsizeptxid_percore;
	uint32 bmc_startaddr = D11MAC_BMC_STARTADDR;
	...
	*/
	if ((wlc_bmac_rsdb_cap(wlc_hw)) &&
		...
		for (i = 0; i < num_of_fifo; i++) {
			...
			W_REG(osh, D11_RXMapFifoSize(wlc_hw), (uint16)0x1);

			W_REG(osh, D11_BMCCmd1(wlc_hw),
			    (uint16)(((R_REG(wlc_hw->osh, D11_BMCCmd1(wlc_hw)) & 0x0) |
			...
			W_REG(osh, D11_BMCMaxBuffers(wlc_hw), (uint16)0x800);
			W_REG(osh, D11_BMCMinBuffers(wlc_hw), (uint16)0x4);
			W_REG(osh, D11_BMCCmd1(wlc_hw),
			    (uint16)(((R_REG(wlc_hw->osh, D11_BMCCmd1(wlc_hw))) |
			    (1 << BMCCmd1_Minmaxlden_SHIFT) | (fifo << 1)) |
		...
		for (i = 0; i < num_of_fifo; i++) {
			...
			W_REG(osh, D11_BMCCmd1(wlc_hw),
			    (uint16)((R_REG(wlc_hw->osh, D11_BMCCmd1(wlc_hw))) | (fifo << 1)));
		...
		for (i = 0; i < num_of_fifo; i++) {
			...
			W_REG(osh, D11_BMCMinBuffers(wlc_hw), (uint16)0x0);
			W_REG(osh, D11_BMCMaxBuffers(wlc_hw), (uint16)0x800);
			...
			W_REG(osh, D11_BMCCmd(wlc_hw), (uint16)((R_REG(wlc_hw->osh,
		...
		W_REG(osh, D11_BMCCore1TXAllMaxBuffers(wlc_hw), (uint16)0x800);

		W_REG(osh, D11_BMCCore0TXAllMaxBuffers(wlc_hw), (uint16)0x800);
