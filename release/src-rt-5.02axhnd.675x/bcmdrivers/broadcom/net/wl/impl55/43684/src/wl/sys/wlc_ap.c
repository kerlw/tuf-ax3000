 ...
 *
 * $Id: wlc_ap.c 778783 2019-09-10 12:31:05Z $
 */
...
static const bcm_iovar_t wlc_ap_iovars[] = {
	...
	{"dy_ed_thresh2", IOV_DY_ED_THRESH, 0, 0, IOVT_BOOL, 0},
	{NULL, 0, 0, 0, 0, 0 }
};
...
BCMATTACHFN(wlc_ap_attach)(wlc_info_t *wlc)
{
	...
	appvt->dynamic_ed_thresh_enable = getintvar(pub->vars, "dy_ed_thresh");
...
#ifdef WLRSDB
		(((RSDB_ENAB(wlc->pub)) &&
		(wlc->band->bandtype == CHSPEC_BANDTYPE(target_bss->chanspec))) ||
		(!(RSDB_ENAB(wlc->pub)))) &&
#endif // endif
	    TRUE) {
	...
	if (scb != NULL) {
		...
		if (mac_mode_deny_client && !scb) {
			...
			wlc_bss_mac_event(wlc, bsscfg, WLC_E_AUTH_FAIL, &scb->ea,
				WLC_E_STATUS_SUCCESS, status, auth_alg, body_start,
				body_len_start);
		}
	}
	...
	void *params, uint p_len, void *arg, uint len, uint val_size, struct wlc_if *wlcif)
{
	...
	*ret_int_ptr = (int32)bsscfg->far_sta_rssi;
	...
	case IOV_SVAL(IOV_DY_ED_THRESH):
		...
		break;
	case IOV_GVAL(IOV_DY_ED_THRESH):
...
wlc_ap_aid_max(wlc_ap_info_t *ap, wlc_bsscfg_t *cfg)
{
	...
	if (wlc_mbss_mbssid_active(appvt->wlc)) {
		return (wlc_ap_aidmap2aid(appvt, cfg, (uint16)wlc->pub->tunables->maxscb -
			WLC_MBSS_MAXBSSID) & DOT11_AID_MASK);
	} else
#endif /* TESTBED_AP_11AX */
	return (wlc_ap_aidmap2aid(appvt, cfg, (uint16)wlc->pub->tunables->maxscb) & DOT11_AID_MASK);
}
	...
	bool short_preamble)
{
	...
	wlc_sendauth(bsscfg, &scb->ea, &bsscfg->BSSID, scb, auth_alg, auth_seq, status,
		NULL, short_preamble, wlc_ap_pre_sendauth_cb, scb, NULL, NULL);
