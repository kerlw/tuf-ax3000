 ...
 *
 * $Id: wlc_scb.c 778655 2019-09-06 11:27:56Z $
 */
...
wlc_scb_sec_sz(void *ctx, void *obj)
{
	...
	/* hwrs scbs don't need any secondary cubbies so skip them */
	if (SCB_HWRS(scb)) {
		return 0;
	}
...
wlc_scb_cq_inc(void *p, int prec, uint32 cnt)
{
	...
	if (CFP_ENAB(SCB_WLC(scb)->pub) == TRUE) {
		...
		wlc_cfp_scb_cq_inc(SCB_WLC(scb), scb, PKTPRIO(p), cnt);
	}
#endif // endif
...
#ifdef WLCFP
	if (CFP_ENAB(SCB_WLC(scb)->pub) == TRUE) {
		wlc_cfp_scb_cq_dec(SCB_WLC(scb), scb, PKTPRIO(p), cnt);
	}
#endif // endif
...
bool
wlc_scb_cq_empty(struct scb *scb, uint8 prio)
{
	ASSERT(scb);
#ifdef WLCFP
	if (CFP_ENAB(SCB_WLC(scb)->pub) == TRUE) {
		return wlc_cfp_scb_cq_empty(SCB_WLC(scb), scb, prio);
	}
#endif // endif
	return ((SCB_PKTS_INFLT_CQCNT_VAL(scb, prio) == 0) ? TRUE : FALSE);
}
