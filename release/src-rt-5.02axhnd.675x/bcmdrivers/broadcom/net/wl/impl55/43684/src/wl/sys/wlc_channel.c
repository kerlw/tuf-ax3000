 ...
 *
 * $Id: wlc_channel.c 778783 2019-09-10 12:31:05Z $
 */
	...
	clm_country_t country)
{
	...
	if (BAND_5G(band->bandtype)) {
		clm_band = CLM_BAND_5G;
	} else {
		clm_band = CLM_BAND_2G;
	}
...
wlc_valid_channel2p5_db(wlc_cm_info_t *wlc_cmi, uint val)
{
	...
	FOREACH_WLC_BAND(wlc, bandunit) {
		if (!is_current_band(wlc, bandunit) &&
		    VALID_CHANNEL2P5_IN_BAND(wlc, bandunit, val)) {
...
wlc_valid_channel5_db(wlc_cm_info_t *wlc_cmi, uint val)
{
	...
	FOREACH_WLC_BAND(wlc, bandunit) {
		if (!is_current_band(wlc, bandunit) &&
		    VALID_CHANNEL5_IN_BAND(wlc, bandunit, val)) {
			return TRUE; /* channel valid, but not on currently selected band */
		}
	}
...
wlc_valid_channel10_db(wlc_cm_info_t *wlc_cmi, uint val)
{
	...
	FOREACH_WLC_BAND(wlc, bandunit) {
		if (!is_current_band(wlc, bandunit) &&
		    VALID_CHANNEL10_IN_BAND(wlc, bandunit, val)) {
			return TRUE; /* channel valid, but not on currently selected band */
		}
	}
...
wlc_valid_channel20_db(wlc_cm_info_t *wlc_cmi, uint val)
{
	...
	FOREACH_WLC_BAND(wlc, bandunit) {
		if (!is_current_band(wlc, bandunit) &&
		    VALID_CHANNEL20_IN_BAND(wlc, bandunit, val)) {
			return TRUE; /* channel valid, but not on currently selected band */
		}
	}
...
wlc_valid_chanspec_ext(wlc_cm_info_t *wlc_cmi, chanspec_t chspec, bool dualband)
{
	...
	if (CHANNEL_BANDUNIT(wlc_cmi->wlc, channel) != CHSPEC_BANDUNIT(chspec))
		return FALSE;
	...
	if (CHSPEC_IS20(chspec)) {
		...
		if (!VALID_80CHANSPEC_IN_BAND(wlc, CHSPEC_BANDUNIT(chspec)))
			return FALSE;
		...
		if (CHSPEC_IS8080(chspec)) {
			if (!VALID_8080CHANSPEC_IN_BAND(wlc, CHSPEC_BANDUNIT(chspec)))
				return FALSE;
		}
