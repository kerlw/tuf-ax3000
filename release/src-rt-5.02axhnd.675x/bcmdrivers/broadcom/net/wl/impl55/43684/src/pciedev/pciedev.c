...
#ifdef AWD_EXT_TRAP
static uint8 *awd_handle_trap_ds_bus_pcie(void *arg, trap_t *tr, uint8 *dst, int *dst_maxlen);
#endif /* AWD_EXT_TRAP */
...
sqs_vpkts_dump(struct dngl_bus *pciedev, uint16 cfp_flowid)
{
	...
	while (!dll_end(prio, &pciedev->active_prioring_list)) {
		...
		while (!dll_end(cur, &prioring->active_flowring_list)) {
			...
			if ((cfp_flowid != (uint16)CFP_ALL_FLOWS) &&
...
static int BCMATTACHFN(pciedev_setup_msgbuf_lpool)(struct dngl_bus *pciedev,
	...
	uint8 bufcnt, uint8 item_cnt, uint16 fetch_limit, bool phase_supported,
...
#ifdef ECOUNTERS
static int pciedev_ecounters_ipcstats(uint16 stats_type, void *context,
...
volatile void *regs, uint bustype)
{
	...
	if (BCM43684_CHIP(pciedev->sih->chip)) {
		uint32 d2h_flags = 0U;
		uint32 src = 0U, dst = 0;
		uint8 burst_len = 2; /* 2^(N+4) => 64 byte burst */

		if (BUSTYPE(pciedev->sih->bustype) == SI_BUS) {
#ifdef BCM_DMA_TRANSCOHERENT
			d2h_flags |= BME_CONFIG_FLAG_SRC_COHERENT;
#endif // endif
		}
		d2h_flags |= BME_CONFIG_FLAG_DEST_HOSTMEM;

		for (i = 0; i < 2; i++) { // Two channels
			pciedev->d2h_bme[i] = bme_attach(pciedev->sih, BME_CHANNEL_0 + i);
			if (pciedev->d2h_bme[i])
				bme_config(pciedev->d2h_bme[i], src, dst, d2h_flags, burst_len);
		}
	}
...
/*	debug_params.txavail = pciedev->txavail; */
	...
	*/
...
#ifdef ECOUNTERS
	if (ECOUNTERS_ENAB()) {
		ecounters_register_entity_entry_point(ECOUNTERS_TOP_LEVEL_SW_ENTITY_BUS,
				pciedev_ecounters_entry_point);
		ecounters_register_source(
			WL_IFSTATS_XTLV_BUS_PCIE, ECOUNTERS_TOP_LEVEL_SW_ENTITY_BUS,
			pciedev_ecounters_ipcstats, (void*)pciedev);
	}
#endif /* ECOUNTERS */
	...
	pciedev->m2m_desc = (m2m_desc_t *)MALLOCZ(pciedev->osh,
		sizeof(m2m_desc_t) + (NUM_VEC_PCIE * sizeof(m2m_vec_t)));

	if (pciedev->m2m_desc == NULL)
		goto fail;

#if defined(AWD_EXT_TRAP)
	awd_register_trap_ext_callback(awd_handle_trap_ds_bus_pcie, pciedev);
#endif /* AWD_EXT_TRAP */
...
pciedev_init(struct dngl_bus *pciedev)
{
	...
	if (pciedev->health_check_timer_on == FALSE) {
		...
		pciedev->axi_check_period = PCIEDEV_AXI_CHECK_INTERVAL;
#endif // endif
	}
...
BCMATTACHFN(pciedev_setup_pcie_ipc)(struct dngl_bus *pciedev)
{
	...
	pcie_ipc_rings->max_interfaces = MBSS_MAXSLAVES;
#elif defined(MAXVSLAVEDEVS)
	pcie_ipc_rings->max_interfaces = MAXVSLAVEDEVS;
#else
...
pciedev_link_pcie_ipc(struct dngl_bus *pciedev)
{
	...
	EVENT_LOG(EVENT_LOG_TAG_PCI_INFO, "Link HME success\n");
#endif /* BCMHME */
	...
	EVENT_LOG(EVENT_LOG_TAG_PCI_INFO, "Link dmaindex success\n");
#endif /* PCIE_DMA_INDEX */
	...
	EVENT_LOG(EVENT_LOG_TAG_PCI_INFO, "Link iDMA success\n");
#endif /* BCMPCIE_IDMA */
...
BCMATTACHFN(pciedev_dmaindex_bind_pcie_ipc)(struct dngl_bus *pciedev)
{
	...
	EVENT_LOG(EVENT_LOG_TAG_PCI_DBG, "Bind dmaindex h2d\n");
	...
	EVENT_LOG(EVENT_LOG_TAG_PCI_DBG, "Bind dmaindex d2h\n");
	...
	EVENT_LOG(EVENT_LOG_TAG_PCI_TRACE, "Bind dmaindex success\n");
...
pciedev_dmaindex_link_pcie_ipc(struct dngl_bus *pciedev)
{
	...
	if (!HADDR64_IS_ZERO(pcie_ipc_rings->h2d_wr_haddr64))
	{
		...
		EVENT_LOG(EVENT_LOG_TAG_PCI_DBG, "link dmaindex h2d wr\n");
		PCI_INFORM(("link dmaindex h2d wr" HADDR64_FMT "\n",
			HADDR64_VAL(pciedev->dmaindex_h2d_wr.haddr64)));
	} else {
		EVENT_LOG(EVENT_LOG_TAG_PCI_DBG, "link dmaindex h2d wr failure\n");
	...
	if (!HADDR64_IS_ZERO(pcie_ipc_rings->h2d_rd_haddr64))
	{
		...
		EVENT_LOG(EVENT_LOG_TAG_PCI_DBG, "link dmaindex h2d rd\n");
		PCI_INFORM(("link dmaindex h2d rd" HADDR64_FMT "\n",
			HADDR64_VAL(pciedev->dmaindex_h2d_rd.haddr64)));
	} else {
		EVENT_LOG(EVENT_LOG_TAG_PCI_DBG, "link dmaindex h2d rd failure\n");
	...
	if (!HADDR64_IS_ZERO(pcie_ipc_rings->d2h_wr_haddr64))
	{
		...
		EVENT_LOG(EVENT_LOG_TAG_PCI_DBG, "link dmaindex d2h wr\n");
		PCI_INFORM(("link dmaindex d2h wr" HADDR64_FMT "\n",
			HADDR64_VAL(pciedev->dmaindex_d2h_wr.haddr64)));
	} else {
		EVENT_LOG(EVENT_LOG_TAG_PCI_DBG, "link dmaindex d2h wr failure\n");
	...
	if (!HADDR64_IS_ZERO(pcie_ipc_rings->d2h_rd_haddr64))
	{
		...
		EVENT_LOG(EVENT_LOG_TAG_PCI_DBG, "link dmaindex d2h rd\n");
		PCI_INFORM(("link dmaindex d2h rd" HADDR64_FMT "\n",
			HADDR64_VAL(pciedev->dmaindex_d2h_rd.haddr64)));
	} else {
		EVENT_LOG(EVENT_LOG_TAG_PCI_DBG, "link dmaindex d2h rd failure\n");
	...
	EVENT_LOG(EVENT_LOG_TAG_PCI_TRACE, "Link dmaindex success\n");
...
pciedev_dmaindex_put(struct dngl_bus *pciedev, msgbuf_ring_t *ring)
{
	...
	SETUP_RX_DESC(desc, dmaindex_h2d_rd->haddr64, dmaindex_h2d_rd_size);
	SETUP_RX_DESC(desc, dmaindex_d2h_wr->haddr64, dmaindex_d2h_wr_size);

	/* Program tx descriptors for h2d-r/d2h-w indices */
	SETUP_TX_DESC(desc, dmaindex_h2d_rd->daddr64, dmaindex_h2d_rd_size);
	SETUP_TX_DESC(desc, dmaindex_d2h_wr->daddr64, dmaindex_d2h_wr_size);
...
BCMATTACHFN(pciedev_idma_bind_pcie_ipc)(struct dngl_bus *pciedev)
{
	...
	EVENT_LOG(EVENT_LOG_TAG_PCI_TRACE, "Bind iDMA success\n");
...
pciedev_idma_link_pcie_ipc(struct dngl_bus *pciedev)
{
	...
	/* H2D WR */
	if (HADDR64_IS_ZERO(pcie_ipc_rings->h2d_wr_haddr64)) {
		EVENT_LOG(EVENT_LOG_TAG_PCI_DBG, "link idma h2d wr failure\n");
		return BCME_BADARG;
	}

	/* D2H RD */
	if (HADDR64_IS_ZERO(pcie_ipc_rings->d2h_rd_haddr64)) {
		EVENT_LOG(EVENT_LOG_TAG_PCI_DBG, "link idma d2h rd failure\n");
		return BCME_BADARG;
	}
	...
	while (dmaindex->inited && index < MAX_IDMA_DESC) {
		EVENT_LOG(EVENT_LOG_TAG_PCI_DBG, "link idma index%d "
	...
	EVENT_LOG(EVENT_LOG_TAG_PCI_TRACE, "Link idma success\n");
...
BCMATTACHFN(pciedev_detach)(struct dngl_bus *pciedev)
{
	...
	if (pciedev->m2m_desc)
		MFREE(pciedev->osh, pciedev->m2m_desc,
			sizeof(m2m_desc_t) + (NUM_VEC_PCIE * sizeof(m2m_vec_t)));
...
#if defined(HNDBME) && defined(BCM_BUZZZ) && defined(BCM_BUZZZ_STREAMING_BUILD)
	{
		...
		for (i = 0; i < 2; i++) { // Two channels
			if (pciedev->d2h_bme[i]) {
				bme_detach(pciedev->d2h_bme[i]);
				pciedev->d2h_bme[i] = NULL;
			}
		}
	}
#endif /* HNDBME && BCM_BUZZZ && BCM_BUZZZ_STREAMING_BUILD */
...
BCMATTACHFN(pciedev_setup_common_rings)(struct dngl_bus *pciedev)
{
	...
	if ((ret = pciedev_setup_msgbuf_lpool(pciedev, ring, rstr_h2dctl, ringid,
	        ...
	        FALSE, dmaindex_supported, dmaindex_supported, FALSE)) != BCME_OK)
	{
	...
	if ((ret = pciedev_setup_msgbuf_lpool(pciedev, ring, rstr_h2drx, ringid,
	        ...
	        FALSE, dmaindex_supported, dmaindex_supported, TRUE)) != BCME_OK)
	{
	...
	if ((ret = pciedev_setup_msgbuf_lpool(pciedev, ring, rstr_d2hctl, ringid,
	        8, 1, PCIEDEV_WORK_ITEM_FETCH_LIMIT_UNDEF, d2h_phase_supported,
	...
	if ((ret = pciedev_setup_msgbuf_lpool(pciedev, ring, rstr_d2htx, ringid,
	        ...
	        PCIEDEV_WORK_ITEM_FETCH_LIMIT_UNDEF, d2h_phase_supported,
	        dmaindex_supported, dmaindex_supported, FALSE)) != BCME_OK)
	{
	...
	bool dmaindex_d2h_supported, bool dmaindex_h2d_supported, bool inusepool)
{
	...
	ring->phase_supported = phase_supported;
...
BCMATTACHFN(pciedev_setup_flowrings)(struct dngl_bus *pciedev)
{
	...
	for (i = 0; i < pciedev->max_flowrings; i++) {
		...
		flowring->phase_supported     = FALSE;
	...
	uint16 data_len, uint8 msgtype)
{
	...
	SETUP_RX_DESC(desc, addr, msglen);
...
#define D11_REG_OFF	0x120	/* maccontrol */
	...
	/* BUZZZ control for CR4, CA7 */
		...
		val = sqs_vpkts_dump(pciedev, CFP_ALL_FLOWS);
		return BCME_OK;
#endif /* WLSQS */
#if defined(BCMHME)
...
BCMATTACHFN(pciedev_time_sync_gpio_init)(struct dngl_bus *pciedev)
{
	...
	if (time_sync_gpio == CC_GCI_GPIO_INVALID) {
		EVENT_LOG(EVENT_LOG_TAG_PCI_TRACE,
			"time_sync GPIO not enabled \n");
	}
...
send_resp:
	...
	if (max_mem_alloc == 0) {
		mu.max_flowring_alloc += bytes_allocated;
		hnd_update_mem_alloc_stats(&mu);
...
pciedev_dump_rw_indices(struct dngl_bus * pciedev)
{
	...
	/* loop through active flow rings */
	prio = dll_head_p(&pciedev->active_prioring_list);
	while (!dll_end(prio, &pciedev->active_prioring_list)) {
		prioring_pool_t *prioring = (prioring_pool_t *)prio;
		cur = dll_head_p(&prioring->active_flowring_list);
		...
		PCI_ERROR(("ID:%d RD:%d WR:%d RDp: %d\n", i,
			MSGBUF_RD(flow_ring), MSGBUF_WR(flow_ring), flow_ring->fetch_ptr));
#endif /* EVENT_LOG_COMPILE */
			cur = dll_next_p(cur);
		}
...
{
	return (fn) ? fn(stats_type, context, req, xtlvbuf, cookie, tlv,
			attempted_write_len) : BCME_ERROR;
}
static
int pciedev_ecounters_ipcstats(uint16 stats_type, void *context,
	const ecounters_stats_types_report_req_t *req,
	struct bcm_xtlvbuf *xtlvbuf, uint32 *cookie, const bcm_xtlv_t* tlv,
	uint16 *attempted_write_len)
{
	...
	write_status = bcm_xtlv_put_data(xtlvbuf,
			WL_IFSTATS_XTLV_BUS_PCIE,
			(uint8 *)&pciedev->metrics, sizeof(pcie_bus_metrics_t));

	/* If there is enough size. */
	if (write_status == BCME_NOMEM) {
		*attempted_write_len = sizeof(pcie_bus_metrics_t);
		/* Only one structure to be written. Nothing to change in
		 * cookie as there is no state to be maintained.
		 * Leave it unchanged for next client to use.
		 */
		return BCME_BUFTOOSHORT;
	}
	return BCME_OK;
...
#ifdef AWD_EXT_TRAP
		pciedev->ext_trap_hc = TRUE;
		pciedev->ext_trap_hc_buffer = (uchar *)pcie_hc;
#endif /* AWD_EXT_TRAP */
...
static uint8 *awd_handle_trap_ds_bus_pcie(void *arg, trap_t *tr, uint8 *dst, int *dst_maxlen)
{
	uint8 *new_dst = dst;
	int trap_tag;
	uchar *ext_trap_data = NULL;
	uint32 ext_trap_data_len = 0;
	struct dngl_bus *pciedev = (struct dngl_bus*)arg;

#ifdef HEALTH_CHECK
	if (pciedev->ext_trap_hc) {
		trap_tag = TAG_TRAP_BUS;
		ext_trap_data = pciedev->ext_trap_hc_buffer;
		ext_trap_data_len = HC_PCIEDEV_CONFIG_REGLIST_MAX * sizeof(uint32);
	}
#endif /* HEALTH_CHECK */
...
pciedev_cfp_flow_delink(struct dngl_bus * pciedev, uint16 cfp_flowid)
{
	...
	while (!dll_end(prio, &pciedev->active_prioring_list)) {
		...
		while (!dll_end(cur, &prioring->active_flowring_list)) {
			...
			flow_ring->cfp_flowid = CFP_FLOWID_INVALID;
	...
	uint32 daddr32, uint32 haddr32, uint16 len, uint8 index, uint32 offset)
{
	...
	INIT_M2M_DESC(desc);
	SETUP_RX_DESC(desc, haddr64, len); /* Rx into BUZZZ host memory */
	SETUP_TX_DESC(desc, daddr64, len); /* Tx from BUZZZ dongle memory */
...
pciedev_buzzz_d2h_copy(uint32 channel,
	uint32 daddr32, uint32 haddr32, uint16 len, uint8 index, uint32 offset)
{
	struct dngl_bus *pciedev = dongle_bus;
	ASSERT(dongle_bus != NULL);

#ifdef HNDBME
	ASSERT(channel <= BME_CHANNEL_1);
	if (pciedev->d2h_bme[channel]) {
		len -= 1; /* Max bme dma size: BMEREG_XMTPTR_BUFFCOUNT_MASK */
		return bme_cpy(pciedev->d2h_bme[channel],
		           (const uint8*)daddr32, (uint8*)haddr32, len);
	} else
#endif /* HNDBME */
	{
 ...
 */
int BCM_BUZZZ_NOINSTR_FUNC
pciedev_buzzz_d2h_done(uint32 channel)
{
#ifdef HNDBME
	struct dngl_bus *pciedev = dongle_bus;
	...
	if (pciedev->d2h_bme[channel]) {
		if (bme_completed(pciedev->d2h_bme[channel]) == FALSE) {
#if defined(BCM_BUZZZ_FUNC)
			/* Loop until BME completes - interrupts locked! */
			bme_sync(pciedev->d2h_bme[channel]);
#else
			return 0;
#endif // endif
		}
		return 1; /* BME channel available for use */
	}
#endif /* HNDBME */
