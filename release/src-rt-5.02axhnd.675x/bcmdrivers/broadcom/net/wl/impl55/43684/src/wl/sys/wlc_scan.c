 ...
 *
 * $Id: wlc_scan.c 778769 2019-09-10 09:43:50Z $
 */
...
void wlc_scan_set_chanparams(const chanspec_t* chanspec_list, int *channel_num,
	...
	int passive_time, int scan_type)
{
	...
	*channel_set0_num = *channel_set1_num = 0;
	...
	if (scan_type  == DOT11_SCANTYPE_PASSIVE) {
		wlc_scan_split_channels_per_band_passive(scan_info, chanspec_list, chan_num,
			chanspec_set0, chanspec_set1, channel_set0_num, channel_set1_num,
			parallel_passive);
	} else {
		wlc_scan_split_channels_per_band_active(scan_info, chanspec_list, chan_num,
			chanspec_set0, chanspec_set1, channel_set0_num, channel_set1_num,
	...
	struct ether_addr *sa_override)
{
	 ...
	 * (reset channel_num to zero afterward to force full band scan)
	 */
	if ((channel_num == 1) && (chanspec_list != NULL) &&
	    (CHSPEC_CHANNEL(chanspec_list[0]) == 0)) {
		bandinfo = CHSPEC_IS2G(chanspec_list[0]) ? WLC_BAND_2G : WLC_BAND_5G;
		channel_num = 0;
	}
	...
	if (RSDB_PARALLEL_SCAN_ON(scan_info) && !parallel_scan_disabled) {
		...
		if (bandinfo == WLC_BAND_2G) {
			channel_num5g = 0;
		} else if (bandinfo == WLC_BAND_5G) {
			channel_num2g = 0;
		}
...
#endif /* WLRSDB */
	{
		ret_val = _wlc_scan(wlc_scan_info, bss_type, bssid, nssid, ssid,
			...
			include_cache, scan_flags, cfg, act_cb, act_cb_arg, bandinfo,
...
/** Invoked by wlc_scan() -> _wlc_scan() */
void
wlc_scan_set_chanparams(const chanspec_t* chanspec_list, int *channel_num,
		wlc_info_t *wlc, wlc_scan_info_t *wlc_scan_info,
		uint scan_flags, wlc_bsscfg_t *cfg)
{
	int i;
...
list_contains_invalid_scan_channel(wlc_info_t *wlc,
	const chanspec_t *channels_list, int channel_count)
{
	int i;
	...
	struct ether_addr *sa_override)
{
	...
	if (list_contains_invalid_scan_channel(wlc, chanspec_list, channel_num)) {
		return BCME_BADCHAN;
	}
	...
	wlc_scan_set_chanparams(chanspec_list, &channel_num, wlc, wlc_scan_info,
	...
	int band)
{
	...
	bandunit = CHSPEC_BANDUNIT(chanspec_start);
	for (i = 0; i < SCAN_NBANDS(scan_info); i++) {
		...
		if (band == WLC_BAND_ALL) {
			...
			chanspec_start = CH20MHZ_CHSPEC(1);
		} else
