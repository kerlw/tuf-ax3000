 ...
 *
 * $Id: rte.c 778605 2019-09-05 14:28:15Z $
 */
...
#ifdef AWD_EXT_TRAP
static uint8 *awd_handle_trap_backplane(void *arg, trap_t *tr, uint8 *dst, int *dst_maxlen);
#endif /* AWD_EXT_TRAP */
...
BCMATTACHFN(hnd_debug_info_init)(osl_t *osh)
{
	...
	hnd_debug_info_ptr->event_log_top = event_log_get_top();
#endif // endif
...
BCMATTACHFN(hnd_init)(void)
{
	...
	event_log_init(hnd_osh);
	event_log_config(hnd_osh);
#endif // endif
	...
	hnd_isr_module_init(hnd_osh, hnd_dpc_schedule);
...
#ifdef AWD_EXT_TRAP
	awd_ext_trap_init(hnd_osh);
	awd_register_trap_ext_callback(awd_handle_trap_backplane, (void *)hnd_sih);
	awd_register_trap_ext_callback_late(awd_handle_trap_heap, NULL);
#endif /* AWD_EXT_TRAP */
...
{
	uint8 *new_dst = dst;
	hnd_ext_trap_bp_err_t bpe = { 0 };
	si_t *sih = (si_t *)arg;

	si_wrapper_dump_last_timeout(sih, &bpe.error, &bpe.coreid, &bpe.baseaddr,
			(uchar *)&bpe.ioctrl);
	if (bpe.error)
	{
		new_dst = bcm_write_tlv_safe(TAG_TRAP_BACKPLANE, &bpe, sizeof(bpe), dst,
				*dst_maxlen);
		if (new_dst != dst)
			*dst_maxlen -= (sizeof(bpe) + BCM_TLV_HDR_SIZE);
	}
	return new_dst;
}
#endif /* AWD_EXT_TRAP */
