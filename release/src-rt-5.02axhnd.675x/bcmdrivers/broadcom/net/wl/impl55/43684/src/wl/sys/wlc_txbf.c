 ...
 *
 * $Id: wlc_txbf.c 778686 2019-09-07 00:28:54Z $
 */
...
#if defined(BCMDBG) || defined(BCMDBG_DUMP) || defined(WLTEST) || defined(DUMP_TXBF)
...
scb_txbf_init(void *context, scb_t *scb)
{
	...
	bfi = (txbf_scb_info_t *)MALLOCZ(txbf->osh, sizeof(txbf_scb_info_t));
	if (!bfi)
		return BCME_ERROR;
...
scb_txbf_deinit(void *context, scb_t *scb)
{
	...
	MFREE(txbf->osh, bfi, sizeof(txbf_scb_info_t));
	...
	if (WLC_PHY_AS_80P80(wlc, wlc->chanspec)) {
		...
		FOREACHSCB(wlc->scbstate, &scbiter, scb) {
			...
			if (RATELINKMEM_ENAB(wlc->pub)) {
				wlc_ratelinkmem_upd_lmem_int(wlc, scb,
					TRUE /* clr_txbf_sts=1 in mreq */);
			}
		}
	}
	/* invalid tx header cache */
...
wlc_txbf_init_exp(wlc_txbf_info_t *txbf, scb_t *scb, txbf_scb_info_t *bfi)
{
	...
	val &= ~((NBITMASK(C_ADDR_BFIDX_BSZ)) << C_ADDR_BFIDX_NBIT);
	val |= (NBITVAL(C_ADDR_BFCAP_NBIT) | shm_idx << C_ADDR_BFIDX_NBIT);
...
wlc_txbf_bfe_init(wlc_txbf_info_t *txbf, txbf_scb_info_t *bfi)
{
	...
	wlc_write_shm(wlc, shm_addr(bfi_blk, C_BFI_BFE_MYAID_POS),
...
void wlc_txbf_txchain_upd(wlc_txbf_info_t *txbf)
{
	...
	*/
	...
	for (i = 0; i <= txbf->max_link; i++) {
		...
		if (D11REV_GE(wlc->pub->corerev, 128)) {
			uint8 ndps;
			uint8 bfe_sts = 0, bfr_sts = 0, bfr_ntx = 0;
			bfi_blk = txbf->mx_bfiblk_base +
					(bfi->amt_index * BFI_BLK_SIZE(wlc->pub->corerev));
			bfi->BFIConfig0 &= ~(NBITMASK(C_LNK_BFRNDPS_BSZ) << C_LNK_BFRNDPS_NBIT);
			bfr_ntx = (uint8) WLC_BITSCNT(wlc->stf->txchain) - 1;
...
wlc_txbf_info_dump(wlc_txbf_info_t *txbf, bcmstrbuf_t *b)
{
	...
	for (i = 0; i < TXBF_MUTXV_MAX; i++) {
		...
		bcm_bprintf(b, "%d: "MACF"(aid 0x%x) CQI\n", txvidx,
			ETHER_TO_MACF(scb->ea), (scb->aid & DOT11_AID_MASK));
	...
	case IOV_SVAL(IOV_TXBF_BFE_CAP):
		...
		if (D11REV_GE(pub->corerev, 128) &&
			...
			if ((int_val & TXBF_HE_CQI_BFE_CAP) &&
				(BSSCFG_STA(wlc->cfg) &&
				HE_DLMU_ENAB(pub))) {
				txbf->bfr_capable |= TXBF_HE_CQI_BFE_CAP;
			}
		}
		break;
