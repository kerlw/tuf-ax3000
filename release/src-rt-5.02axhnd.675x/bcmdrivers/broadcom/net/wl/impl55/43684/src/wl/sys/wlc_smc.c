...
bool wlc_smc_hw_supported(uint corerev)
{
	return (D11REV_IS(corerev, 128) || D11REV_IS(corerev, 129) ||
		D11REV_IS(corerev, 130) || D11REV_IS(corerev, 131));
}
...
BCMINITFN(wlc_smc_download)(wlc_hw_info_t *wlc_hw)
{
	...
	if (wlc_hw->smc_loaded == FALSE) {
		phy_smc_reset((phy_info_t *)wlc_hw->band->pi, FALSE);
		...
		if D11REV_IS(wlc_hw->corerev, 128)
			phy_smc_download((phy_info_t *)wlc_hw->band->pi, d11smc0sz/6, d11smc0);

		if (D11REV_IS(wlc_hw->corerev, 129) || D11REV_IS(wlc_hw->corerev, 130))
			phy_smc_download((phy_info_t *)wlc_hw->band->pi, d11smc1sz/6, d11smc1);

		if (D11REV_IS(wlc_hw->corerev, 131))
			phy_smc_download((phy_info_t *)wlc_hw->band->pi, d11smc2sz/6, d11smc2);
	}
	phy_smc_reset((phy_info_t *)wlc_hw->band->pi, TRUE);
