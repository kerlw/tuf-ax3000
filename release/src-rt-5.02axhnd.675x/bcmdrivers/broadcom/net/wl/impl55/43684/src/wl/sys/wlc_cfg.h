 ...
 *
 * $Id: wlc_cfg.h 778791 2019-09-10 14:48:03Z $
 */
...
#define _IS_SINGLEBAND_6G(device) \
	((device == BCM6710_D11AX6G_ID))
...
#define _IS_SINGLEBAND_5G(device) \
	((device == BCM4360_D11AC5G_ID) || \
	 ...
	 (device == BCM6715_D11AX5G_ID) || \
	 (device == BCM6878_D11AC5G_ID))

#define _IS_SINGLEBAND_2G(device)\
	((device == BCM43217_D11N2G_ID) || \
	 (device == BCM4360_D11AC2G_ID) || \
	 (device == BCM4352_D11AC2G_ID) || \
	 (device == BCM43569_D11AC2G_ID) || \
	 (device == BCM43570_D11AC2G_ID) || \
	 (device == BCM43602_D11AC2G_ID) || \
	 (device == BCM53573_D11AC2G_ID) || \
	 (device == BCM47189_D11AC2G_ID) || \
	 (device == BCM4347_D11AC2G_ID) || \
	 (device == BCM6878_D11AC2G_ID) || \
	 (device == BCM4365_D11AC2G_ID) || \
	 (device == BCM4366_D11AC2G_ID) || \
	 (device == BCM43684_D11AX2G_ID) || \
	 (device == EMBEDDED_2x2AX_DEV2G_ID) || \
	 (device == BCM6715_D11AX2G_ID) || \
	 (device == BCM6710_D11AX2G_ID))
...
int wlc_is_singleband_2g(unsigned int device, unsigned int corecap);
#define IS_SINGLEBAND_2G(device, corecap)	(bool)wlc_is_singleband_2g(device, corecap)
...
#define ACPHY_DEFAULT5		0x00000007	/* Supported acphy revs:
						 *	128	6878
						 *	129	6710a0
						 *	130	6715a0
						 */
