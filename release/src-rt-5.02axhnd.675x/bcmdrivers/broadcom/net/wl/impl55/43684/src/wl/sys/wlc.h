 ...
 *
 * $Id: wlc.h 778655 2019-09-06 11:27:56Z $
 */
 ...
 * entries in the wlc->bandstate[] array (in case of a single band device). Avoids code duplication.
 */
#define FOREACH_WLC_BAND(wlc, bandunit) \
	for ((bandunit) = 0; (bandunit) < MAXBANDS; (bandunit)++) \
		if (wlc_get_band((wlc), (bandunit)) != NULL)
...
struct wlc_info {
	...
	*/
	...
	wlc_filter_ie_info_t *fiei; /* filter_ie module info */
...
extern void *wlc_sendauth(wlc_bsscfg_t *cfg, struct ether_addr *ea, struct ether_addr *bssid,
	...
	int (*pre_send_fn)(wlc_info_t *wlc, wlc_bsscfg_t *cfg, void *data, void *pkt), void *data,
...
#define WLC_RX_CHANNEL(rev, rxh) (CHSPEC_CHANNEL(D11RXHDR_ACCESS_VAL(rxh, rev, RxChan)))
...
#define SIFS(bandunit) (bandunit == WLC_BAND_5G ? APHY_SIFS_TIME : BPHY_SIFS_TIME);
