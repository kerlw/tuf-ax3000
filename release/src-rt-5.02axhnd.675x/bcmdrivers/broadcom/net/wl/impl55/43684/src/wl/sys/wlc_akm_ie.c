 ...
 *
 * $Id: wlc_akm_ie.c 777717 2019-08-07 10:04:19Z $
 */
...
#ifdef WLRSDB
#define AKM_COPY_SIZE	(sizeof(bss_akm_info_t))
static int
wlc_akm_bsscfg_get(void *ctx, wlc_bsscfg_t *cfg, uint8 *data, int *len)
{
	bss_akm_info_t *bai;
	ASSERT(ctx);
	ASSERT(data);
	ASSERT(len);
	bai = BSS_AKM_INFO((wlc_akm_info_t *)ctx, cfg);
	ASSERT(bai);

	if (*len < AKM_COPY_SIZE) {
		*len = AKM_COPY_SIZE;
		return BCME_BUFTOOSHORT;
	}
	*len = AKM_COPY_SIZE;

	memcpy(data, bai, AKM_COPY_SIZE);

	return BCME_OK;
}

static int
wlc_akm_bsscfg_set(void *ctx, wlc_bsscfg_t *cfg, const uint8 *data, int len)
{
	bss_akm_info_t *bai;
	ASSERT(ctx);
	ASSERT(data);
...
#ifdef WLRSDB
	cubby.fn_get = wlc_akm_bsscfg_get;
	cubby.fn_set = wlc_akm_bsscfg_set;
	cubby.config_size = AKM_COPY_SIZE;
#endif /* WLRSDB */
