 ...
 *
 * $Id: wlc_twt.c 778666 2019-09-06 13:21:48Z $
 */
...
wlc_twt_af_info_recv(wlc_twt_info_t *twti, scb_t *scb, uint8 *body, uint body_len, uint16 fc)
{
	...
	if (twt_info & TWT_INFO_RESP_REQ) {
		err = BCME_BADARG;
		WL_ERROR(("wl%d: %s: Response Requested should have been 0\n", twti->wlc->pub->unit,
			__FUNCTION__));
		goto fail;
	}
	if (twt_info & TWT_INFO_NEXT_TWT_REQ) {
		err = BCME_BADARG;
		WL_ERROR(("wl%d: %s: Next TWT Request should have been 0\n", twti->wlc->pub->unit,
			__FUNCTION__));
		goto fail;
	}
	...
	if (twt_info & TWT_INFO_ALL_TWT) {
		...
		indv = wlc_twt_find_fid(scb_twt, flow_id);
		if (!indv) {
			WL_ERROR(("%s No indv: Unexpected Flow ID in TWT Information=%d\n",
				__FUNCTION__, flow_id));
			err = BCME_BADARG;
			goto fail;
		}
	...
	if (next_twt_size) {
		...
		if (next_twt_size == TWT_INFO_NEXTTWT_SIZE_1) {
			if (body_len < S1G_AF_TWT_INFO_OFF + TWT_INFO_SIZE_INFO +
				...
				err = BCME_BUFTOOSHORT;
				WL_ERROR(("wl%d: %s: buffer too short (1) %d\n",
		...
		if (next_twt_size == TWT_INFO_NEXTTWT_SIZE_2) {
			if (body_len < S1G_AF_TWT_INFO_OFF + TWT_INFO_SIZE_INFO +
				...
				err = BCME_BUFTOOSHORT;
				WL_ERROR(("wl%d: %s: buffer too short (2) %d\n",
					twti->wlc->pub->unit, __FUNCTION__, body_len));
				goto fail;
			}
			twt_l = ltoh32_ua(&body[S1G_AF_TWT_INFO_OFF + TWT_INFO_SIZE_INFO]);
		...
		if (next_twt_size == TWT_INFO_NEXTTWT_SIZE_3) {
			if (body_len < S1G_AF_TWT_INFO_OFF + TWT_INFO_SIZE_INFO +
				...
				err = BCME_BUFTOOSHORT;
				WL_ERROR(("wl%d: %s: buffer too short (3) %d\n",
					twti->wlc->pub->unit, __FUNCTION__, body_len));
				goto fail;
			}
			twt_l = ltoh32_ua(&body[S1G_AF_TWT_INFO_OFF + TWT_INFO_SIZE_INFO]);
	...
	for (i = start; i < stop; i++) {
		...
		if (indv->state == WLC_TWT_STATE_ACTIVE) {
			if (resume) {
				if (indv->paused) {
					indv->paused = FALSE;
					/* wlc_twt_sp_resume(twti, scb, indv, twt_h, twt_l); */
				} else {
					if (flow_id != TWT_INFO_ALL_FLOW_ID) {
						WL_ERROR(("%s Flow ID %d not paused\n",
							__FUNCTION__, i));
						err = BCME_BADARG;
						goto fail;
					}
				...
				if (indv->paused) {
					if (flow_id != TWT_INFO_ALL_FLOW_ID) {
						WL_ERROR(("%s Flow ID %d already paused\n",
							__FUNCTION__, i));
						err = BCME_BADARG;
						goto fail;
					}
...
wlc_twt_fill_link_entry(wlc_twt_info_t *twti, scb_t *scb, d11linkmem_entry_t *link_entry)
{
	...
	if (scb_twt == NULL || !scb_twt->indv_active_cnt) {
		WL_TWT(("%s TWT not active for this SCB %p AID %d\n", __FUNCTION__, scb,
			SCB_AID(scb)));
		return;
	}
