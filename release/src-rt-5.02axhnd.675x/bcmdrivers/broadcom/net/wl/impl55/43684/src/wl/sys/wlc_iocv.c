 ...
 *
 * $Id: wlc_iocv.c 778648 2019-09-06 09:44:56Z $
 */
...
BCMATTACHFN(wlc_register_iovt_all)(wlc_info_t *wlc, wlc_iocv_info_t *ii)
{
	...
	FOREACH_WLC_BAND(wlc, bandunit) {
		...
		pi = (phy_info_t *)WLC_PI_BANDUNIT(wlc, bandunit);
		if (pi == NULL)
			continue;
...
void *
BCMATTACHFN(wlc_iovc_obj_registry)(void *high_ctx, uint sz)
{
	wlc_info_t *wlc = (wlc_info_t *)high_ctx;
	void *cmn = NULL;

	cmn = obj_registry_get(wlc->objr, OBJR_IOCV_HIGH_CMN);
	if (cmn == NULL) {
		/* Object not found ! so alloc new object here and set the object */
		if ((cmn = MALLOCZ(wlc->osh, sz)) == NULL) {
			WL_ERROR(("%s: MALLOC failed\n", __FUNCTION__));
			return NULL;
		}
		obj_registry_set(wlc->objr, OBJR_IOCV_HIGH_CMN, cmn);
	}
	/* Hit a reference for this object */
	(void) obj_registry_ref(wlc->objr, OBJR_IOCV_HIGH_CMN);
	return cmn;
}

void *
BCMATTACHFN(wlc_iovc_obj_registry_unref)(void *high_ctx, void *cmn, uint sz)
{
	wlc_info_t *wlc = (wlc_info_t *)high_ctx;

	if (obj_registry_unref(wlc->objr, OBJR_IOCV_HIGH_CMN) == 0) {
		MFREE(wlc->osh, cmn, sz);
		cmn = NULL;
		obj_registry_set(wlc->objr, OBJR_IOCV_HIGH_CMN, cmn);
	}
	return NULL; /* return null whoever requested */
}
	...
	bool set, struct wlc_if *wlcif)
{
	...
	struct wlc_if *wlcif_update;
	const uint8 *iovbuf = NULL;
	uint move = 0;
	bool moved = FALSE;
	uint8 save[4];
	uint8 loop_again = FALSE;
	...
	do
	{
		...
		if ((err = wlc_iocv_proc_iov(wlc_update, tid, actionid, vi, params, p_len,
				arg, len, val_size, set, wlcif_update)) != BCME_OK) {
			goto exit;
		}

#ifdef WLRSDB
		if (RSDB_ENAB(wlc->pub) && (++idx < MAX_RSDB_MAC_NUM)) {
			if (wlc_rsdb_chkiovar(wlc_update, vi, actionid, wlc_idx)) {
				wlc_update = wlc_rsdb_get_other_wlc(wlc_update);
				loop_again = TRUE;
			} else {
				wlcif_update = wlc_rsdb_linked_cfg_chkiovar(wlc_update, vi,
						actionid, wlc_idx, wlcif_update);
				if (wlcif_update != NULL) {
					/* Update wlc of linked cfg */
					wlc_update = wlcif_update->u.bsscfg->wlc;
					loop_again = TRUE;
				}
			}
		}
#endif // endif
	} while (wlc_update && loop_again);
