 ...
 *
 * $Id: rte_priv.h 778441 2019-09-01 10:06:08Z $
 */
...
bool hnd_cpu_gtimer_trap_validation(void);
...
/* ===========DEADMAN ================ */
typedef enum {
	DEADMAN_EVENT_ISR_START = 0,
	DEADMAN_EVENT_ISR_END = 1,
	DEADMAN_EVENT_ISR_POST = 2,
	DEADMAN_EVENT_DPC_START = 3,
	DEADMAN_EVENT_DPC_END = 4
} deadman_event_t;

#if !defined(THREADX_WATCHDOG)
extern void deadman_refresh(uint32 now);
extern void deadman_timeout_enab(bool enable);
extern void deadman_state_machine(deadman_event_t event);
#else
#define deadman_refresh(arg)
#define deadman_timeout_enab(arg)
#define deadman_state_machine(arg)
