...
wlc_musched_write_ack_policy(wlc_muscheduler_info_t *musched)
{
	...
	uval16 = ((musched->ack_policy << MUSCHED_HEMSCH_STP_SHIFT) &
		MUSCHED_HEMSCH_STP_MASK) | musched->dl_schidx;
...
wlc_musched_get_min_dlofdma_users(wlc_muscheduler_info_t *musched,
		uint16 *min_users, uint16 *users_sz)
{
...
wlc_musched_print_ru_stats(musched_ru_stats_t* ru_stats, bcmstrbuf_t *b, bool is_160)
{
	...
	wlc_ampdu_dump_table(b, "    TX", ru_stats->tx_cnt, "   PER", ru_stats->txsucc_cnt,
	...
	if ((!WSEC_ENABLED(bsscfg->wsec) && !(oldstate & ASSOCIATED) &&
		...
		if (ofdma_en) {
			if (!PIO_ENAB_HW(wlc->wlc_hw) && !SCB_A4_DATA(scb) &&
				...
				MUMAX_NSTS_ALLOWED) && (musched->num_dlofdma_users <
				wlc_txcfg_max_clients_get(wlc->txcfg, DLOFDMA))) {
				musched->num_dlofdma_users++;
				ASSERT(musched->num_dlofdma_users <=
...
#ifdef WLTAF
	wlc_taf_scb_state_update(wlc->taf_handle, scb, TAF_PARAM(SCB_DLOFDMA(scb)),
...
wlc_musched_config_ru_alloc_type(wlc_muscheduler_info_t *musched)
{
	...
	if (wlc_musched_get_rualloctype(musched) == MUSCHED_RUALLOC_MINRU) {
		uval16 |= MXHF1_RUMASK;
	} else if (wlc_musched_get_rualloctype(musched) == MUSCHED_RUALLOC_UCODERU) {
		uval16 |= MXHF1_RUALLOC;
	}
	wlc_bmac_mhf(wlc->hw, MXHF1, MXHF1_RUMASK, uval16, WLC_BAND_ALL);
}
