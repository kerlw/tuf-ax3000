 ...
 *
 * $Id: wlc_qoscfg.c 778350 2019-08-29 11:59:58Z $
 */
...
static const bcm_iovar_t wme_iovars[] = {
	{"wme", IOV_WME, IOVF_SET_DOWN|IOVF_OPEN_ALLOW|IOVF_RSDB_SET, 0, IOVT_INT32, 0},
...
#endif /* STA */
	{"wme_dp", IOV_WME_DP, IOVF_SET_DOWN|IOVF_OPEN_ALLOW|IOVF_RSDB_SET, 0, IOVT_UINT8, 0},
	{"wme_prec_queuing", IOV_WME_PREC_QUEUING, IOVF_OPEN_ALLOW|IOVF_RSDB_SET, 0, IOVT_BOOL, 0},
#if defined(WLCNT)
	{"wme_counters", IOV_WME_COUNTERS, IOVF_OPEN_ALLOW, IOVF2_RSDB_CORE_OVERRIDE, IOVT_BUFFER,
	sizeof(wl_wme_cnt_t)},
	{"wme_clear_counters", IOV_WME_CLEAR_COUNTERS, IOVF_OPEN_ALLOW, IOVF2_RSDB_CORE_OVERRIDE,
	IOVT_VOID, 0},
#endif // endif
...
#ifdef WLRSDB
static int wlc_wme_bss_get(void *ctx, wlc_bsscfg_t *cfg, uint8 *data, int *len);
static int wlc_wme_bss_set(void *ctx, wlc_bsscfg_t *cfg, const uint8 *data, int len);
#endif /* WLRSDB */

#define WME_COPY_SIZE sizeof(wlc_wme_t)
...
#ifdef WLRSDB
static int
wlc_wme_bss_get(void *ctx, wlc_bsscfg_t *cfg, uint8 *data, int *len)
{
	wlc_wme_t *cp = (wlc_wme_t *)data;
	ASSERT(cfg != NULL);
	if (len == NULL) {
	...
	if ((data == NULL) || (*len < WME_COPY_SIZE)) {
		WL_ERROR(("wl%d: %s: Buffer too short\n", cfg->wlc->pub->unit, __FUNCTION__));
		*len = WME_COPY_SIZE;
		return BCME_BUFTOOSHORT;
	}

	cp->wme_param_ie = cfg->wme->wme_param_ie;
	if (BSSCFG_AP(cfg))
		cp->wme_param_ie_ad = cfg->wme->wme_param_ie_ad;
	bcopy(cfg->wme->edcf_txop, cp->edcf_txop, sizeof(cp->edcf_txop));
	cp->apsd_trigger_ac = cfg->wme->apsd_trigger_ac;
	cp->apsd_auto_trigger = cfg->wme->apsd_auto_trigger;
	cp->apsd_sta_qosinfo = cfg->wme->apsd_sta_qosinfo;
	cp->wme_apsd = cfg->wme->wme_apsd;
	cp->wme_noack = cfg->wme->wme_noack;
	cp->wme_admctl = cfg->wme->wme_admctl;

	return BCME_OK;
}

static int
wlc_wme_bss_set(void *ctx, wlc_bsscfg_t *cfg, const uint8 *data, int len)
{
	const wlc_wme_t *cp = (const wlc_wme_t *)data;
	ASSERT(cfg != NULL);
	if ((data == NULL) || (len < WME_COPY_SIZE)) {
		WL_ERROR(("wl%d: %s: Buffer too short\n", cfg->wlc->pub->unit, __FUNCTION__));
		return BCME_ERROR;
	...
	cfg->wme->wme_param_ie = cp->wme_param_ie;
	if (BSSCFG_AP(cfg))
		bcopy(cp->wme_param_ie_ad, cfg->wme->wme_param_ie_ad, sizeof(wme_param_ie_t));
	bcopy(cp->edcf_txop, cfg->wme->edcf_txop, sizeof(cp->edcf_txop));
	cfg->wme->apsd_trigger_ac = cp->apsd_trigger_ac;
	cfg->wme->apsd_auto_trigger = cp->apsd_auto_trigger;
	cfg->wme->apsd_sta_qosinfo = cp->apsd_sta_qosinfo;
	cfg->wme->wme_apsd = cp->wme_apsd;
	cfg->wme->wme_noack = cp->wme_noack;
	cfg->wme->wme_admctl = cp->wme_admctl;
	return BCME_OK;
...
#endif // endif
#ifdef WLRSDB
	cubby_params.fn_get = wlc_wme_bss_get;
	cubby_params.fn_set = wlc_wme_bss_set;
	cubby_params.config_size = WME_COPY_SIZE;
#endif // endif
