 ...
 *
 * $Id: wlc_wds.c 778467 2019-09-03 05:52:07Z $
 */
...
#ifdef WL11AX
		/* XXX UL OFDMA is not supported under DWDS mode yet,
		 * after UL OFDMA is ready for DWDS mode, we should remove this WAR.
		 * BCAWLAN-208851, BCAFWUCODE-5381.
		 */
		if (HE_ENAB(wlc->pub) && (bsscfg == wlc_bsscfg_primary(wlc))) {
			if (bsscfg->_dwds == TRUE) {
				WLC_HE_FEATURES_CLR(wlc->pub, WL_HE_FEATURES_ULOMU);
			} else {
...
#define AID2WDSIF(aid) ((aid) & DOT11_AID_MASK)
...
wlc_wds_create(wlc_info_t *wlc, struct scb *scb, uint flags)
{
	...
	if (!(flags & WDS_INFRA_BSS)) {
		...
		scb->wds->wlif = wl_add_if(wlc->wl, scb->wds, AID2WDSIF(scb->aid), &scb->ea);
...
#ifdef WL_HAPD_WDS
	if ((data->oldstate & AUTHORIZED) &&
		!SCB_AUTHORIZED(scb) && SCB_LEGACY_WDS(scb)) {
