 ...
 *
 * $Id: wlc_apps.c 778753 2019-09-10 06:20:42Z $
 */
...
wlc_apps_scb_psinfo_init(void *context, struct scb *scb)
{
	...
	if (BSSCFG_HAS_PSINFO(bsscfg)) {
		*cubby_info = wlc_scb_sec_cubby_alloc(wlc, scb, sizeof(apps_scb_psinfo_t));
		...
		pktq_init(&(*cubby_info)->psq, WLC_PREC_COUNT, PKTQ_LEN_MAX);
		{	/* Init supress ps spktq */
			int prec;
			for (prec = 0; prec < WLC_PREC_COUNT; prec++)
				spktq_init(&(*cubby_info)->supr_psq[prec], PKTQ_LEN_MAX);
		}
#else
	...
	if (on) {
		...
		if (bss_info->ps_nodes == 0) {
			...
			if (aid == bss_info->aid_hi) {
				...
				max_aid = wlc_ap_aid_max(wlc->ap, SCB_BSSCFG(scb));
...
wlc_apps_pvb_update(wlc_info_t *wlc, struct scb *scb)
{
	...
	*/
	...
	/* XXX FIXME: PS friendly CFP task #4. For now, make the count always non-zero
	 * anyway. Should consider SCB queue later.
	 */
	if (!ps_count)
		ps_count = 1;
	...
	if (ps_count > 0 && SCB_PS(scb)) {
		...
		if (bss_info->ps_nodes == 0) {
			...
			if (aid == bss_info->aid_hi) {
				...
				uint16 max_aid = wlc_ap_aid_max(wlc->ap, SCB_BSSCFG(scb));
				/* find the next highest aid value with PS pkts pending */
				for (aid = aid + 1; aid < max_aid; aid++)
					if (isset(bss_info->pvb, aid)) {
						bss_info->aid_lo = aid;
...
void
wlc_apps_twt_exit(wlc_info_t *wlc, scb_t *scb)
{
	struct apps_scb_psinfo *scb_psinfo;
	struct scb_iter scbiter;
	scb_t *scb_tst;
 ...
 * datablock on a per scb is to be handled. Currently only in use for TWT.
 */
void
