 ...
 *
 * $Id: wlc_wnm.c 778593 2019-09-05 09:47:04Z $
 */
	...
	uint8 *body, int body_len, bool unicast_req, dot11_bsstrans_req_t* req)
{
	...
	if (req->reqmode & DOT11_BSSTRANS_REQMODE_PREF_LIST_INCL) {
		...
		retval = wlc_wnm_bsstrans_update_nbrlist(wnm, bsscfg, body, &body_len);
			...
			for (channum = 0; channum < ARRAYSIZE(list); channum++) {
				if (list[channum] == 0) {
					break;
				}
				/* update rcc list */
				rcc_add_chanspec(bsscfg->roam, list[channum]);
			}
	...
	void *params, uint len, wnm_bsscfg_cubby_t *wnm_cfg)
{
	...
	if (btcfg->band != WLC_BAND_2G && btcfg->band != WLC_BAND_5G) {
		return BCME_RANGE;
	}
	...
	void *params, wnm_bsscfg_cubby_t *wnm_cfg)
{
	...
	if (btcfg->band <= WLC_BAND_AUTO || btcfg->band > WLC_BAND_ALL) {
		return BCME_RANGE;
	}
	...
	void *params, uint p_len, void *arg, uint len, uint val_size, struct wlc_if *wlcif)
{
	...
	case IOV_SVAL(IOV_WNM_BSS_SELECT_WEIGHT): {
		...
		if (bwcfg->band <= WLC_BAND_AUTO || bwcfg->band > WLC_BAND_ALL) {
			return BCME_RANGE;
		}
...
/* Derive good AP score incase IMNT, ABRIDGED, PREF_LIST are all set */
static uint32
wlc_wnm_get_default_score(wlc_wnm_info_t *wnm, wlc_bsscfg_t *bsscfg)
{
	uint32 score;
	uint8 band = CHSPEC_IS2G(bsscfg->current_bss->chanspec) ?
		WLC_BAND_2G : WLC_BAND_5G;

	score = wlc_wnm_bsstrans_get_rssi_score(wnm, bsscfg, band, WNM_DEFAULT_RSSI) +
		wlc_wnm_bsstrans_get_cu_score(wnm, bsscfg, band, WNM_DEFAULT_CU);
	return (score);
}
	...
	int8 bcn_rssi, uint32 *score)
{
	...
	band = CHSPEC_IS2G(bi->chanspec) ? WLC_BAND_2G : WLC_BAND_5G;
