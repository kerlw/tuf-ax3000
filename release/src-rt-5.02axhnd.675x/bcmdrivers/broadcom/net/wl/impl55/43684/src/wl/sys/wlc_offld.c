...
BCMATTACHFN(wlc_offload_attach)(wlc_info_t *wlc)
{
	...
	base = (volatile char *)si_switch_core(wlc_offl->sih, M2MDMA_CORE_ID, &saved_core_idx,
		&intr_val);
	ASSERT(base);
	wlc_offl->m2m_regs = (volatile m2md_regs_t*)base;
	...
	wlc_offl->txsts.regs =
		(volatile m2md_bme_status_regs_t *)(base + M2M_OFFSET_BME_TXSTS_REGS);
#endif /* WLC_OFFLOADS_TXSTS */
#ifdef WLC_OFFLOADS_RXSTS
	wlc_offl->phyrxsts.regs =
		(volatile m2md_bme_status_regs_t *)(base + M2M_OFFSET_BME_PHYRXSTS_REGS);
...
wlc_offload_init(wlc_offload_t *wlc_offl, uint8 *va)
{
	...
	if (wlc_offl->txsts.regs != NULL) {
		...
		OR_REG(wlc_offl->osh, &wlc_offl->m2m_regs->intcontrol, M2M_INTCONTROL_MASK);
	}
#endif /* WLC_OFFLOADS_TXSTS */
#ifdef WLC_OFFLOADS_RXSTS
...
wlc_offload_deinit(wlc_offload_t *wlc_offl)
{
	...
	if (wlc_offl->txsts.regs != NULL) { /* channel supports phyrx/tx status offloading */
		AND_REG(wlc_offl->osh, &wlc_offl->m2m_regs->intcontrol, ~M2M_INTCONTROL_MASK);
...
{
	volatile m2md_regs_t *regs = wlc_offl->m2m_regs;
	uint32 active_irqs;
