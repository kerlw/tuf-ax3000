 ...
 *
 * $Id: wlc_ampdu.c 778769 2019-09-10 09:43:50Z $
 */
...
BCMATTACHFN(wlc_ampdu_tx_cfg_init)(wlc_info_t *wlc, ampdu_tx_config_t *ampdu_tx_cfg)
{
	...
	if (D11REV_IS(wlc->pub->corerev, 129) && (!wlc->pub->sih->otpflag)) {
		for (i = 0; i < AMPDU_MAX_SCB_TID; i++) {
			if (i != PRIO_8021D_BE)
				ampdu_tx_cfg->ini_enable[i] = FALSE;
		}
	}
...
#ifdef WLAMPDU_PRECEDENCE
				const bool en_pair = TRUE;
#else
				const bool en_pair = FALSE;
#endif // endif
				prec_cnt = wlc_txq_prec_log_enable(wlc, q, (uint32)prec, en_pair);
			}
...
free_and_next:
	...
	if (fix_rate && !txs_mu) {
		...
		if (actq_cnt) {
			...
			WLCNTADD(actq_cnt->txrate_main, (uint64)txrate_main);
			WLCNTADD(actq_cnt->txrate_succ, (uint64)txrate_succ);
			WLCNTADD(actq_cnt->txbw_succ, (uint64)txbw_succ);
...
#if defined(BCMDBG) || defined(BCMDBG_DUMP) || defined(DL_RU_STATS_DUMP)
				wlc_musched_upd_ru_stats(wlc->musched, scb, txs);
#endif /* defined(BCMDBG) || defined(BCMDBG_DUMP) || defined(BCMDBG_DUMP) */
		...
		if (!txs_mu && succ_count && (prec_cnt || actq_cnt ||
			...
			txbw_succ = mcs_bw[0] * rs_txs.txsucc_cnt[0] +
			              mcs_bw[1] * rs_txs.txsucc_cnt[1] +
			              mcs_bw[2] * rs_txs.txsucc_cnt[2] +
			              mcs_bw[3] * rs_txs.txsucc_cnt[3];
			...
			if (actq_cnt) {
				...
				WLCNTADD(actq_cnt->txrate_succ, (uint64)txrate_succ);
				WLCNTADD(actq_cnt->txbw_succ, (uint64)txbw_succ);
				for (_mfbr_idx = 0; _mfbr_idx < RATESEL_MFBR_NUM; _mfbr_idx++) {
					if (rs_txs.txsucc_cnt[_mfbr_idx] == 0) {
						continue;
					}
					WLCNTADD(actq_cnt->nss[mcs_nss[_mfbr_idx]],
...
wlc_ampdu_tx_dump(ampdu_tx_info_t *ampdu_tx, struct bcmstrbuf *b)
{
	...
	wlc_ampdu_dump_table(b, "TX HT", amdbg->txmcs,
		"HT PER", amdbg->txmcs_succ, AMPDU_HT_MCS_LAST_EL, MAX_HT_RATES, TABLE_MCS);

#ifdef WL11AC
	if (AMPDU_AQM_ENAB(pub)) {
		wlc_ampdu_dump_table(b, "TX VHT", amdbg->txvht,
	...
	for (gi = 0; gi < AMPDU_MAX_HE_GI; gi++) {
		...
		if (AMPDU_AQM_ENAB(pub)) {
			wlc_ampdu_dump_table(b, name, amdbg->txhe[gi],
	...
	wlc_ampdu_dump_table(b, "MPDUdens", amdbg->mpdu_histogram,
		NULL, NULL, AMPDU_MAX_MPDU, 8, TABLE_NONAME);

#ifdef AMPDU_NON_AQM
	if (AMPDU_HOST_ENAB(pub)) {
		wlc_ampdu_dump_table(b, "Retry", amdbg->retry_histogram,
			NULL, NULL, AMPDU_MAX_MPDU, 8, TABLE_NONAME);

		wlc_ampdu_dump_table(b, "Till End", amdbg->end_histogram,
			NULL, NULL, AMPDU_MAX_MPDU, 8, TABLE_NONAME);
	}
#endif /* AMPDU_NON_AQM */

	if (WLC_SGI_CAP_PHY(wlc)) {
		wlc_ampdu_dump_table(b, "TX HT SGI", amdbg->txmcssgi,
			NULL, NULL, AMPDU_HT_MCS_LAST_EL, MAX_HT_RATES, TABLE_MCS);
#ifdef WL11AC
		wlc_ampdu_dump_table(b, "TX VHT SGI", amdbg->txvhtsgi,
			NULL, NULL, AMPDU_MAX_VHT, MAX_VHT_RATES, TABLE_MCS);
#endif /* WL11AC */

		if (WLC_STBC_CAP_PHY(wlc)) {
			wlc_ampdu_dump_table(b, "TX HT STBC", amdbg->txmcsstbc,
				NULL, NULL, AMPDU_HT_MCS_LAST_EL, MAX_HT_RATES, TABLE_MCS);
#ifdef WL11AC
			wlc_ampdu_dump_table(b, "TX VHT STBC", amdbg->txvhtstbc,
				NULL, NULL, AMPDU_MAX_VHT, MAX_VHT_RATES, TABLE_MCS);
#endif /* WL11AC */
#ifdef WL11AX
			wlc_ampdu_dump_table(b, "TX HE STBC", amdbg->txhestbc,
				NULL, NULL, AMPDU_MAX_HE, MAX_HE_RATES, TABLE_MCS);
#endif /* WL11AX */
		}
	}
	...
	taf_scheduler_public_t* taf)
{
	...
	if (q->pktqlog) {
		...
		if (prec_cnt == NULL && (q->pktqlog->_prec_log & PKTQ_LOG_AUTO)) {
			prec_cnt = wlc_txq_prec_log_enable(wlc, q, (uint32)prec, FALSE);
		}
	...
	scb_cfp_t *scb_cfp)
{
	...
	if (pq->pktqlog) {
		...
		if (prec_cnt == NULL && (pq->pktqlog->_prec_log & PKTQ_LOG_AUTO)) {
			prec_cnt = wlc_txq_prec_log_enable(wlc, pq, (uint32)tid, FALSE);
		}
	}
#endif /* PKTQ_LOG */
...
/** 802.1D Priority Order */
const uint8 prio_order[] = {
	PRIO_8021D_NC,	/* NC - Network Control */
	PRIO_8021D_VO,	/* Vo - Voice */
	PRIO_8021D_VI,	/* Vi - Video */
	PRIO_8021D_CL,	/* CL - Controlled Load */
	PRIO_8021D_EE,	/* EE - Excellent-effort */
	PRIO_8021D_BE,	/* BE - Best-effort */
	PRIO_8021D_BK,	/* BK - Background */
	PRIO_8021D_NONE	/* None = - */
};
...
#ifdef PKTQ_LOG
	if (pktq->pktqlog) {
		pktq_counters_t *prec_cnt = pktq->pktqlog->_prec_cnt[prio];
	...
	void *pktlist_head, void *pktlist_tail, uint16 pkt_count, scb_cfp_t *scb_cfp)
{
	...
	if (!use_taf) {
		...
		/* Check CFP state before release */
		if (CFP_ENAB(wlc->pub) &&
			wlc_cfp_tcb_is_ini_valid(wlc, scb, tid, NULL)) {
...
#if defined(WLAMPDU_PRECEDENCE)
	prec = WLAMPDU_PREC_TID2PREC(pkt, tid);
#endif /* WLAMPDU_PRECEDENCE */
		...
		if (CFP_ENAB(wlc->pub) && wlc_cfp_tcb_is_ini_valid(wlc, scb, tid, &scb_cfp)) {
			...
			if (wlc_cfp_tcb_is_EST(wlc, scb, tid, NULL)) {
				cfp_pkts_released += wlc_cfp_ampdu_release(scb_cfp, scb_ampdu, ini,
					n_mpdus_to_release - 1, WLC_AMSDU_IN_AMPDU(wlc, scb, tid),
					taf);
	...
	uint8 index, bool clear, uint32 timelo, uint32 timehi, uint32 tid_mask, uint32 flags)
{
	...
	tid_mask |= PKTQ_LOG_DEVELOPMENT_VERSION;
