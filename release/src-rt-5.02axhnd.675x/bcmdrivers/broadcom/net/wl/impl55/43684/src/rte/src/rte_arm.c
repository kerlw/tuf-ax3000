 ...
 *
 * $Id: rte_arm.c 778603 2019-09-05 14:20:24Z $
 */
...
#if !defined(THREADX_WATCHDOG)
#ifndef BCM_BOOTLOADER

#define DEADMAN_REFRESH_FACTOR			(3)

/* Deadman default timeout set to 3 seconds */
#if defined(__ARM_ARCH_7A__) && !defined(BCMSTANDALONE_TEST)
#define DEADMAN_DEFAULT_TO  (3 * hnd_cpu_gtimer_clock(sih))
#else
#define DEADMAN_DEFAULT_TO  (3 * si_cpu_clock(sih))
#endif /* __ARM_ARCH_7A__ */

/* Factor of DEADMAN_TO where timer refresh is done */

typedef enum {
	DEADMAN_STATE_DISABLED = 0,
	DEADMAN_STATE_ENABLED = 1,
	DEADMAN_STATE_POSTED = 2,
	DEADMAN_STATE_DPC_STARTED = 3
} deadman_state_t;
static deadman_state_t deadman_state = DEADMAN_STATE_DISABLED;
static const char BCMATTACHDATA(rstr_deadman_to)[] = "deadman_to";
static uint32 deadman_to = 0;
static uint32 refresh_time_ms = 0, last_refresh = 0;
#endif /* BCM_BOOTLOADER */

/* refresh deadman timer based on current time 'now' in milliseconds */
void
deadman_refresh(uint32 now)
{
#ifndef BCM_BOOTLOADER
	if (refresh_time_ms && ((now - last_refresh) > refresh_time_ms)) {
		last_refresh = now;
		hnd_cpu_deadman_timer(hnd_sih, deadman_to);
	}
#endif /* BCM_BOOTLOADER */
}

/* enable/disable deadman timer */
void
deadman_timeout_enab(bool enable)
{
#ifndef BCM_BOOTLOADER
	if (enable == TRUE) {
		hnd_cpu_deadman_timer(hnd_sih, deadman_to);
...
/* Disable deadman timer if running and we intend to wait on an event in a DPC context */
/* As of now this API is being used only in the ATE runs in which we wait on SR sleep state,
 * for GPIO trigger to happen for infinite time
 */
void
deadman_disable(void)
{
	deadman_state_machine(DEADMAN_EVENT_DPC_START);
	deadman_state_machine(DEADMAN_EVENT_DPC_END);
}

/* Deadman state machine implementation based on input events.
	If event is not handled in current state - no action is required
*/
void
deadman_state_machine(deadman_event_t event)
{
#ifndef BCM_BOOTLOADER
	...
	switch (deadman_state) {
		case DEADMAN_STATE_DISABLED:
			if (event == DEADMAN_EVENT_ISR_START) {
				deadman_timeout_enab(TRUE);
				deadman_state = DEADMAN_STATE_ENABLED;
			}
			else if (event == DEADMAN_EVENT_DPC_START) {
				/* Valid state as DPCs can be scheduled outside ISRs too */
				deadman_timeout_enab(TRUE);
				deadman_state = DEADMAN_STATE_DPC_STARTED;
			}
			else {
				wrong_state = TRUE;
			}
			break;
		case DEADMAN_STATE_ENABLED:
			if (event == DEADMAN_EVENT_ISR_END) {
				deadman_timeout_enab(FALSE);
				deadman_state = DEADMAN_STATE_DISABLED;
			}
			else if (event == DEADMAN_EVENT_ISR_POST) {
				deadman_state = DEADMAN_STATE_POSTED;
			}
			else {
				wrong_state = TRUE;
			}
			break;
		case DEADMAN_STATE_POSTED:
			if (event == DEADMAN_EVENT_DPC_START) {
				deadman_state = DEADMAN_STATE_DPC_STARTED;
			}
			break;
		case DEADMAN_STATE_DPC_STARTED:
			if (event == DEADMAN_EVENT_DPC_END) {
				deadman_timeout_enab(FALSE);
				deadman_state = DEADMAN_STATE_DISABLED;
			}
			else if (event == DEADMAN_EVENT_ISR_POST) {
	...
	if (wrong_state) {
#ifndef ATE_BUILD
		printf("%s: Unexpected event [%d] in state [%d]!!!\n",
			__func__, event, deadman_state);
		ASSERT(0);
#endif /* ATE_BUILD */
	}
...
hnd_cpu_gtimer_trap_validation(void)
{
	...
	if (BCM43684_CHIP(hnd_sih->chip) || BCM6715_CHIP(hnd_sih->chip)) {
...
#define GTIMER_VALIDATION_RETRY 2
	...
	/* CP15 c14 is not used on any ARMv7 implementation, see Unallocated CP15 encodings on
	 * page B4-27.
	 */
...
{
#if !defined(BCM_BOOTLOADER) && !defined(THREADX_WATCHDOG) && !defined(WLTEST)
	char *deadman_timer;
#endif // endif
...
#if !defined(BCM_BOOTLOADER) && !defined(THREADX_WATCHDOG)
#ifndef WLTEST
	/* Check if entry is present in nvram. By defaule it is enabled */
	deadman_timer = getvar(NULL, rstr_deadman_to);
	if (deadman_timer == NULL) {
		/* If not present, use default deadman time out */
		deadman_to = DEADMAN_DEFAULT_TO;
	} else {
		/* If value is zero, deadman timer would be disabled */
		deadman_to = getintvar(NULL, rstr_deadman_to);
	}
#else
	/* disable deadman timer if mfg builds */
	deadman_to = 0;
#endif /* WLTEST */

#ifdef __ARM_ARCH_7A__
	/* always need to set ALP ILP Period, because not only deadman_to needs
	 * it but also others. e.g. HWA
	 */
	...
	if (deadman_to) {
		/* deadman_to * 10 to keep first decimal value after division. */
		uint32 time = (deadman_to/DEADMAN_REFRESH_FACTOR) * 10;

		/**
		 * refresh every ((deadman timer timeout)/3) seconds.
		 * (time/clock * 100) here as *10 is already done above.
		 */
#ifdef __ARM_ARCH_7A__
		/* enable gtimer */
		hnd_cpu_enable_gtimer(sih);
		refresh_time_ms = (time/hnd_cpu_gtimer_clock(sih)) * 100;
#else
		refresh_time_ms = (time/si_cpu_clock(sih)) * 100;
#endif /* __ARM_ARCH_7A__ */
	}
#elif !defined(BCM_BOOTLOADER)
#ifdef __ARM_ARCH_7A__
	/* always need to set ALP ILP Period, because not only deadman_to needs
	 * it but also others. e.g. HWA
	 */
