 ...
 *
 * $Id: wlc.c 778769 2019-09-10 09:43:50Z $
 */
...
#endif /* WLTINYDUMP */
#endif /* BCMDBG || BCMDBG_DUMP || BCMDBG_PHYDUMP || WLTEST || BCM_DNGDMP */
...
wlc_get_band_by_chanspec(wlc_info_t *wlc, chanspec_t chanspec)
{
	...
	if (wlc->band->bandunit == CHSPEC_BANDUNIT(chanspec)) {
		return wlc->band;
	}
...
#if defined(BCMDBG_MEM) && (defined(BCMDBG) || defined(BCMDBG_DUMP))
static int
...
wlc_get_chan_info(wlc_info_t *wlc, chanspec_t chspec, uint32 *presult)
{
	...
	if (CHSPEC_IS2G(chspec) &&
	    (channel < MAXCHANNEL) && isset(chanvec_all_2G.vec, channel)) {
		result |= WL_CHAN_VALID_HW;
	} else if (CHSPEC_IS5G(chspec) &&
	           (channel < MAXCHANNEL) && isset(chanvec_all_5G.vec, channel)) {
		result |= WL_CHAN_VALID_HW | WL_CHAN_BAND_5G;
	}
#if defined(WL_BAND6G)
	else if (CHSPEC_IS6G(chspec) &&
	         (channel < MAXCHANNEL) && isset(chanvec_all_6G.vec, channel)) {
		result |= WL_CHAN_VALID_HW | WL_CHAN_BAND_6G;
	}
#endif /* WL_BAND6G */
...
BCMINITFN(wlc_init_chanspec)(wlc_info_t *wlc)
{
	...
	if (RSDB_ENAB(wlc->pub))
	{
		...
		if (other_wlc && other_wlc->pub->up &&
			CHSPEC_BAND(other_wlc->chanspec) == CHSPEC_BAND(chanspec)) {
			chanspec = wlc_default_chanspec_by_band(wlc->cmi,
				CHSPEC_BANDUNIT(other_wlc->chanspec));
...
wlc_bandinit_ordered(wlc_info_t *wlc, chanspec_t chanspec)
{
	...
	for (i = 0; i < NBANDS(wlc); i++) {
		enum wlc_bandunit bandunit = band_order[i];
#ifdef WLRSDB
		if (RSDB_ENAB(wlc->pub)) {
			if (!wlc_rsdb_band_match(wlc, wlc->bandstate[bandunit])) {
				continue;
			}
		}
#endif // endif
...
#ifdef BCMSDIO
	WL_INFORM(("wl%d: %s: SDIO Block_size %d SD rev %d\n", unit, __FUNCTION__,
		getintvar(pub->vars, rstr_sdmaxblk), getintvar(pub->vars, rstr_sdrev)));
#endif // endif
...
#endif /* EVENT_LOG_COMPILE */
#ifdef WLRSDB
#if defined(BCMDBG) || defined(BCMDBG_DUMP) || defined(WLTEST) || defined(BCMDBG_RSDB)
	if (RSDB_ENAB(wlc->pub)) {
...
wlc_change_band(wlc_info_t *wlc, enum wlc_bandunit new_band)
{
	...
	/* sanity check arg */
	if (new_band != BAND_2G_INDEX && new_band != BAND_5G_INDEX &&
#if defined(WL_BAND6G)
	    new_band != BAND_6G_INDEX) {
#else
	    TRUE) {
#endif /* WL_BAND6G */
		return BCME_RANGE;
...
wlc_doioctl(void *ctx, uint cmd, void *arg, uint len, struct wlc_if *wlcif)
{
	...
	case WLC_GET_BANDLIST: {
		if (len < sizeof(*pval) * (NBANDS(wlc) + 1)) {
			bcmerror = BCME_BUFTOOSHORT;
			break;
		}
...
wlc_dump_wlc(wlc_info_t *wlc, struct bcmstrbuf *b)
{
	...
	FOREACH_WLC_BAND(wlc, bandunit) {
		if (wlc->bandstate[bandunit]->hwrs_scb != NULL) {
			wlc_scb_dump_scb(wlc, NULL, wlc->bandstate[bandunit]->hwrs_scb, b, -1);
		}
	}
...
static const uint16 BCMATTACHDATA(wlc_devid)[] = {
	...
	BCM6710_D11AX_ID, BCM6710_D11AX2G_ID, BCM6710_D11AX5G_ID, BCM6710_D11AX6G_ID,
	BCM6878_D11AC_ID, BCM6878_D11AC2G_ID, BCM6878_D11AC5G_ID,
	BCM43569_D11AC_ID, BCM43569_D11AC2G_ID, BCM43569_D11AC5G_ID,
	BCM6715_D11AX_ID, BCM6715_D11AX2G_ID, BCM6715_D11AX5G_ID,
...
wlc_mod_prb_rsp_rate_table(wlc_info_t *wlc, wlc_bsscfg_t *cfg, uint frame_len)
{
	...
	sifs = SIFS(CHSPEC_BANDTYPE(cfg->current_bss->chanspec));
...
BCMATTACHFN(wlc_bss_default_init)(wlc_info_t *wlc)
{
	...
	if (RSDB_ENAB(wlc->pub) && (wlc_get_wlcindex(wlc) == 0)) {
		/* fill the default channel for 5G */
		chanspec = wlc_default_chanspec_by_band(wlc->cmi, BAND_5G_INDEX);
	} else
#endif // endif
	{
		/* fill the default channel for 2G */
		chanspec = wlc_default_chanspec_by_band(wlc->cmi, BAND_2G_INDEX);
	}
	...
	if (BSSCFG_STA(bsscfg) && bsscfg->associated &&
		...
		if ((bcmwpa_is_wpa_auth(bsscfg->WPA_auth) &&
		    ...
		    (IS_WAPI_AUTH(bsscfg->WPA_auth) &&
		     (ie = bcm_parse_tlvs(parse, len, DOT11_MNG_WAPI_ID)) != NULL) ||
#endif // endif
		    FALSE) {
...
#ifdef WL11K
	if (SCB_RRM(scb)) {
		sta.flags |= WL_STA_RRM_CAP;
	}
	if (scb->flags3 & SCB3_RRM_BCN_PASSIVE) {
		sta.flags |= WL_STA_RRM_BCN_PASSIVE_CAP;
	}
#endif /* WL11K */
...
wlc_tsf_dump(wlc_info_t *wlc, struct bcmstrbuf *b)
{
	...
	tsf_cfprep = R_REG(osh, D11_CFPRep(wlc));
	tsf_cfpstart = R_REG(osh, D11_CFPStart(wlc));
...
int
wlc_is_singleband_5g(unsigned int devid, unsigned int phycap)
{
	return (_IS_SINGLEBAND_5G(devid) ||
		((phycap & PHY_PREATTACH_CAP_SUP_5G) && !(phycap & PHY_PREATTACH_CAP_SUP_2G)));
}

int
wlc_is_singleband_2g(unsigned int devid, unsigned int phycap)
{
	return (_IS_SINGLEBAND_2G(devid) ||
		((phycap & PHY_PREATTACH_CAP_SUP_2G) && !(phycap & PHY_PREATTACH_CAP_SUP_5G)));
}
 ...
 */
void wlc_pi_band_update(wlc_info_t *wlc, enum wlc_bandunit bandunit)
{
	wlc->band = wlc->bandstate[bandunit];
	wlc->pi = WLC_PI_BANDUNIT((wlc), bandunit);
}
...
{
	const wlc_hw_info_t *wlc_hw = wlc->hw;
	/* So if this is a single band 11a card, use band 1 */
	if (IS_SINGLEBAND_5G(wlc_hw->deviceid, wlc_hw->phy_cap)) {
		if (bandunit != BAND_5G_INDEX) {
			return NULL;
		}
	} else if (IS_SINGLEBAND_2G(wlc_hw->deviceid, wlc_hw->phy_cap)) {
		if (bandunit != BAND_2G_INDEX) {
			return NULL;
		}
	}
