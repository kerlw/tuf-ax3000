 ...
 *
 * $Id: wlc_macdbg.c 778660 2019-09-06 12:21:21Z $
 */
...
#if defined(BCMDBG) || defined(BCMDBG_DUMP) || defined(WLVASIP) || defined(WL_PSMR1)
#include <wlc_hw_priv.h>
#endif /* BCMDBG || BCMDBG_DUMP || WLVASIP || WL_PSMR1 */
...
#if defined(BCMDBG) || defined(BCMDBG_DUMP) || defined(WL_MACDBG)
...
enum {
	...
	IOV_PSMWD_AFTER = 13,	/* manual cause psm wd */
	IOV_PSMXWD_AFTER = 14,	/* manual cause psmx wd */
	IOV_DTRACE_EA = 15, /* driver trace */
	IOV_PSMWD_TXERR = 16, /* manual cause psm wd on tx phy error */
	IOV_LAST
};
...
static const bcm_iovar_t macdbg_iovars[] = {
	...
	{"psmwd_txerr", IOV_PSMWD_TXERR, (IOVF_SET_UP), 0, IOVT_UINT16, 0},
	{NULL, 0, 0, 0, 0, 0}
};
...
wlc_macdbg_up(void *hdl)
{
	...
	if (!(BTCX_ENAB(wlc->hw) && SWBTCX_ENAB(wlc->hw))) {
		if (((macdbg->smpl_ctrl) & SC_STRT) && (macdbg->smpl_info)) {
			...
			params.gpio_sel = WL_MACDBG_GPIO_PC; /* default is psmr0 PC */
...
BCMATTACHFN(wlc_macdbg_init_dumplist)(wlc_macdbg_info_t *macdbg)
{
	...
	svmp_list_t svmpmems_ge128[] = {
		...
		{0x24000, 32}, // TXV_IDX = 66
		{0x26000, 32}  // TXV_IDX = 67
	};
...
wlc_dump_peruser(wlc_info_t *wlc, struct bcmstrbuf *b)
{
	...
	d11regs_list_t reglist[] = {
		...
		{D11REG_TYPE_IHR16, 0x922, 0x00000005, 2, 0},
...
wlc_dump_d11cnts(wlc_info_t *wlc, struct bcmstrbuf * b)
{
	...
	while (i < macdbg->d11print_sz) {
		...
		i++;
		if (!strcmp(macdbg->pd11print[i].name, "txampdu_cnt")) {
			tot_ampdu_cnt = macdbg->d11cnts[i];
		}
	...
	if (wlc->clk) {
		...
		bcm_bprintf(b, "\n%30s %6u", "su ampdu cnt:",
			tot_ampdu_cnt - histo.muagg_sum_ampdu);
...
#endif  /* defined(BCMDBG) || defined(BCMDBG_DUMP) || defined(DUMP_D11CNTS) */
...
wlc_macdbg_frameid_trace_pkt(wlc_macdbg_info_t *macdbg, void *pkt, uint8 fifo,
	uint16 txFrameID, uint16 MacTxControlLow, uint8 epoch, void *scb)
{
	wlc_info_t *wlc = macdbg->wlc;
...
#endif // endif
	macdbg->pkt_hist[macdbg->pkt_hist_cnt].mactc = MacTxControlLow;
...
wlc_macdbg_frameid_trace_dump(wlc_macdbg_info_t *macdbg, uint fifo)
{
	...
	for (k = 0; k < PKT_HIST_NUM_ENT; k++) {
		...
		printf("h %04d pkt:%p data:%p scb:%p flgs:0x%08x flgs3:0x%08x frmid:0x%04x"
			"(0x%04x:fifo %02d) seq:0x%04x epch:%d fifo:%02d mactc:0x%04x\n",
