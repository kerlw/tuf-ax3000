 ...
 *
 * $Id: phy_ac_cache.c 777374 2019-07-30 18:13:54Z $
 */
...
{
	/* radio rev 20707 (6710) and 20709 (6878) will not go to here. */
	phy_info_t *pi = pi_ac->pi;
	phy_param_info_t *p = pi_ac->paramsi;
	...
	FOREACH_CORE(pi, i) {
		...
		if (phy_txiqlocal_num_multilo(pi) != 0) {
			...
			for (multilo_cal_cnt = 1;
				...
				multilo_cal_cnt++) {
				PHY_CAL(("0x%x  ",
					cache->ofdm_txd[i+(multilo_cal_cnt-1)*PHY_CORE_MAX]));
			}
			PHY_CAL(("\n"));
		} else {
	...
	FOREACH_CORE(pi, i) {
		...
		if (phy_txiqlocal_num_multilo(pi) != 0) {
			...
			for (multilo_cal_cnt = 1;
				...
				multilo_cal_cnt++) {
				bcm_bprintf(b, "0x%x  ",
				    cache->ofdm_txb[i+(multilo_cal_cnt-1)*PHY_CORE_MAX]);
			}
			bcm_bprintf(b, "\n");
		} else {
	...
	if (ACMAJORREV_32(pi->pubpi->phy_rev) ||
		...
		FOREACH_ACTV_CORE(pi, coremask, core) {
			...
			for (rf_idx = 0; rf_idx < gmap_size; rf_idx++) {
				...
				if (((rf_idx % 6) == 5) && (rf_idx != gmap_size - 1)) {
					bcm_bprintf(b, "\n   ");
				}
			}
			bcm_bprintf(b, "\n");
