 ...
 *
 * $Id: wlc_tx.c 778769 2019-09-10 09:43:50Z $
 *
 */
...
enum {
	WLC_WRR_WEIGHT_SU =	0,
	WLC_WRR_WEIGHT_VHTMU = 1,
	WLC_WRR_WEIGHT_HEOF =	2,
	WLC_WRR_WEIGHT_HEMU =	3,
	WLC_WRR_WEIGHT_END /* Add new entries above. Leave marker at the end */
};
...
#define WLC_WRR_WEIGHT_HEOF_DEFAULT		WLC_WRR_WEIGHT_DISABLED
...
#define WLC_WRR_WEIGHT_HEOF_ADAPTIVE	WLC_WRR_FB_BASE_WEIGHT
...
#define WLC_WRR_BYTEOFF_HEOF(wlc)	(MX_WRR_QUOTA(wlc) + 4)
#define WLC_WRR_BYTEOFF_HEMU(wlc)	(MX_WRR_QUOTA(wlc) + 6)

#define WLC_WRR_NUM_WEIGHTS			WLC_WRR_WEIGHT_END
...
static int wlc_tx_vhtmu_nominal_users_perppdu(wlc_info_t *wlc,
	muagg_histo_t *histo, uint32 *nom_users);
static int wlc_tx_hemu_nominal_users_perppdu(wlc_info_t *wlc,
	muagg_histo_t *histo, uint32 *nom_users);
...
#define WLC_WRR_HISTO_BYTEOFF_HEOF(wlc)		(MX_WRR_HISTO(wlc) + 4)
#define WLC_WRR_HISTO_BYTEOFF_HEMU(wlc)		(MX_WRR_HISTO(wlc) + 6)
	...
	void *params, uint p_len, void *arg, uint len, uint vsize, struct wlc_if *wlcif)
{
	...
	case IOV_SVAL(IOV_WRR_WEIGHT): {
		if (WLC_WRR_ENAB(txqi->wlc->pub)) {
			...
			weight[WLC_WRR_WEIGHT_HEOF] = wrr_params->data[WLC_WRR_WEIGHT_HEOF];
	...
	case IOV_GVAL(IOV_WRR_WEIGHT): {
		if (WLC_WRR_ENAB(txqi->wlc->pub)) {
			uint16 weight[WLC_WRR_NUM_WEIGHTS] = {0, 0, 0, 0};
			...
			if (err == BCME_OK) {
				...
				wrr_params->data[WLC_WRR_WEIGHT_HEOF] =
					weight[WLC_WRR_WEIGHT_HEOF];
	...
	if (WAPI_WAI_RESTRICT(wlc, bsscfg) && !SCB_ISMULTI(scb) && !SCB_AUTHORIZED(scb)) {
		if (!is_wai) {
			WL_ERROR(("wl%d.%d: non-WAI frame len %d to unauthorized station %s\n",
				...
				bcm_ether_ntoa(&scb->ea, eabuf)));
			WL_TX_STS_UPDATE(toss_reason, WLC_TX_STS_TOSS_NWAI_FRAME_NOTAUTH);
			goto toss;
		}
	}
#endif /* BCMWAPI_WAI */
...
wlc_prep_pdu(wlc_info_t *wlc, struct scb *scb, void *pdu, uint fifo)
{
	 ...
	 */
	if (scb->bandunit != CHSPEC_BANDUNIT(WLC_BAND_PI_RADIO_CHANSPEC)) {
		PKTFREE(wlc->osh, pdu, TRUE);
	...
	ratespec_t rspec_override)
{
	...
	if (bf_shmx_idx != BF_SHM_IDX_INV) {
		...
		wlc_fifo_isMU(wlc->fifo, scb, ac)) {
			...
			wlc_check_fifo_type(wlc->fifo, scb, ac, WL_MU_VHTMU);
#endif // endif
	...
	ratespec_t rspec_override)
{
	...
	if (TXBF_ENAB(wlc->pub) && SCB_MU(scb) &&
		...
		wlc_fifo_isMU(wlc->fifo, scb, ac)) {
#if defined(BCMDBG) || defined(WLTEST)
			mu_type_t mu = SCB_VHTMU(scb) ? WL_MU_VHTMU:WL_MU_HEMMU;
...
#if defined(BCMDBG) || defined(WLTEST)
		mu_type_t mu = SCB_HEMMU(scb) ? WL_MU_HEMMU:WL_MU_DLOFDMA;
		if (wlc_twt_scb_active(wlc->twti, scb)) {
			mu = WL_MU_TWT;
		}
		wlc_check_fifo_type(wlc->fifo, scb, ac, mu);
...
wlc_recover_pkt_scb(wlc_info_t *wlc, void *pkt)
{
	...
	if (scb == wlc->band->hwrs_scb ||
		...
		/* Even if the packet belongs to an hardware SCB, we should drop the
		 * packet if it belongs to a bsscfg that is down.
		 */
		if (bsscfg && bsscfg->enable && bsscfg->up) {
			return scb;
		} else {
			WLPKTTAGSCBSET(pkt, NULL);
			return NULL;
...
wlc_tx_fifo_sync_complete(wlc_info_t *wlc, void *fifo_bitmap, uint8 flag)
{
	...
	for (i = 0; i < WLC_HW_NFIFO_INUSE(wlc); i++, txq_swq++) {
		...
		WL_ERROR(("MQ: %s: fifo %d: collected %d pkts\n", __FUNCTION__,
...
uint16 weight_in[WLC_WRR_NUM_WEIGHTS], uint16 weight_out[WLC_WRR_NUM_WEIGHTS])
{
	...
	if (weight_in[WLC_WRR_WEIGHT_SU] == WLC_WRR_WEIGHT_DISABLED) {
		...
		weight_out[WLC_WRR_WEIGHT_HEOF] = WLC_WRR_WEIGHT_DISABLED;
		return;
	}
	...
	weight_out[WLC_WRR_WEIGHT_HEOF] =
		(weight_in[WLC_WRR_WEIGHT_HEOF] < wrr->weight_max[WLC_WRR_WEIGHT_HEOF]) ?
		weight_in[WLC_WRR_WEIGHT_HEOF] : wrr->weight_max[WLC_WRR_WEIGHT_HEOF];
	...
	if (WRR_PSMX_ENAB(wlc->pub)) {
		...
		if (wlc->hw->clk) {
			...
			if (weight[WLC_WRR_WEIGHT_SU] == WLC_WRR_WEIGHT_DISABLED) {
				...
				wlc_write_shmx(wlc, WLC_WRR_BYTEOFF_HEOF(wlc),
				 wrr->weight[WLC_WRR_WEIGHT_HEOF]);
				wlc_write_shmx(wlc, WLC_WRR_BYTEOFF_HEMU(wlc),
				 wrr->weight[WLC_WRR_WEIGHT_HEMU]);
			} else {
			 /* NOTE: Write in strict  reverse order */
				wlc_write_shmx(wlc, WLC_WRR_BYTEOFF_HEMU(wlc),
				 wrr->weight[WLC_WRR_WEIGHT_HEMU]);
				wlc_write_shmx(wlc, WLC_WRR_BYTEOFF_HEOF(wlc),
				 wrr->weight[WLC_WRR_WEIGHT_HEOF]);
...
wlc_tx_get_wrr_weights(wlc_info_t* wlc, uint16 weight[WLC_WRR_NUM_WEIGHTS])
{
	...
	if (WRR_PSMX_ENAB(wlc->pub) && (wlc->hw->clk)) {
		...
		weight[WLC_WRR_WEIGHT_HEOF] = wlc_read_shmx(wlc, WLC_WRR_BYTEOFF_HEOF(wlc));
	} else {
		wrr_state_t *wrr = wlc->txqi->wrr;
		...
		weight[WLC_WRR_WEIGHT_HEOF] = wrr->weight[WLC_WRR_WEIGHT_HEOF];
...
wlc_tx_set_wrr_weights_max(wlc_info_t* wlc, uint16 weight[WLC_WRR_NUM_WEIGHTS])
{
	if (WRR_PSMX_ENAB(wlc->pub)) {
		...
		wrr->weight_max[WLC_WRR_WEIGHT_HEOF] = weight[WLC_WRR_WEIGHT_HEOF];
...
wlc_tx_get_wrr_weights_max(wlc_info_t* wlc, uint16 weight[WLC_WRR_NUM_WEIGHTS])
{
	...
	if (WRR_PSMX_ENAB(wlc->pub)) {
		...
		weight[WLC_WRR_WEIGHT_HEOF] = wrr->weight_max[WLC_WRR_WEIGHT_HEOF];
...
wlc_tx_clear_wrr_histo(wlc_info_t* wlc)
{
	...
	if (WRR_PSMX_ENAB(wlc->pub) && (wlc->hw->clk)) {
		...
		wlc_write_shmx(wlc, WLC_WRR_HISTO_BYTEOFF_HEOF(wlc), 0);
...
wlc_tx_get_wrr_histo(wlc_info_t* wlc, uint16 weight[WLC_WRR_NUM_WEIGHTS])
{
	...
	if (WRR_PSMX_ENAB(wlc->pub) && (wlc->hw->clk)) {
		...
		weight[WLC_WRR_WEIGHT_HEOF] = wlc_read_shmx(wlc, WLC_WRR_HISTO_BYTEOFF_HEOF(wlc));
...
wlc_tx_wrr_dump_history(struct bcmstrbuf *b, wrr_counters_t *wrr_counter)
{
	...
	bcm_bprintf(b, "     W:  SU  VHTMU  HEMU  HEOF    U:   SU  VHTMU  HEMU  HEOF\n");
	for (n = 0; n < WLC_WRR_WEIGHT_HISTORY_SIZE; n++) {
		bcm_bprintf(b, "%3d: %5d %5d %5d %5d       %5d %5d %5d %5d\n", n,
...
#define _PCT(x) ((100*(x))/sum)
	...
	wlc_info_t *wlc = ctx;
	uint32 sum;
	uint32 pct_su, pct_vhtmu, pct_ofdma, pct_hemu;
	...
	bcm_bprintf(b, "WRR Weights     SU:%-6u VHTMU:%-6u HEMU:%-6u HEOF:%-6u\n",
		weight[WLC_WRR_WEIGHT_SU],
		weight[WLC_WRR_WEIGHT_VHTMU],
		weight[WLC_WRR_WEIGHT_HEMU],
		weight[WLC_WRR_WEIGHT_HEOF]);
	...
	bcm_bprintf(b, "WRR Max Weights SU:%-6u VHTMU:%-6u HEMU:%-6u HEOF:%-6u\n",
		weight[WLC_WRR_WEIGHT_SU],
		weight[WLC_WRR_WEIGHT_VHTMU],
		weight[WLC_WRR_WEIGHT_HEMU],
		weight[WLC_WRR_WEIGHT_HEOF]);
	...
	bcm_bprintf(b, "\nPPDU histogram:\n");
	bcm_bprintf(b, " SU AMPDU:      %8d (%d%%)\n",
		wrr_counter->wrr_su_ampdus, pct_su);
	bcm_bprintf(b, " VHTMU PPDU:    %8d (%d%%)\n",
		wrr_counter->wrr_vhtmu_ppdus, pct_vhtmu);
	bcm_bprintf(b, " HEMU PPDU:     %8d (%d%%)\n",
		wrr_counter->wrr_hemu_ppdus, pct_hemu);
	bcm_bprintf(b, " DLOFDMA PPDU:  %8d (%d%%)\n",
		wrr_counter->wrr_ofdma_ppdus, pct_ofdma);
	bcm_bprintf(b, " TOTALS:        %8d (%d%%)\n",
		sum, pct_su + pct_vhtmu + pct_hemu + pct_ofdma);

	if (wlc->hw->clk) {
		muagg_histo_t histo;
		uint nominal_ofdma_users;
		uint nominal_vhtmu_users;
		uint nominal_hemu_users;
		...
		wlc_tx_hemu_nominal_users_perppdu(wlc,
			&histo, &nominal_hemu_users);
		wlc_tx_vhtmu_nominal_users_perppdu(wlc,
			&histo, &nominal_vhtmu_users);

		bcm_bprintf(b, "\nNominal users per PPDU: VHTMU:%u HEMU:%u DLOFDMA:%u\n",
			nominal_vhtmu_users, nominal_hemu_users, nominal_ofdma_users);
	}
...
wlc_tx_update_wrr_histo(wlc_info_t* wlc)
{
	if (wlc->hw->clk) {
		...
		wrr_counter->wrr_ofdma_ppdus += histogram[WLC_WRR_WEIGHT_HEOF];
		wrr_counter->wrr_hemu_ppdus += histogram[WLC_WRR_WEIGHT_HEMU];
...
wlc_tx_wrr_init_adaptive_weights(wlc_info_t* wlc)
{
	...
	weight[WLC_WRR_WEIGHT_HEOF] = WLC_WRR_WEIGHT_HEOF_ADAPTIVE;
...
static int
wlc_tx_hemu_nominal_users_perppdu(wlc_info_t *wlc,
	muagg_histo_t *histo, uint32 *nom_users)
{
	int ret = BCME_OK;

	*nom_users = wlc_tx_nominal_users(wlc,
		&histo->bucket[MUAGG_HEMUMIMO_BUCKET], MAX_HEMUMIMO_USERS);
...
wlc_tx_set_wrr_ratio(wlc_info_t* wlc, uint32 active_su_users,
	...
	uint32 active_hemu_users, uint32 nominal_hemu_users,
	uint32 active_vhtmu_users, uint32 nominal_vhtmu_users)
{
	...
	if (active_su_users &&
		...
		/* HE-MU weights (W-MUMIMO = ROUND(N-HEMIMO/M-HEMIMO)) */
		weight[WLC_WRR_WEIGHT_HEMU] = (active_hemu_users) ?
			_ROUNDING_DIVISION(active_hemu_users, nominal_hemu_users) : 1;

		/* OFDMA DL weights */
		/* W-OFDMA = ROUND(N-OFDMA/M-OFDMA) */
		weight[WLC_WRR_WEIGHT_HEOF] = (active_ofdma_users) ?
...
#if defined(BCMDBG) || defined(BCMDBG_DUMP)
	{
		...
		users[WLC_WRR_WEIGHT_HEMU] = active_hemu_users;
		users[WLC_WRR_WEIGHT_HEOF] = active_ofdma_users;
...
wlc_tx_update_wrr_weights(wlc_info_t* wlc)
{
	...
	if (active_users) {
		if (wlc->hw->clk) {
			...
			if (nominal_hemu_users == 0) {
				nominal_hemu_users = 3;
			}
		...
		wlc_tx_set_wrr_ratio(wlc, active_su_users,
			...
			active_hemu_users, nominal_hemu_users,
 ...
 */
	...
	if (wrr) {
		...
		wrr->weight_max[WLC_WRR_WEIGHT_HEOF] = WLC_WRR_WEIGHT_MAX_DEFAULT;
		...
		if (WLC_WRR_ADAPTIVE_DEFAULT == WLC_WRR_ADAPTIVE_ON) {
			...
			wrr->weight[WLC_WRR_WEIGHT_HEOF] = WLC_WRR_WEIGHT_HEOF_ADAPTIVE;
		} else {
			...
			wrr->weight[WLC_WRR_WEIGHT_HEOF] = WLC_WRR_WEIGHT_HEOF_DEFAULT;
		}
