 ...
 *
 * $Id: wlc_tdls.c 778655 2019-09-06 11:27:56Z $
 */
...
wlc_tdls_create_oper_mode(wlc_info_t *wlc, struct scb *scb, wlc_bsscfg_t *cfg)
{
	...
	band = wlc->bandstate[CFG_BAND_INDEX(cfg)];
	...
	void *p, uint8 action_field, uint16 status_code, uint8 token, void *req)
{
	...
	if (CHSPEC_BAND(parent->current_bss->chanspec) ==
			WL_CHANSPEC_BAND_2G) {
		tdls_band_type = WLC_BAND_2G;
	} else {
		tdls_band_type = WLC_BAND_5G;
	}
...
wlc_tdls_bsscfg_create(wlc_info_t *wlc, wlc_bsscfg_t *parent, bool initiator)
{
	...
	band = wlc->bandstate[CFG_BAND_INDEX(bsscfg)];
