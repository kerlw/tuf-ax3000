 ...
 *
 * $Id: wlc_mfp.c 776538 2019-07-02 07:35:10Z $
 *
	...
	uint *iv_len, uint *tail_len)
{
	...
	*tail_len = 0;
	...
	do {
		...
		if (!bsscfg || !bcmwpa_includes_wpa2_auth(bsscfg->WPA_auth) ||
	...
	d11rxhdr_t *rxhdr, struct dot11_management_header *hdr, void *p)
{
	...
	if (!(SCB_MFP(scb) && bsscfg && bcmwpa_includes_wpa2_auth(bsscfg->WPA_auth))) {
		ret = !(fc & FC_WEP);
		if (!ret)
